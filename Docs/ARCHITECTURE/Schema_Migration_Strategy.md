...
- Production code migration ongoing
...
### Phase 3: Field Renames (Complete — v3.0/v3.1)
- **Breaking change applied**: All legacy JSON field names removed from codebase
- Renamed `facts[]` → `evidenceItems[]`
- Renamed `fact` field → `statement`
- Renamed `supportingFactIds` → `supportingEvidenceIds`
- Renamed `distinctProceedings` → `analysisContexts`
- Renamed `relatedProceedingId`/`proceedingId` → `contextId`
- Schema versioning at 3.0.0
- Migration script: `apps/api/scripts/migrate-terminology-v2.7.ts`

See TERMINOLOGY.md for full v3.0/v3.1 migration details.
