...
> 
> **xWiki file**: `Docs/xwiki-pages/FactHarbor_Spec_and_Impl/FactHarbor/Specification/Implementation/Pipeline Architecture/Quality Gates Reference/WebHome.xwiki`
> 
...
- Overview.md (Quality Gates section)
- Calculations.md (Gate 4 sections)
- Pipeline_TriplePath_Architecture.md (quality gates mentions)
...
2. **Prediction/Speculation**: Future-oriented claims that cannot be verified
   - Example: "Technology Y will dominate the market by 2030"
   - Action: Exclude unless claim is central to thesis

3. **Low Specificity**: Vague statements without concrete assertions
...
2. Deterministic filter applies Gate 1 criteria
3. Excluded claims logged with reasons
4. Valid claims proceed to research phase
...
2. Count facts extracted from sources
3. Measure reasoning length
4. Assign confidence tier
5. Apply context scoping for counter-evidence
...
- **High confidence** (90%): 72 + 28×0.9 = 97% → **TRUE**
- **Medium confidence** (60%): 72 + 28×0.6 = 89% → **TRUE**
- **Low confidence** (30%): 72 + 28×0.3 = 80% → **MOSTLY-TRUE**
...
2. **Opinion editorial** → Expect: Most claims excluded by Gate 1
3. **Low-source analysis** → Expect: Low Gate 4 confidence tiers

### 10.3 Manual Testing

**Steps**:
1. Run analysis on test input
2. Check result JSON for `qualityGates` object
3. Verify exclusion reasons match expected criteria
4. Verify confidence tiers match source/fact counts

---

## 11. Related Documentation

- [Calculations.md](Calculations.md) - Verdict calculation methodology, confidence modulation
- [Overview.md](Overview.md) - Architecture overview, pipeline flow
- [Pipeline_TriplePath_Architecture.md](Pipeline_TriplePath_Architecture.md) - Pipeline variants and quality gate enforcement
- [Evidence_Quality_Filtering.md](Evidence_Quality_Filtering.md) - Evidence filtering (related to proposed Gate 3)
...
2. **Output quality control** (Gate 4) - Verdicts backed by sufficient evidence
3. **Transparency** - Gate stats included in every result
4. **Configurability** - Thresholds adjustable via UCM
