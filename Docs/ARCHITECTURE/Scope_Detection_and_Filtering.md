...
This document explains how FactHarbor detects and manages **Contexts** (AnalysisContexts / bounded analytical frames), how context relevance is enforced, and how claim relevance (`thesisRelevance`) affects research, verdicting, and UI.
...
| **ArticleContext** | Narrative/background framing of the input. NOT a reason to split. | `proceedingContext` field (name kept for backward compat) |
| **EvidenceScope** | Per-fact source methodology/boundaries. Attached to individual facts. | `ExtractedFact.evidenceScope` |

### Key distinctions

- **AnalysisContext** = TOP-LEVEL analytical frames (e.g., "TSE Electoral Case", "STF Criminal Case")
- **EvidenceScope** = PER-FACT source metadata (e.g., "WTW methodology", "EU regulatory framework")
- **ArticleContext** = Background narrative (NOT a reason to create separate AnalysisContexts)

### Prompt note (synonyms)

In prompts, you may use **scope / bounded context / boundary** as synonyms for *AnalysisContext* **only if** the prompt includes a terminology glossary. Avoid the bare word "context" unless explicitly referring to **ArticleContext** or "bounded context" (AnalysisContext).

## Where contexts come from (pipeline)
...
2. **Supplemental context pass (optional)**: If the initial understanding under-splits, a best-effort context-only prompt may propose additional contexts.
3. **Evidence-driven refinement**: After research produces facts, the system may refine contexts using facts (and per-fact `evidenceScope`) and remap assignments.
...
- Do **not** create contexts from unrelated domains just because they share a broad category.
- Do **not** create "perspective contexts" (e.g., "US view" vs "Brazil view") unless the evidence defines a distinct analytical frame.
- Prefer fewer contexts when uncertain.
- A context with **zero relevant claims and zero relevant evidence** must not persist.

### Enforcement

FactHarbor enforces this in two layers:

- **Prompt constraints**: Context-producing prompts explicitly include the Context Relevance Requirement.
- **Deterministic backstop**: Contexts with **zero claims and zero facts** are pruned before verdict generation, and `requiresSeparateAnalysis` is recomputed.

Additionally, when analysis has **2+ contexts** and some claims cannot be assigned to a specific context, FactHarbor creates a special context:
