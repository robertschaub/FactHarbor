...
- **Enabled**: Loads `KNOWLEDGE_INSTRUCTION_ALLOW_MODEL` — LLM may supplement evidence with training knowledge
- **Disabled**: Loads `KNOWLEDGE_INSTRUCTION_EVIDENCE_ONLY` — LLM must rely strictly on fetched evidence
...
**Changes**:
- Orchestrated search relevance mode instructions moved from inline code to prompt file sections (`SEARCH_RELEVANCE_MODE_STRICT/MODERATE/RELAXED`) (commit ef2def6)
