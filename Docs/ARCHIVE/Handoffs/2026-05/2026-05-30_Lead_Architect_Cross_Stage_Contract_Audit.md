...
- **RESOLVED — live variance test (2026-05-30, Captain-approved (a); live runs at HEAD `e04f26ef` = `8061954e` analyzer):** Re-ran the Hydrogen input **4× fresh**. **All 4 passed Stage-1 contract validation (`preservesContract: true`) and produced in-band verdicts** — `FALSE 8/79`, `FALSE 14/71`, `FALSE 9/75`, `MOSTLY-FALSE 15/65` (benchmark band: FALSE/MOSTLY-FALSE, truth 5-25, conf 65-85). The failure is not a stable regression. **[CORRECTED 2026-05-30 per Captain — scored against the FULL documented bar (≥2 boundaries + TTW/full-pathway *distinct*, not just verdict bands): only 2/5 meet it.** `460c52ad` conflates the dimensions (two WTW boundaries, NO TTW boundary, single undecomposed claim — Captain-flagged); `b885c508` borderline (methodology-clustered boundaries). So this is **extraction-decomposition instability — Locus L1-dominant**, NOT simply "cause #2 LLM judgment variance"; `report_damaged` is one tail of the L1 distribution. L2/L3 fixes would not address the conflation cases. See proposal §8.]** The same preliminary-fetch 403s appear in passing runs too, so they are not the determinant.
...
1. **[DONE 2026-05-30] Variance test — NOT a stable regression; scored against the FULL documented bar, only 2/5 meet it** (per Captain — bands + ≥2 boundaries + TTW/full-pathway distinct). `460c52ad` conflates dimensions; `b885c508` borderline. Dominant cause = **extraction-decomposition instability (L1)**; `report_damaged` is one tail. **⇒ No bisect, no revert, no rule/prompt edit; L2/L3 alone insufficient.**
2. **[DONE 2026-05-30] Gate-hardening fix designed + reviewed.** See `Docs/ARCHIVE/WIP/2026-05-30_Stage1_Contract_Gate_Hardening_Proposal.md`. **Option A (prune-and-revalidate) REJECTED** by 3 reviewers + advisor (doesn't fire on the actual failure; circular re-validation; can publish a half-the-input verdict). **Fix locus unresolved** — L1 extraction-shape variance (favored by 5/5 claim-count↔outcome), L2 validator-flip (→ resample), L3 `selfContradicted` code-check false-positive — needing different fixes.
