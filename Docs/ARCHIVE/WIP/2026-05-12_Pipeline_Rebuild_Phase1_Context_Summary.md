...
On the reset main branch this document is retained as historical design context, not as a mandate to recreate the later Captain-Deputy process. Escalate only if a decision is high-risk, reviewers materially disagree, or the decision would materially change product behavior, UI/API/report compatibility, persisted data behavior, validation spend, production/secrets/security posture, or the governing intent.
