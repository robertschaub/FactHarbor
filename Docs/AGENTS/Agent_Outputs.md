...
---
### 2026-04-29 | Unassigned | Codex (GPT-5) | Handelsregister Entry Letter Filing And Checklist Status Upgrade -- [Standard] [open-items: yes]
**For next agent:** The official entry letter received on `2026-04-29` was copied into the repo as [Handelsregister_Eintrag_FactHarbor_2026-04-29.pdf](/c:/DEV/FactHarbor/Docs/Legal/Handelsregister_Eintrag_FactHarbor_2026-04-29.pdf). [NPO_Formation_Checklist.md](/c:/DEV/FactHarbor/Docs/Legal/NPO_Formation_Checklist.md) was upgraded from “filing submitted / entry pending” to “entry letter received”, the document inventory now includes the stored letter, and the timeline was updated. Zefix visibility was intentionally not marked complete without a direct verification step.
...
---
### 2026-04-27 | Senior Developer | Codex (GPT-5) | UNVERIFIED Ambiguity Discipline -- [Standard] [open-items: yes]
**For next agent:** Captain clarified that every `UNVERIFIED` report is suspicious and may only be accepted as analytical reality after documented deep research exhausts accessible material evidence and source acquisition appears healthy. Codified this in `AGENTS.md`, `.claude/skills/pipeline/SKILL.md`, and `.claude/skills/report-review/SKILL.md`. Current job `b8def4575c0749288a76c138838934d9` is classified `mixed`, leaning `factharbor-miss`: prior exact-input jobs varied across `MIXED`, `LEANING-FALSE`, and `UNVERIFIED`; current local signals include `budget_exceeded`, zero contradiction iterations, zero contradiction sources found, skewed evidence balance, and AC_12 integrity downgrade; external web research found accessible SEM, BFS/PKS, SECO, SKOS/BFS, and parliamentary/PVK evidence for major subclaims.
**Warnings:** This is workflow enforcement for agents, not a runtime pipeline guard. Application code can still emit `UNVERIFIED`; agents must not accept it without escalation. No live rerun was submitted because Live Job Submission Discipline requires commit and runtime refresh.
**Learnings:** `UNVERIFIED` is a suspicious hypothesis, not an acceptable terminal explanation, until same-input history and external web research rule out missed accessible evidence.
-> Docs/AGENTS/Handoffs/2026-04-27_Senior_Developer_UNVERIFIED_Ambiguity_Discipline.md

---
### 2026-04-27 | Senior Developer | Codex (GPT-5) | UNVERIFIED Skill Debate Review -- [Standard] [open-items: yes]
**For next agent:** Debate team reviewed the UNVERIFIED ambiguity workflow changes. Advocate supported adoption; Challenger objected to unbounded “deep research” and too-absolute wording; Reconciler verdict was `MODIFY`. Applied wording amendments in `AGENTS.md`, `.claude/skills/pipeline/SKILL.md`, and `.claude/skills/report-review/SKILL.md`: `UNVERIFIED` remains always suspicious for review purposes, but now explicitly means unresolved rather than automatic FactHarbor failure; external checks are bounded with stop conditions; full escalation is scoped to `/report-review`, quality/regression review, suspicious-job debugging, or any conclusion of genuine scarcity; automatic live reruns remain disallowed without approval/justification.
**Warnings:** This is workflow text only. Runtime Stage 5 guard from the previous patch remains separate. Open follow-up remains Stage 1/claim-selection stability for long multi-claim PDFs.
-> Docs/AGENTS/Handoffs/2026-04-27_Senior_Developer_UNVERIFIED_Ambiguity_Discipline.md

---
### 2026-04-27 | Senior Developer | Codex (GPT-5) | UNVERIFIED Research Exhaustion Gate -- [Standard] [open-items: yes]
**For next agent:** Current SVP PDF job `b8def4575c0749288a76c138838934d9` is not acceptable analytical scarcity: exact-input history used different prepared/selected `AtomicClaim` sets (prepared counts 18/34/38/50 where available), previous verdicts varied across `MIXED`, `LEANING-FALSE`, and `UNVERIFIED`, and the current run had `budget_exceeded` plus zero contradiction iterations/sources. Runtime guard added: `aggregateAssessment(...)` now calls `enforceUnverifiedResearchExhaustionGate(...)`, which emits degrading `unverified_research_incomplete` and fails `qualityGates.passed` when final article verdict is `UNVERIFIED` before required research exhaustion. Warning type registered in `types.ts` and `warning-display.ts`; focused tests and web build passed.
**Warnings:** Old stored jobs are unchanged until rerun. This fix blocks false-passed `UNVERIFIED` reports; it does not solve Stage 1/claim-selection instability for long multi-claim PDFs. A follow-up should stabilize or better surface prepared/selected `AtomicClaim` variance.
**DEBT-GUARD RESULT:** Classification: incomplete-existing-mechanism. Chosen option: amend existing Stage 5 quality-gate enforcement. Rejected paths: truth-scale changes, domain-specific search rules, or mixing Stage 1 extraction changes into this patch. Failed-validation recovery: kept the guard and amended it to tolerate legacy partial state objects after `npm test` exposed missing `warnings` in an existing aggregation test. Verification: focused Vitest, `tsc --noEmit`, `npm -w apps/web run build`, `git diff --check`; `npm test` later only failed unrelated runner integration timeouts, and those timed-out files passed in isolation.
-> Docs/AGENTS/Handoffs/2026-04-27_Senior_Developer_UNVERIFIED_Ambiguity_Discipline.md

---
### 2026-04-27 | Lead Architect | Codex (GPT-5) | SVP ACS Research Waste Investigation -- [Standard] [open-items: yes]
**For next agent:** Investigated the suspected waste where broad inputs produce many `AtomicClaim` candidates and later select only up to 5. Finding: draft-backed final jobs already filter prepared understanding to `selectedClaimIds` before Stage 2, so direct post-ACS research on unselected claims is not the main issue. The real waste/risk is Stage 1 preliminary evidence work over pre-selection candidates, lack of persisted source/full-text reuse from preparation into the final selected job, and fixed final research time budget that can be consumed before contradiction research. SVP job `b8def4575c0749288a76c138838934d9` had 18 prepared candidates, 5 selected claims, 38 LLM calls, 4 main iterations, 0 contradiction iterations/sources, 41 total evidence items, 0 contradicting evidence items, and `budget_exceeded` before ending `UNVERIFIED`.
**Warnings:** No code changes or live reruns were made. Do not blindly cap article-like candidate extraction; the current bypass preserves broad input contract coverage. Use `/debt-guard` before implementation, and keep selection/relevance decisions LLM-powered and UCM-managed.
**Learnings:** For ACS jobs, inspect `PreparedStage1Json` and `ClaimSelectionJson` first. The next improvement should measure selected-vs-dropped preliminary work, persist a bounded selected preliminary source bundle into final jobs, reserve research time for contradiction, and then make ACS recommendation budget-aware.
-> Docs/AGENTS/Handoffs/2026-04-27_Lead_Architect_SVP_ACS_Research_Waste_Investigation.md

---
### 2026-04-27 | Lead Architect | Codex (GPT-5) | SVP ACS Debate Consolidation -- [Standard] [open-items: yes]
**For next agent:** FULL-tier `/debate` reviewed the SVP ACS research-waste investigation. Reconciler verdict: `MODIFY`. Adopt the staged direction, but do not implement source-bundle reuse or ACS prompt/config behavior yet. First implement shadow instrumentation for selected-vs-dropped preliminary work and Stage 1→Stage 2 source overlap; protect contradiction work as UCM-configurable admission/reservation so main research cannot consume the entire budget; keep article-like claim-contract coverage and do not restore a blind cap. Treat source-bundle reuse as a gated hypothesis: initially persist only provenance hashes and exact selected-source URL references in shadow metrics, then enable bounded document/data artifact reuse only after overlap metrics and safety design prove value. Budget-aware ACS requires LLM Expert review and explicit budget-fit/defer semantics.
**Warnings:** No code changes or live reruns were made. `/debt-guard` is required before implementation. Source reuse must preserve the existing document/data vs HTML safety boundary. The decision is valid as a gated plan, not as implementation approval for reuse or ACS prompt/config changes.
**Learnings:** Reviewer consensus moved the first slice from "persist selected preliminary source bundle" to "instrument + contradiction admission first"; source reuse remains plausible but unproven until G1/G2/G3 are closed.
-> Docs/AGENTS/Handoffs/2026-04-27_Lead_Architect_SVP_ACS_Debate_Consolidation.md

---
### 2026-04-27 | Lead Architect | Codex (GPT-5) | SVP ACS Gated Implementation Plan -- [Standard] [open-items: yes]
**For next agent:** Documented the execution plan at `Docs/WIP/2026-04-27_SVP_ACS_Gated_Implementation_Plan.md`. The first implementation branch is limited to shadow instrumentation plus contradiction admission: measure selected-vs-dropped preliminary work, Stage 1 to Stage 2 source overlap, per-selected-claim research cost, and contradiction reachability; then protect contradiction work through UCM-configurable admission/reservation and emit research-incomplete degradation when minimum contradiction work cannot run. Source artifact reuse, ACS budget-aware prompt/config behavior, and Stage 1 two-lane redesign are explicitly gated and not first-branch work.
**Warnings:** This is documentation only. `/debt-guard` remains mandatory before implementation. Source reuse requires metrics plus safety design before artifact persistence/reuse; ACS budget behavior requires LLM Expert review; live validation requires commit and runtime/config refresh first.
**Learnings:** The actionable implementation baseline is now narrower than the original investigation: Slices 1-2 are ready; later slices are decision gates, not approved code changes.
-> Docs/WIP/2026-04-27_SVP_ACS_Gated_Implementation_Plan.md

---
### 2026-04-27 | Lead Architect | Codex (GPT-5) | SVP ACS Plan Specificity Review -- [Standard] [open-items: yes]
**For next agent:** A focused reviewer pass hardened `Docs/WIP/2026-04-27_SVP_ACS_Gated_Implementation_Plan.md` against vagueness. The plan now names the baseline SVP sanity case, states metrics are observability only, defines attribution by stable candidate/AtomicClaim IDs, defines exact normalized URL overlap, defines structural source-family values, specifies the minimum `researchWasteMetrics` schema, fixes the output location to final-job observability with optional draft preparation mirror, and tightens Slice 1 tests. Slice 2 now has an explicit contradiction-admission algorithm, UCM/default requirements, registered research-incomplete signal contract, and tests for protected budget, adequate budget, disabled admission, and config drift. Slices 3-5 now have explicit allowed outcomes and non-approval boundaries.
**Warnings:** This remains documentation only. Numeric threshold values are still intentionally not hardcoded in the plan; implementation-owned UCM defaults must be added in JSON and TypeScript schema together. Source artifact reuse and budget-aware ACS remain gated, not approved.
**Learnings:** "Gated plan" was too loose until each gate stated allowed outcomes, non-authorized outcomes, and exact data contracts; future architecture plans should include those from the start.
-> Docs/WIP/2026-04-27_SVP_ACS_Gated_Implementation_Plan.md

---
### 2026-04-27 | Lead Architect | Codex (GPT-5) | SVP ACS Opus Review Corrections -- [Standard] [open-items: yes]
**For next agent:** Applied the Claude Opus review corrections to `Docs/WIP/2026-04-27_SVP_ACS_Gated_Implementation_Plan.md`. The plan now resolves the URL normalization conflict by calling out `EvidenceDeduplicator.normalizeUrl()` extraction into a shared utility, preserving tracking-param removal while adding trailing-slash handling and keeping non-tracking query params. It now uses a concrete contradiction-admission field/formula: `contradictionProtectedTimeMs` with `elapsedMs + contradictionProtectedTimeMs > researchTimeBudgetMs`. It clarifies that existing `ClaimSelectionStage1Observability.candidateClaimCount` remains the Stage 1 field while `researchWasteMetrics.preparedCandidateCount` is the final-job waste-analysis copy/derivation and must stay synced. It also includes the iteration-duration variance risk row.
**Warnings:** Documentation only. The plan still does not authorize source artifact reuse, ACS prompt/config behavior changes, or Stage 1 redesign.
**Learnings:** Plan formulas should reference actual existing config concepts (`researchTimeBudgetMs`) rather than generic placeholder names, otherwise developer handoff still leaves guesswork.
-> Docs/WIP/2026-04-27_SVP_ACS_Gated_Implementation_Plan.md
...
---
### 2026-04-27 | Lead Architect | Codex (GPT-5) | SVP ACS Slice 3 Live Metrics Review -- [Standard] [open-items: yes]
**For next agent:** Reviewed live post-instrumentation SVP run `73fb6650d4674540bb91354e3705423f` from draft `b041b493b1294bba8253ef807de88720` on commit `b43f6b53`. Metrics showed `28` prepared candidates, `5` selected, `23` dropped, `15` preliminary evidence items, and exact Stage 1 -> Stage 2 URL overlap `0/5`. Preliminary evidence was `0` selected, `11` dropped, `4` unmapped. Stage 2 exhausted budget after researching only `3/5` selected claims; `AC_04` and `AC_14` had `0` iterations/evidence, and contradiction research did not start (`time_budget_exhausted`). Slice 3 decision: stop source reuse for now; do not proceed to Slice 4 from this evidence. Next meaningful gate is Slice 5 budget-aware ACS review/design.
**Warnings:** This is one post-instrumentation live run, but it is the motivating same-input case. Automatic ACS mode can avoid manual UI action for future validation, but it does not solve budget mismatch by itself. Changing selection cap, research budget, or contradiction protected time affects analysis behavior and should be treated as UCM-tuned/product-reviewed behavior, not a hidden fix.
**Learnings:** The new metrics separated the suspected preliminary-search waste from the actual bottleneck. For the SVP workload, source reuse is not the lever; selected-claim budget fit is.
-> Docs/AGENTS/Handoffs/2026-04-27_Lead_Architect_SVP_ACS_Slice3_Live_Metrics_Review.md

---
### 2026-04-27 | LLM Expert | Codex (GPT-5) | ACS Budget-Aware Selection Gate Review -- [Standard] [open-items: yes]
**For next agent:** Reviewed the proposed Slice 5 budget-aware ACS gate after live SVP metrics showed `28` prepared claims, `5` selected, only `3/5` researched, and no contradiction pass. Verdict: `MODIFY before implementation`. ACS may recommend fewer than the configured cap, but only as an explicit LLM budget-fit recommendation over the full final Stage 1 candidate set; the cap remains an upper bound, not a target. Required contract additions: budget inputs in the prompt, per-run `budgetFitRationale`, explicit `deferredClaimIds`/per-claim deferred state with reason codes and short LLM rationale, and downstream/UI/API disclosure that deferred claims were not researched rather than failed, disproven, unimportant, or hidden.
**Warnings:** Do not implement a deterministic budget formula that filters claims by claim text, source yield, keywords, language, or heuristic importance. Structural budget arithmetic may compute exposed budget context, but semantic selection/defer decisions must stay inside the batched LLM recommendation and be persisted for audit. Defaults and thresholds must be UCM-backed in `pipeline.default.json` and `config-schemas.ts`; prompt changes require build/schema tests and multilingual validation using Captain-approved inputs.
**Learnings:** Current ACS already permits fewer than the cap through `recommendedClaimIds` size `0..cap`, but it lacks a budget-specific explanation and deferred-claim contract. The safe next design is to extend the recommendation schema, not lower `claimSelectionCap` globally or silently auto-select fewer claims without preserved deferred state.

---
### 2026-04-27 | Senior Developer | Codex (GPT-5) | ACS Budget-Aware Selection Feasibility Review -- [Standard] [open-items: yes]
**For next agent:** Feasibility review agrees with the Slice 5 direction but recommends an operational UCM-only mitigation first for validation runs: use `claimSelectionDefaultMode=automatic` for unattended runs and either lower `claimSelectionCap` or raise `researchTimeBudgetMs`/`contradictionProtectedTimeMs` in a clearly scoped config profile. Product behavior should then extend the existing ACS recommendation path rather than adding a new selector: `recommendedClaimIds` already accepts `0..cap`, draft state and job metadata are JSON-backed, and API/UI confirmation already permits one-to-cap selected IDs. Lowest-complexity implementation shape is optional `budgetFitRationale` plus explicit deferred claim state (`deferredClaimIds` and/or per-assessment budget treatment) persisted through `ClaimSelectionDraftState` and `ClaimSelectionMetadata`, disclosed in the select UI/admin JSON, and passed through `JobService`.
**Warnings:** Do not make the final runner silently drop selected claims; do not add deterministic budget formulas that rank/filter semantic claim content; do not change `claimSelectionCap` defaults globally as a hidden product fix. Any prompt/schema change must keep prepared snapshot provenance behavior in mind: UCM changes alter `pipelineConfigHash`, so existing prepared drafts should be retried rather than reused across budget-behavior changes.
**Learnings:** The current ACS code path is already structurally permissive enough for fewer-than-cap recommendations; the missing contract is auditability and disclosure, not selection enforcement.

---
### 2026-04-27 | Lead Architect | Codex (GPT-5) | Budget-Aware ACS Slice 5 Review Design -- [Significant] [open-items: yes]
...
---
### 2026-04-28 | Unassigned | Codex (GPT-5) | Daily Bug Scan Runner Bootstrap Leak Fix -- [Standard] [open-items: no]
**For next agent:** The daily scan found one concrete regression in the recent runner window: full `npm test` timed out in `test/unit/lib/drain-runner-pause.integration.test.ts` while the same test passed in isolation. The recent `internal-runner-queue.ts` changes were fine functionally, but import-time bootstrap timers were still starting during Vitest and leaking cross-suite async work. `apps/web/src/lib/internal-runner-queue.ts` now skips the module bootstrap when `NODE_ENV=test` or `VITEST=true`; queue/drain behavior is unchanged. Verification passed on the isolated pause test, `runner-concurrency-split.integration.test.ts`, and the full safe suite `npm test`.
**Warnings:** This is a test-environment containment fix, not a runtime behavior change. The runner module still logs expected mocked-network errors in some integration tests, but they no longer block suite completion.
**Learnings:** The outstanding full-suite timeout warning carried forward from the ACS runner work was caused by import-side timers, not by the pause/probe queue semantics.
-> Docs/AGENTS/Handoffs/2026-04-28_Unassigned_Daily_Bug_Scan_Runner_Bootstrap_Leak_Fix.md

---
### 2026-04-28 | Lead Developer + LLM Expert | Codex (GPT-5) | Prompt Split and LLM Runtime Efficiency Diagnosis -- [Standard] [open-items: yes]
**For next agent:** Recovered the prior prompt split plan from `Docs/ARCHIVE/2026-04-20_Prompt_Split_Plan.md` and companion token debate from `Docs/WIP/2026-04-20_Token_Reduction_Chunking_Debate_Consolidated.md`. The original decision, targeted reads instead of physical split, is still valid for agent ergonomics but the active `apps/web/prompts/claimboundary.prompt.md` has grown from about 212KB/53K estimated tokens to about 294KB/84K estimated tokens, near the deferred split trigger. Current runtime prompt state is not drifted: active DB hash `21bfbc004d1284bc5b9e43521f25dec981acb511e67c6703e616e46677d4048a` matches the canonical file hash. Recommendation is to reopen PROMPT-SPLIT as a design/telemetry track first: define manifest/composite hash semantics, UCM/admin behavior, section-level provenance, Stage 1 subset invalidation, and per-call telemetry for static/dynamic prompt size, cache boundaries, and retry causes before physically splitting files.
**Warnings:** Physical file splitting alone will not reduce production LLM tokens, latency, or retries because current calls commonly interpolate dynamic payloads into the cache-controlled system message. Runtime efficiency requires static/dynamic prompt separation and retry-cause prevention on high-volume sections such as `CLAIM_EXTRACTION_PASS2`, `EXTRACT_EVIDENCE`, `CLAIM_CONTRACT_VALIDATION`, and verdict prompts. No prompt text, code, live jobs, or expensive tests were changed/run during this diagnosis.
**Learnings:** Agent context efficiency and production LLM context efficiency are related but separate levers. Targeted prompt reads help coding-agent quality and cost; production savings come from preserving stable cacheable system instructions, moving dynamic payloads out of cache-controlled content where safe, recording section-level token/cache telemetry, and preventing schema/contract retries rather than relying on retries as normal control flow.

---
### 2026-04-28 | Lead Developer + LLM Expert | Codex (GPT-5) | Prompt Split Runtime Efficiency Execution Plan -- [Standard] [open-items: yes]
**For next agent:** Wrote `Docs/WIP/2026-04-28_Prompt_Split_Runtime_Efficiency_Execution_Plan.md`. The plan converts the diagnosis into an execution sequence: Phase 1 prompt-runtime telemetry with no behavior change, Phase 2 manifest/composite-hash design, Phase 3 static/dynamic prompt separation, Phase 4 physical split, and Phase 5 retry prevention. It explicitly reuses existing telemetry: `LLMCallMetric` / `AnalysisMetrics`, `metrics-integration.ts`, `MetricsController`, final-job `analysisObservability` pattern from ACS research-waste metrics, and draft preparation observability where needed.
**Warnings:** The first executable slice is instrumentation only. Do not bundle prompt wording edits, physical file splitting, retry-policy changes, model changes, or live validation into Slice 1. Prompt behavior changes still need explicit human approval; live jobs require commit plus runtime/config refresh first.
**Learnings:** Yesterday's telemetry work gives the right model for this track: diagnostic-only, sanitized, persisted, and non-behavioral. The likely first implementation should extend existing `LLMCallMetric` with optional prompt profile/section/hash/size/cache/retry-cause fields rather than adding a separate metrics store.

---
### 2026-04-28 | Lead Developer | Codex (GPT-5) | Archive Status Review: Prompt Split and Text-First Session Prep -- [Standard] [open-items: yes]
**For next agent:** Reviewed whether `Docs/ARCHIVE/2026-04-20_Prompt_Split_Plan.md` and `Docs/ARCHIVE/2026-04-23_Session_Preparation_Text_First_Follow_On_Proposal.md` should be unarchived. Decision: do not plain-revert either archive move. The prompt split plan is historical for Option E, but its deferred Option A/manifest thread has been reopened through `Docs/WIP/2026-04-28_Prompt_Split_Runtime_Efficiency_Execution_Plan.md`; metadata was corrected in `Docs/STATUS/Backlog.md`, `Docs/WIP/README.md`, and `Docs/ARCHIVE/README_ARCHIVE.md`. The text-first session prep proposal should remain archived because active docs and Captain direction reject it as a semantics-changing redesign; active WIP links were corrected to point to the archive path.
**Warnings:** The old prompt split document still contains obsolete size/count facts (`212KB`, `28 sections`) and should not be used as the current execution plan. The text-first proposal still contains useful problem framing, but its recommended default path is rejected for current ACS work; use the semantics-preserving async proposal and `MON-STG1-LAT` instead.
**Learnings:** For mixed WIP docs, unarchiving the exact old file is often worse than keeping a clean active successor and correcting pointers. Archive status can be accurate as historical while backlog/WIP status tracks the reopened successor work.

---
### 2026-04-28 | Lead Developer + LLM Expert | Codex (GPT-5) | Prompt Runtime Telemetry Slice 1 Readiness Map -- [Standard] [open-items: yes]
**For next agent:** Per Captain approval, prepared Slice 1 in read-only mode while a local verification job was still running. Do not edit watched runtime files or restart services until that job completes or an isolated worktree is used. Recommended first patch: extend `LLMCallMetric` in `apps/web/src/lib/analyzer/metrics.ts` with optional prompt profile/section/content hash/section hash/rendered-system-size/dynamic-payload-size/retry-cause/retry-branch/output-branch fields; reuse existing `hashContent` and `estimateTokens` helpers from `prompt-loader.ts`; add cache read/write token extraction from AI SDK `usage.inputTokenDetails.cacheReadTokens/cacheWriteTokens`; instrument `research-extraction-stage.ts` `EXTRACT_EVIDENCE` first, then Stage 1 `runPass2` or `validateClaimContract` as the retry branch sample.
**Warnings:** `recordLLMCall` is currently manual across many call sites, so do not attempt a broad all-call-site refactor in Slice 1. Start with one Stage 2 call family and one Stage 1 retry/contract family. `loadAndRenderSection` currently returns rendered content plus composite hash only; section hash requires either extending the return shape to include raw section metadata or adding a helper that loads the parsed prompt and hashes the unrendered section content.
**Learnings:** AI SDK v6 usage exposes cache fields as `usage.inputTokenDetails.cacheReadTokens` and `usage.inputTokenDetails.cacheWriteTokens`; the existing metric names `cacheReadInputTokens` and `cacheCreationInputTokens` can map directly to those fields without changing persisted summary semantics.

---
### 2026-04-28 | Lead Developer + LLM Expert | Codex (GPT-5) | Prompt Runtime Split Execution -- [Significant] [open-items: yes]
**For next agent:** Executed the prompt-runtime efficiency plan through Phase 4 with no prompt wording changes. Commits: `58b1235c`, `0bf5ae49`, `d9635da9`, `6209cdd3`, `09cfbe27`, `55ef87bf`. The active `claimboundary` source is now `apps/web/prompts/claimboundary/manifest.json` plus dedicated section files, but UCM/admin/job provenance still use one composite `prompt|claimboundary` blob. Reseed reported `0 changed`, and the active UCM hash matches the canonical manifest composite hash `21bfbc004d1284bc5b9e43521f25dec981acb511e67c6703e616e46677d4048a`.
**Warnings:** Live jobs used before Captain reset: 2/8. Captain then reset the next-track live validation budget to 8 available jobs. Both hydrogen jobs returned the expected `FALSE` result (`77` then `80` confidence); only the second run was post-refresh and confirmed prompt-runtime/cache telemetry. Runtime remains `+dirty` because unrelated NPO docs and another agent's runner bootstrap patch remain in the worktree. Do not implement Phase 5 prompt wording or retry-policy changes without explicit approval and telemetry evidence of repeated structural waste.
**Learnings:** Physical prompt splitting improves maintainability, but production savings came from static/dynamic message separation. Service refresh is required before trusting newly added metrics in live jobs. UCM hashes canonical prompt markdown, not raw file bytes.
-> Docs/AGENTS/Handoffs/2026-04-28_Lead_Developer_LLM_Expert_Prompt_Runtime_Split_Execution.md

---
### 2026-04-28 | Lead Developer + LLM Expert | Codex (GPT-5) | Prompt Hygiene Follow-Up Sequencing -- [Standard] [open-items: yes]
**For next agent:** The prompt-structure/redundancy critique is primarily in `Docs/WIP/2026-04-19_Consolidated_Prompt_Audit_Implementation_Plan.md` and `Docs/AGENTS/Handoffs/2026-04-19_Unassigned_Consolidated_Prompt_Audit_Adjudication.md`, not the April 20 split memo itself. `Docs/WIP/2026-04-28_Prompt_Split_Runtime_Efficiency_Execution_Plan.md` now includes Phase 6 for prompt hygiene after the no-wording prompt-runtime baseline. Recommended order: read-only `/prompt-audit`; inverse verification hardening; source-reliability prompt/runtime contract; `claimboundary` ecosystem-rule governance; then telemetry-backed retry wording only if needed.
**Warnings:** Prompt wording edits are analysis behavior changes. Do not mix them into manifest/source-layout/cache-boundary commits, and do not spend live jobs on static audit. Captain reset the next-track live validation budget to 8 jobs; use it only after a concrete prompt change is committed, reseeded, and services are refreshed.
**Learnings:** The split layout changes the risk profile: section-level files make redundancy cleanup safer, but the right safety boundary is still behavioral. Static audit finds candidate fixes; runtime telemetry decides whether a retry/cost concern is worth changing.

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | Runner Concurrency Safety Review -- [Standard] [open-items: yes]
**For next agent:** Initial review found the previous `.env.local` values made the effective report-job lane `3` because `FH_RUNNER_JOB_MAX_CONCURRENCY` overrides legacy `FH_RUNNER_MAX_CONCURRENCY`; prep remained a separate default lane of `1` unless `FH_RUNNER_PREP_MAX_CONCURRENCY` was set. Recent DB/events review found no post-2026-04-21 `Re-queued after application restart (previous execution lost)` events and no suspicious duplicate-start pattern under the April 24 API-side SQLite runner-claim fix. Recent repeated starts after April 24 were paired with explicit `Job interrupted by server restart.` / `previous execution interrupted`, so they are real local restart interruptions, not the old stale-snapshot replay bug. The 2026-04-28 3-wide batch had zero warn/error events across 7 completed jobs plus one in-progress job at review time, which indicates `job=3` is technically viable, but local restarts/config changes can now interrupt up to 3 jobs at once.
**Warnings:** Google-CSE 429s were observed on April 24 and April 26, but the sampled windows were effectively serial and all affected jobs succeeded, so this is not strong evidence that `job=3` caused provider pressure. Still, higher job concurrency raises per-minute search/LLM burst risk. Final recommendation after draft-preparation review is to use the same Alpha setting locally and deployed: `FH_RUNNER_MAX_CONCURRENCY=2`, `FH_RUNNER_JOB_MAX_CONCURRENCY=2`, and `FH_RUNNER_PREP_MAX_CONCURRENCY=1`. This allows limited report parallelism while keeping session preparation on the currently proven serial lane.
**Learnings:** Keep `FH_RUNNER_MAX_CONCURRENCY` and `FH_RUNNER_JOB_MAX_CONCURRENCY` aligned when the explicit split var is present; otherwise operators may think the system is serial while it is actually running multiple report jobs.

---
### 2026-04-28 | Lead Developer + LLM Expert | Codex (GPT-5) | Bolsonaro EN Validation Regression Triage -- [Standard] [open-items: yes]
**For next agent:** Captain flagged job `790573c784214a97b7fa75365960f13a` as not matching documented Bolsonaro expectations. Confirmed. Relevant docs are `Docs/ARCHIVE/2026-04-11_Phase2_Per_Input_Expectations.md` R4, `Docs/WIP/2026-04-16_Benchmark_Report_Quality_Status_Matrix.md`, and the April 20 Bolsonaro handoffs. Current EN job has only `2` ACs, both claim verdicts `UNVERIFIED` (`32/40`, `22/42`), aggregate `MOSTLY-FALSE | 27 | 40`, and `distinctEvents` again includes the 4 Aug 2021 fake-news/electoral-system inquiry that the April 20 hardening explicitly wanted gone. PT companion job `93eb78a0e53b4e3f93e2279f88ad8880` succeeded with `3` ACs and `LEANING-TRUE | 65 | 60`, using Brazilian legal-source boundaries.
**Warnings:** This is not a stale prompt hash: both jobs used prompt hash `21bfbc004d1284bc5b9e43521f25dec981acb511e67c6703e616e46677d4048a`. The likely first fix is not broad prompt hygiene; start with exact-job report review and `/debt-guard`, then target Stage 1 English decomposition / same-proceeding-path anchoring and Stage 4/report consistency. Prompt wording changes still need Captain approval. The temporary validation-batch UCM setting was restored from automatic hash `7e7f69c...` to interactive hash `4145aacc...` after all eight jobs completed.
**Learnings:** The hidden failure is visible only in diagnostics: `gate1Stats` says both EN claims failed specificity but still shipped, per-claim verdict labels are `UNVERIFIED` while the aggregate presents `MOSTLY-FALSE`, and no user-visible warning was emitted. The PT run shows the system can produce the intended 3-claim legal/procedural/sentence decomposition when the target proceeding is anchored tightly.

---
### 2026-04-28 | Lead Developer + LLM Expert | Codex (GPT-5) | Bolsonaro PT Report Prominence Follow-Up -- [Standard] [open-items: yes]
**For next agent:** Captain judged Portuguese job `93eb78a0e53b4e3f93e2279f88ad8880` directionally as expected, but noted one non-blocking report-quality imperfection: the trial outcome and sentence are not prominent enough. The job evidence contains the facts, including `EV_1777381100464` and `EV_1777381213901` stating a conviction/sentence of `27 anos e 3 meses de prisão` in AP 2.668/DF, and AC_03 reasoning discusses proportionality. However, the report headline/key finding does not prominently surface the conviction, sentence length, case/proceeding identifier, date, or appeal posture.
**Warnings:** Do not address this together with the English Bolsonaro regression unless the implementation is already touching report synthesis. This is a report-generation prominence issue, not an analysis or evidence-acquisition failure.
**Learnings:** For legal proceeding reports, target-proceeding outcome facts should be promoted when available: proceeding/case identifier, verdict/conviction result, sentence length, date, and appeal posture. Keep the rule generic and evidence-driven, not Bolsonaro-specific.

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | Draft Preparation Concurrency Safety Review -- [Standard] [open-items: yes]
**For next agent:** Reviewed whether `FH_RUNNER_PREP_MAX_CONCURRENCY > 1` is theoretically safe. The draft-preparation lane has only process-local guards in `globalThis.__fhDraftQueueState`; unlike report jobs, it has no API-side atomic claim/lease before Stage 1 work. The runner reads a draft as `QUEUED`, then PUTs `PREPARING` unconditionally, so two web process contexts could theoretically prepare the same draft. API safeguards make duplicate final jobs unlikely: `Jobs.ClaimSelectionDraftId` is unique and `CreateJobFromDraftAsync` recovers the concurrent winner after a unique-constraint violation.
**Warnings:** Local draft events since 2026-04-23 show no true overlapping prep intervals, no duplicate `prepare` success events, no duplicate final jobs, and one restart-recovery reset on 2026-04-28 (`PREPARING` -> `QUEUED`, "previous draft preparation lost"). `prep=2` is acceptable only as a low-traffic Alpha canary if Captain accepts wasted-work/race risk; it is not theoretically as robust as `FH_RUNNER_JOB_MAX_CONCURRENCY=2`. Conservative operational setting remains `FH_RUNNER_PREP_MAX_CONCURRENCY=1` until an API-side prep claim is added.
**Learnings:** To make `FH_RUNNER_PREP_MAX_CONCURRENCY > 1` structurally safe, add a serializable API `claim-prep`/lease endpoint for drafts analogous to job `claim-runner`, then have the runner start Stage 1 only after the claim succeeds.

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | Automatic ACS Non-Interactive Submission Guard -- [Standard] [open-items: yes]
**For next agent:** Fixed the non-interactive all-AtomicClaims bypass in commit `7b5b3409`. The guarded web `/api/fh/analyze` route now rejects automatic/default-automatic direct submissions before the input-policy LLM and fails closed on pipeline-config loader fallbacks. Non-interactive scripts now submit via ACS automatic drafts using `apps/web/scripts/automatic-claim-selection.js`; the official validation runner and matrix runner no longer post directly to `/v1/analyze` or the guarded proxy. Verification passed: focused Vitest route/draft/runner tests (40 tests), Node syntax checks for modified scripts, and `npm -w apps/web run build`. Independent reviewer cleared the final diff with no blockers.
**Warnings:** Low-level API `/v1/analyze` still exists as a manual bypass if external callers use it directly. Runtime UCM was set to `claimSelectionDefaultMode=automatic`, cap `5`, after restart. Live submissions created final jobs and verified selected counts: SVP PDF job `9f338d563781412f80e7a2edf43f1dd6` selected 5, Bolsonaro EN job `7ece4b51add7470685ceb56387c715da` selected 2, RT URL job `18b07fa8efa345178f5eb59406ea2805` selected 5. Last monitor had SVP queued and the other two running at 58%; no early failures observed.
**Learnings:** The cheapest robust fix is to reject legacy automatic direct submissions before LLM work and route batch tooling through the existing ACS draft flow. Config-loader fail-closed checks must account for sentinel errors and active-hash default fallbacks, not just thrown exceptions.

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | Automatic ACS UNVERIFIED Follow-Up -- [Standard] [open-items: yes]
**For next agent:** Captain reported all jobs from `7b5b3409` were `UNVERIFIED`. Exact-job inspection plus independent reviewer found no selected-vs-prepared corruption: each final job result used exactly the selected draft claims. The regression is path parity/rollout: `7b5b3409` correctly moved non-interactive submissions from direct all-claims jobs to ACS automatic, but active UCM had ACS budget awareness off, so automatic auto-confirmed up to 5 claims without the existing budget-fit guard. The structural routing fix is classified `keep`; the rollout/path is classified `amend`.
**Warnings:** Active UCM was changed, not source: pipeline/default is now `claimSelectionDefaultMode=automatic`, `claimSelectionCap=5`, `claimSelectionBudgetAwarenessEnabled=true`, `claimSelectionBudgetFitMode=allow_fewer_recommendations`, `claimSelectionMinRecommendedClaims=1`, active hash `29dbe0043a0e822d24e5537d405119ccad6ba1c2e9eead5dc11fb6c910c2ce8e`. `/api/fh/analyze` omitted-mode check returns 409 again. No new live validation jobs were submitted after this config amendment.
**Learnings:** ACS automatic parity depends on both the routing path and the active recommendation profile. Moving validation tooling to ACS is necessary to avoid all-claims evaluation, but automatic auto-confirm should not be evaluated with budget-fit behavior disabled when the target is publication-quality non-interactive reports.

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | ACS Validation Path Documentation -- [Standard] [open-items: yes]
**For next agent:** The reviewed solution is now documented in `Docs/WIP/2026-04-28_ACS_Validation_Path_Deployment_Solution.md`: keep the product ACS guard/default path, add an explicit two-flag `direct-full-baseline` validation-only comparator, and emit submission-path plus selected/prepared claim-count metadata. Next implementation should touch validation tooling only first, then run safe tests/build before any live jobs.
**Warnings:** Documentation only; no script/code amendment has been implemented yet. The direct-full path must remain opt-in forensic tooling, not the default validation or product runtime path.
**Learnings:** ACS automatic parity and historical direct-full comparability are separate questions; validation outputs must label the path and claim counts before `UNVERIFIED` outcomes can be interpreted as prompt/analyzer regressions.
-> Docs/AGENTS/Handoffs/2026-04-28_Senior_Developer_ACS_Validation_Path_Documentation.md

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | ACS Validation Simplified Review Plan -- [Standard] [open-items: yes]
**For next agent:** `Docs/WIP/2026-04-28_ACS_Validation_Path_Deployment_Solution.md` now supersedes the earlier direct-full rerun proposal. The review-ready plan has one live non-interactive path: ACS automatic draft via the shared helper; validation metadata is passive; historical direct jobs are read-only comparators. Implement script/helper/summary extraction only first.
**Warnings:** Do not implement `direct-full-baseline` in the next slice. It is deferred unless historical job evidence is missing or too stale for a future explicit review question.
**Learnings:** The cheapest and lowest-risk comparator is an existing job. Add execution paths only after read-only evidence and passive metadata are insufficient.
-> Docs/AGENTS/Handoffs/2026-04-28_Senior_Developer_ACS_Validation_Simplified_Review_Plan.md

---
### 2026-04-28 | Senior Developer + Lead Architect + LLM Expert | Codex (GPT-5) | ACS Validation Full Debate Spec -- [Standard] [open-items: yes]
**For next agent:** Full debate reconciled to `MODIFY`: keep one supported live ACS validation path, but strengthen it with a Submission Policy Thin Waist. `Docs/WIP/2026-04-28_ACS_Validation_Path_Deployment_Solution.md` now defines current vs target state, direct-caller inventory/quarantine, passive schema v2 metadata, validation gates by readiness, read-only historical references, and slices 1-7.
**Warnings:** `/v1/analyze` remains a live direct API surface and is only a follow-up governance gap in this spec; supported validation automation must not rely on it. No prompt/config behavior changes, no analyzer changes, and no live jobs are part of the next slice.
**Learnings:** One-path validation must be enforced as a supported-automation policy, not assumed from the web guard. Cheap diagnostics require admin-aware ACS metadata, otherwise `UNVERIFIED` still triggers expensive manual investigations.
-> Docs/AGENTS/Handoffs/2026-04-28_Senior_Developer_ACS_Validation_Full_Debate_Spec.md

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | ACS Validation Reviewer Disposition Applied -- [Standard] [open-items: yes]
**For next agent:** Reviewer feedback has been folded into `Docs/WIP/2026-04-28_ACS_Validation_Path_Deployment_Solution.md`. The spec now includes the quarantined `apps/web/test/scripts/regression-test.js`, explicitly says `submissionPath` is a new Slice 3 field, splits metadata into Tier 1 Slice 3 vs Tier 2 future telemetry, removes compatibility aliases, moves API provenance/governance out of the implementation stream, and adds acceptance criteria for admin metadata fetch, `AWAITING_CLAIM_SELECTION`, per-slice tests/build, and quarantine output.
**Warnings:** The old direct-full-baseline handoff is marked superseded by the full debate spec. Do not implement direct-full reruns, compatibility aliases, or API governance in the next slice. `regression-test.js` currently uses the ACS helper, but remains quarantined because its inputs and result parsing are stale.
**Learnings:** The lowest-risk implementation boundary is now crisp: first make ACS validation auditable through a shared client and Tier 1 passive metadata, then migrate scripts. API-level `/v1/analyze` policy belongs in a separate follow-up after the validation path is stable.
-> Docs/AGENTS/Handoffs/2026-04-28_Senior_Developer_ACS_Validation_Full_Debate_Spec.md

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | ACS Validation Final Spec Consolidation -- [Standard] [open-items: yes]
**For next agent:** `Docs/WIP/2026-04-28_ACS_Validation_Path_Deployment_Solution.md` is now the consolidated implementation-facing specification. It contains the final decision, scope boundaries, three target diagrams, shared ACS client contract, schema v2 Tier 1/Tier 2 metadata, structural gates, historical comparison policy, five implementation slices, API-governance follow-up, verification requirements, non-goals, and review checklist.
**Warnings:** This remains documentation-only. Do not start live jobs from this state. The first code slice is the shared ACS validation client plus direct-caller quarantine labels; API governance, prompt changes, analyzer changes, and direct-full reruns are explicitly out of scope.
**Learnings:** The clearest low-overhead plan is five slices plus one follow-up: build the supported validation thin waist first, then Tier 1 passive metadata, then migrate scripts, then historical mapping, then an ACS-only canary after commit/restart/UCM capture.
-> Docs/AGENTS/Handoffs/2026-04-28_Senior_Developer_ACS_Validation_Full_Debate_Spec.md

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | ACS Budget-Aware Selection Exchange Integrated -- [Standard] [open-items: yes]
**For next agent:** The external exchange `.codex/context-extension/2026-04-28_acs-budget-aware-selection_exchange.md` has been folded into `Docs/WIP/2026-04-28_ACS_Validation_Path_Deployment_Solution.md`. The spec now treats Stage 2 commits `c1c6e62a`, `d59d18f3`, and `22b1caa2` as baseline behavior, adds future budget-aware ACS telemetry labels, and records budget-aware selector behavior as a separate follow-up after validation metadata and canary evidence.
**Warnings:** Do not implement ACS budget-aware selector changes in the current validation-tooling stream. If selector behavior changes later, it crosses prompt/config behavior and needs review/debate first. Use only Captain-approved live inputs unless Captain explicitly approves additional stress cases.
**Learnings:** The validation client should make selected-set budget stress observable without becoming a second Stage 2 controller. Future ACS budget behavior should optimize the selected set with LLM-produced structured labels and UCM-configured guardrails, not deterministic topic rules.
-> Docs/AGENTS/Handoffs/2026-04-28_Senior_Developer_ACS_Validation_Full_Debate_Spec.md

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | ACS Validation Review Findings Addressed -- [Standard] [open-items: yes]
**For next agent:** Addressed three spec review findings in `Docs/WIP/2026-04-28_ACS_Validation_Path_Deployment_Solution.md`: the canary selected-claim distribution condition is now diagnostic output rather than a hard tooling acceptance gate; Stage 2 telemetry is anchored to `resultJson.analysisObservability.acsResearchWaste.selectedClaimResearch` and `.contradictionReachability`; and explicit budget-deferred claims are separated from ordinary `notSelectedClaimIds` / `notRecommendedClaimIds`.
**Warnings:** A canary can expose zero-iteration selected claims while contradiction still ran. That may be a pipeline-quality or release issue, but it should not block landing validation tooling if the tooling surfaces and classifies it correctly.
**Learnings:** Validation tooling should make pipeline behavior reviewable, not convert every pipeline-quality finding into a tooling rollout blocker.
-> Docs/AGENTS/Handoffs/2026-04-28_Senior_Developer_ACS_Validation_Full_Debate_Spec.md

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | ACS Validation Slice 1 Implementation -- [Standard] [open-items: yes]
**For next agent:** Slice 1 is implemented. `apps/web/scripts/automatic-claim-selection.js` now exposes a structured shared ACS validation client beside the legacy helper: `submitAutomaticValidationJob`, `waitForAutomaticDraftOutcome`, `fetchJobDetail`, `waitForJobOutcome`, `submissionPath: "acs-automatic-draft"`, structured `AWAITING_CLAIM_SELECTION`, admin-aware final job fetch, and visible `metadataUnavailable` states. The direct caller scripts from the spec are labeled as quarantined validation tooling in place.
**Warnings:** Supported validation scripts are not migrated yet; Slice 2 should add Tier 1 schema v2 extraction, then Slice 3 should migrate supported scripts. Quarantine is labeling/documentation only, not runtime enforcement. No live jobs were submitted.
**Learnings:** The additive helper surface keeps existing automation stable while giving the migration a single thin waist. Final-job failure must propagate as `ok: false`; metadata fetch disabled or unavailable must be visible, not silently treated as comparable.
-> Docs/AGENTS/Handoffs/2026-04-28_Senior_Developer_ACS_Validation_Slice1_Implementation.md

---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | ACS Validation Slices 2-3 Implementation -- [Standard] [open-items: yes]
**For next agent:** Schema v2 passive metadata extraction and supported validation-script migration are implemented. The shared ACS client now emits `submissionPath`, prepared/ranked/recommended/not-recommended/not-selected/deferred/selected claim counts and IDs, current verdict/evidence/source/warning fields, and read-if-present Stage 2 telemetry from `analysisObservability.acsResearchWaste`. `extract-validation-summary.js`, `run-validation-matrix.js`, and `baseline-runner.js` now use the shared client; `scripts/validation/captain-approved-families.json` contains the exact current Captain-approved inputs.
...
---
### 2026-04-28 | Senior Developer | Codex (GPT-5) | UCM Switched Back To Interactive Claim Selection -- [Standard] [open-items: yes]
...
---
### 2026-04-29 | Senior Developer | Codex (GPT-5) | Unification Review Follow-Up -- [Standard] [open-items: yes]
**For next agent:** Four reviews were consolidated and low-risk comments addressed in commit `1ed5744f`: explicit direct `submissionPath`, admitted-vs-final evidence count comments, restored Prompt Architecture section map, and mixed git/prompt hash headers in `compare-batches.js`.
**Warnings:** No behavior/prompt/UCM/selector/stage scheduling changes. C# `SubmissionPath` contract tests remain deferred because `apps/api` has no current test project; add that before making provenance release-gating.
-> Docs/AGENTS/Handoffs/2026-04-29_Senior_Developer_Unification_Review_Followup.md

---
### 2026-04-29 | Senior Developer | Codex (GPT-5) | Daily Bug Scan Connectivity Probe Fix -- [Standard] [open-items: no]
**For next agent:** Daily bug scan reviewed commits since `2026-04-28T06:00:53.306Z` and found a concrete timeout hole in [`/C:/DEV/FactHarbor/apps/web/src/lib/connectivity-probe.ts`](/C:/DEV/FactHarbor/apps/web/src/lib/connectivity-probe.ts). The helper aborted its controller after `timeoutMs` but still depended on `fetchImpl` honoring `AbortSignal`; a mocked or wrapped fetch that ignored the signal could leave the runner pause auto-resume probe hanging indefinitely. The fix races the fetch with an explicit timeout rejection and `unref()`s the timer when available. Regression coverage was added in [`/C:/DEV/FactHarbor/apps/web/test/unit/lib/connectivity-probe.test.ts`](/C:/DEV/FactHarbor/apps/web/test/unit/lib/connectivity-probe.test.ts) for a never-resolving fetch that ignores abort.
**Warnings:** The original full-suite evidence was a runner-pause timeout, but the deterministic reproduction is at the shared probe-helper level rather than a direct single-test flake reproducer. No prompt, UCM, API schema, or live-job behavior outside probe timeout semantics was changed.
**Learnings:** Shared infra helpers need self-contained timeout completion, not just `AbortController`, because tests and wrappers often stub `fetch` without signal support.

---
### 2026-04-29 | Unassigned | Codex (GPT-5) | Daily Bug Scan No New Regression -- [Standard] [open-items: no]
**For next agent:** Reviewed commits since `2026-04-29T05:39:00.760Z`. The only post-run code commit was `159834ce5a58789a3852e1b794624ff6d15384dc` (`fix: harden connectivity probe timeout`) touching [`/C:/DEV/FactHarbor/apps/web/src/lib/connectivity-probe.ts`](/C:/DEV/FactHarbor/apps/web/src/lib/connectivity-probe.ts) and [`/C:/DEV/FactHarbor/apps/web/test/unit/lib/connectivity-probe.test.ts`](/C:/DEV/FactHarbor/apps/web/test/unit/lib/connectivity-probe.test.ts). Diff review found no new concrete bug signal, and `npm -w apps/web exec vitest run test/unit/lib/connectivity-probe.test.ts` passed (`7` tests).
**Warnings:** This scan found no additional repo-backed regression after the earlier connectivity timeout fix. No CI failure, failing local verifier, or contradictory diff evidence surfaced in the post-run window.
**Learnings:** When the post-run window contains only a targeted fix commit plus passing touched tests, the correct daily-scan outcome is a clean baseline rather than a speculative second bug hunt.

---
### 2026-04-29 | Senior Developer | Codex (GPT-5) | Admin Waiting-Input Selection Access -- [Standard] [open-items: no]
**For next agent:** Admins now have a direct waiting-input queue at `/admin/preparation`. The page defaults to unlinked `AWAITING_CLAIM_SELECTION` draft sessions, labels the view as `Jobs Waiting For Input`, and exposes `Open selection` actions that reuse the existing `/analyze/select/[draftId]` flow with the admin key from session storage. The admin detail page also has the same explicit action, while the live selection page displays an admin-access notice. Commit: `47e9b005`.
**Warnings:** This is UI/access-surface work only. It does not add a second selector, new confirm endpoint, prompt/config behavior, or pipeline scheduling change. The underlying API already authorized admin `GET`, `selection-state`, and `confirm`; the change makes that capability discoverable and operational.
**Verification:** `npm -w apps/web exec vitest run test/unit/app/admin/preparation/page-helpers.test.ts`; `npm -w apps/web run build`; local admin API check returned two unlinked `AWAITING_CLAIM_SELECTION` rows. Live selected Grander job `03aafc89d380495894662304be44cd9a` completed `SUCCEEDED` with `SubmissionPath=acs-interactive-draft`, selected IDs `AC_02,AC_05,AC_03`, and no zero-targeted selected-claim coverage.
**DEBT-GUARD RESULT:** Classification: missing UI affordance over existing backend capability. Chosen option: amend/add at admin UI edge. Rejected path: adding a new admin confirm API because the existing draft access headers already carry admin authority. Net mechanism count: unchanged for analysis/pipeline; one small UI helper added. Debt accepted: none.

---
### 2026-04-29 | Senior Developer | Codex (GPT-5) | Daily Bug Scan No Confirmed Regression -- [Standard] [open-items: no]
**For next agent:** Reviewed post-`2026-04-29T06:01:31.256Z` commits `47e9b00511b70fcef4c8b1f5d54a8c430bd0b1fc`, `b8ead3111181165b7448da5d4e4970b6804de708`, and `63e95019e63a642c0bea9a7a9ce484511c4d3435`. The touched claim-selection/admin-preparation surfaces stayed consistent with the current proxy/API contracts, targeted Vitest coverage passed, and `git diff --check 47e9b005^..HEAD -- . ':(exclude)Docs/**'` was clean. No concrete repo-backed bug was strong enough to justify a code change.
**Warnings:** Residual risk remains in lightly covered integration seams only: admin waiting-input navigation still lacks browser/integration coverage, and the new budget-display helpers are only exercised at unit level. No failing test, CI signal, or code-path contradiction surfaced in this scan.
**Verification:** `npm -w apps/web exec vitest run test/unit/app/admin/preparation/page-helpers.test.ts test/unit/lib/claim-selection-budget-display.test.ts test/unit/lib/analyzer/claim-selection-recommendation.test.ts`; diff review of the three commits plus current files `apps/web/src/app/admin/preparation/page.tsx`, `apps/web/src/app/analyze/select/[draftId]/page.tsx`, `apps/web/src/app/api/fh/claim-selection-drafts/route.ts`, `apps/web/src/lib/claim-selection-draft-proxy.ts`, and `apps/web/src/lib/analyzer/claim-selection-recommendation.ts`.

---
### 2026-04-29 | Unassigned | Codex (GPT-5) | Daily Bug Scan No Confirmed Regression -- [Standard] [open-items: no]
**For next agent:** Reviewed post-`2026-04-29T07:38:47.988Z` commit `36fe7c17a12cc8a7912ca32c702fad5b48c4109b` (`fix: clarify claim selection budget display`). The change consistently renamed claim-selection budget prompt variables from milliseconds to seconds, added a UI-only formatter for legacy `budgetFitRationale` text, and kept touched web tests green. Diff review, repo-wide reference search, and focused Vitest coverage produced no concrete regression signal strong enough to justify a fix.
**Warnings:** Residual risk is limited to presentation-only seams: `formatBudgetFitRationaleDisplay(...)` is unit-tested but not browser-covered, and any future prompt output that mixes non-duration `ms` tokens into `budgetFitRationale` would rely on this formatter's regex assumptions. No failing test, CI signal, or contradictory call site was found in this commit.
**Verification:** `npm -w apps/web exec vitest run test/unit/lib/claim-selection-budget-display.test.ts test/unit/lib/analyzer/claim-selection-recommendation.test.ts`; `rg -n "budgetResearchTimeBudget(Ms|Seconds)|budgetContradictionProtectedTime(Ms|Seconds)|budgetMainResearchTimeBudget(Ms|Seconds)|formatBudgetFitRationaleDisplay|formatBudgetDurationDisplay" apps/web`; `git diff --check 36fe7c17^ 36fe7c17 -- apps/web`; diff review of `apps/web/src/lib/analyzer/claim-selection-recommendation.ts`, `apps/web/src/lib/claim-selection-budget-display.ts`, `apps/web/src/app/analyze/select/[draftId]/page.tsx`, and the touched prompt/test files.

---
### 2026-04-29 | Senior Developer | Codex (GPT-5) | Claim Selection Cap Enforcement -- [Standard] [open-items: no]
**For next agent:** Implemented the requirement that Stage 2 must not analyze more than the claim-selection cap. Commit `8aa02c91` makes direct, non-prepared analysis fail closed before Stage 2 when Stage 1 yields more than `claimSelectionCap`, directing callers to the claim-selection draft endpoint. Commit `0514b4d7` preserves `PreparedStage1Json` and `ClaimSelectionJson` on retry jobs so retries of draft-backed runs keep the selected-claim constraint instead of becoming unlimited direct reruns.
**Warnings:** The direct path now rejects over-cap jobs rather than selecting claims itself; this deliberately avoids a second selector. Automatic selection still uses the system recommendation output from the draft preparation path. Historical direct jobs can still show more than 5 analyzed claims because they predate the guard.
**Verification:** `npm -w apps/web exec vitest run test/unit/lib/analyzer/claimboundary-prepared-stage1.test.ts`; `npm -w apps/web run build`; `dotnet test apps/api.Tests/FactHarbor.Api.Tests.csproj --filter JobServiceTests -c Release`; local automatic SVP job `82754f4611c84bedb4290bbe7b131a35` prepared 28 candidates, selected 5, and result `understanding.atomicClaims` contained exactly those 5; local manual SVP job `b20113fb8f8b4833b56c5844648c208b` prepared 26, selected 4, and result contained exactly those 4. Services restarted and API/Web health returned `200`.
**DEBT-GUARD RESULT:** Classification: incomplete existing mechanism. Chosen option: amend existing direct and retry boundaries. Rejected path: adding direct-path automatic selection because it would create a second selector. Net mechanism count: unchanged; no new prompt, UCM, or semantic selection logic. Debt accepted: none.

---
### 2026-04-29 | Unassigned | Codex (GPT-5) | Search API Nonprofit Outreach Templates -- [Standard] [open-items: no]
**For next agent:** Added ready-to-send Serper and SerpApi outreach templates at `Docs/Legal/Search_API_Nonprofit_Outreach_2026-04-29.md`, including FactHarbor UID `CHE-448.446.098`, attachment guidance, official signup/contact links, and fallback wording for manual public-interest/nonprofit pricing requests.
**Warnings:** The document is intentionally limited to Serper and SerpApi and reflects public pricing/contact information verified on `2026-04-29`; neither provider had a publicly documented nonprofit tariff at that time. Estimated usage/QPS fields remain placeholders for the user to fill before sending.
**Learnings:** For vendor-discount outreach, a reusable template plus a checklist link is more durable than scattering ad hoc message drafts across chat and WIP docs.

---
### 2026-04-29 | Unassigned | Codex (GPT-5) | FactHarbor Legal And Provider Documentation Sync -- [Standard] [open-items: no]
**For next agent:** Synced the core legal/NPO/provider docs to the current association status: new central reference `Docs/Legal/AI_And_Search_Provider_Nonprofit_Programs_2026-04-29.md`, refreshed `Docs/Legal/NPO_Formation_Checklist.md`, updated legal/governance/funding/academic xWiki pages with the Commercial Register entry letter and UID `CHE-448.446.098`, and replaced stale `UID expected` wording in the active Innosuisse/DIZH/cost-reduction references.
**Warnings:** Public SHAB / Zefix visibility was intentionally kept as pending/propagating where exact public visibility had not yet been re-verified. Historical documents such as the meeting Kurzreader and the older Innosuisse application draft were not rewritten as current plans; they were instead annotated or minimally corrected to avoid status drift.
**Learnings:** For org-status work, the most stable pattern is: one canonical checklist, one provider-programs reference, one outreach-template doc, then short status notes in the outward-facing xWiki and funding pages.

---
### 2026-04-29 | Unassigned | Codex (GPT-5) | ZKB Vereinskonto Application Status Update -- [Standard] [open-items: no]
**For next agent:** Recorded the sent ZKB club-account application in `Docs/Legal/NPO_Formation_Checklist.md`, added the supporting PDF `Docs/Legal/Eröffnung ZKB FactHarbor.pdf` to the document inventory, updated the milestone timeline, and noted the new bank-status baseline in `Docs/xwiki-pages/FactHarbor/Organisation/Legal and Compliance/Finance and Compliance/WebHome.xwiki`.
**Warnings:** The bank account is not yet marked as opened. The current documented fact is only that the application was sent and the PDF is on file.
**Learnings:** Banking status needs three distinct states in docs: application sent, account confirmed/opened, and first official bank document available for downstream verification workflows.

---
### 2026-04-29 | Senior Developer | Codex (GPT-5) | Selected Claim Research Floor Guard -- [Standard] [open-items: yes]
**For next agent:** Commit `d8961dbb` amends the Stage 2 protected contradiction-time guard so it cannot transition out of main research while a selected claim is still below `sufficiencyMinResearchedIterationsPerClaim`, has main query budget beyond the contradiction reserve, and is not zero-yield exhausted. This fixes the live symptom from job `82754f4611c84bedb4290bbe7b131a35`, where automatic ACS selected 5 claims but two selected claims had zero targeted main iterations. Review/debate result: pass, no merge blockers.
**Warnings:** The fix guarantees targeted-main scheduling coverage, not evidence yield. Live verification job `f4b78f99f7524e55a3d630a8fc69b5e4` had zero targeted selected-claim count fixed (`0`), no unselected claim IDs in the final Stage 2 ledger, and final `understanding.atomicClaims` exactly matched selected IDs. It still exposed a residual quality/speed issue: `AC_16` generated main queries but had `fetchAttemptCount=0` because the in-iteration protected-time guard stopped query spending; the claim remained insufficient. The full automatic path was slow: draft preparation ~8m37s, final Stage 2 ~12m37s, clustering/verdict/aggregation ~8m, total ~29m48s from draft creation to completion.
**Verification:** `npm -w apps/web exec vitest run test/unit/lib/analyzer/research-contradiction-admission.test.ts`; `npm -w apps/web run build`; services restarted via `scripts/restart-clean.ps1`; live automatic SVP draft `81fbf6f5effc4971ab9baff526453f36` created job `f4b78f99f7524e55a3d630a8fc69b5e4` on hash `d8961dbb534e91c28260635357f8aa5b76e57493+f6790722` and completed `SUCCEEDED`.
**DEBT-GUARD RESULT:** Classification: incomplete existing mechanism. Chosen option: amend existing protected-time admission guard. Rejected path: lowering ACS cap or changing prompts because inspected evidence showed a scheduler floor preemption bug, not selector over-cap behavior. Net mechanism count: unchanged; no new selector, prompt, UCM, or semantic rules. Residual debt: decide whether selected-claim floor should require at least one actual search/fetch attempt, and whether automatic ACS should be more conservative under the 480s main-research budget.

---
### 2026-04-29 | Senior Developer | Codex (GPT-5) | Bolsonaro Fair-Trial Regression Investigation -- [Standard] [open-items: yes]
**For next agent:** Investigated the current local regression where the extended Bolsonaro fair-trial input produced `LEANING-FALSE` / low confidence despite prior positive baselines (`eb02cd2e535a4556a2bc3c29868412a0`, `3f76f6eb069c4d329ca670bcd3c34506`). Commits `afceee1a` and `1076c20a` added generic prompt/config fixes for evidence qualifier preservation and non-controlling procedural positions, with focused prompt/config tests and build passing. Live canary `4a88594aae84485f8ba7961644ed4a1e` ran on `1076c20a+e707979d`, prompt hash `548a839b79e99310272c3a343625af89f0ec36cb9119583a8fce83e876a2070d`, selected 2 claims with zero starvation, but still ended `LEANING-FALSE` 41/33.
**Warnings:** Do not stack another prompt patch blindly. The remaining evidence points to an applicability/verdict-interface mismatch: Stage 4 used contextual adjacent-proceeding evidence directionally before the citation-integrity guard later dropped several non-direct AC_03 citations. The same run also marked useful positive procedural facts as contextual, so simply neutralizing all contextual evidence would be too blunt. Debt-guard hard stop is active because two live validation attempts did not restore the expected verdict shape.
**Verification:** `npm -w apps/web run test -- test/unit/lib/analyzer/research-extraction-stage.test.ts test/unit/lib/analyzer/verdict-prompt-contract.test.ts test/unit/lib/config-drift.test.ts`; `npm -w apps/web run test -- test/unit/lib/analyzer/verdict-prompt-contract.test.ts`; `npm -w apps/web run build`; `npm -w apps/web run reseed:force`; services restarted and API/Web health returned `200` after the failed canary.
**DEBT-GUARD RESULT:** Classification: introduced-regression plus incomplete existing mechanism. Chosen option so far: amend existing prompt/applicability semantics rather than revert the late-April direction-preservation guardrail cluster. Rejected path: broad revert of direction/applicability/citation guard commits because they also protect numeric comparison and citation-integrity fixes. Net mechanism count: unchanged. Residual debt: decide the next fix boundary explicitly, likely by aligning Stage 4 verdict inputs with the existing applicability/citation-integrity contract or by refining applicability labels for rule-governed procedural evidence.

---
### 2026-04-29 | Senior Developer | Codex (GPT-5) | Job Admin Annotations -- [Standard] [open-items: no]
**For next agent:** Added nullable `Jobs.AdminAnnotation`, admin-only API/proxy update path `PUT /v1/jobs/{jobId}/annotation` / `PUT /api/fh/jobs/{jobId}/annotation`, and admin-only UI editors/display in `/jobs` and `/jobs/[id]`. Non-admin responses keep `adminAnnotation` null, matching the existing admin-only field pattern.
**Warnings:** Existing DBs rely on API startup `EnsureJobsColumn(db, "AdminAnnotation", "TEXT")` or the included manual SQL migration `apps/api/migrations/008_add_job_admin_annotation.sql`. Annotation text is not written to job events; the audit event only records update/clear.
-> Docs/AGENTS/Handoffs/2026-04-29_Senior_Developer_Job_Admin_Annotations.md

---
### 2026-04-29 | Unassigned | Codex (GPT-5) | GitHub Private Repo Split Recommendation -- [Standard] [open-items: no]
**For next agent:** GitHub still exposes access at repository scope, so the recommended FactHarbor pattern is one main repo for code/public docs plus one private companion repo for legal, banking, fundraising, internal governance, and sensitive xWiki/admin material. Keep `apps/`, `packages/`, `tools/vscode-xwiki-preview`, and public product/architecture docs in the main repo; move `Docs/Legal/` and sensitive `Docs/xwiki-pages/FactHarbor/Organisation/**` sections into a private org repo with team-based access.
**Warnings:** `CODEOWNERS` and rulesets only restrict who can approve or merge changes; they do not hide paths from readers. If any contributor has repo read access, assume they can clone the full history. Avoid submodules unless the private content is operationally required by the build.
