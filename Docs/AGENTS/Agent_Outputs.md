...
**For next agent:** Audit verdict: **one shared pattern (upstream-accept criterion ⊥ downstream-reject criterion), but NOT one root/fix.** Hydrogen `184f0bba…` = **FLAKY Stage-1 contract abort, NOT a regression — live-confirmed.** Read-only narrowing exonerated all static suspects (Rule 2/3 + Rule 11 unchanged at/before last-good; self-consistency check `b5b6ce4e` 04-12 + `report_damaged` termination `f5cf31ee` 04-13 predate last-good; `0d97a7a6` only *added* recovery; comparison-side rules don't apply to dimension splits; `a1fa8530` satisfied by AC_01/02). **Live variance test (Captain-approved): 4 fresh runs at HEAD all passed `preservesContract:true` with in-band verdicts (`FALSE 8/79`, `FALSE 14/71`, `FALSE 9/75`, `MOSTLY-FALSE 15/65`) → 4 pass / 1 fail at the same source.** ⇒ **No revert, no bisect, no rule/prompt edit.** Gate-hardening fix designed + reviewed (`Docs/WIP/2026-05-30_Stage1_Contract_Gate_Hardening_Proposal.md`): **Option A (prune-and-revalidate) REJECTED** by 3 reviewers + advisor (doesn't fire on the real failure — both thesis-direct carriers AC_01/AC_02 flagged, AC_03 tangential; circular re-validation; can publish a half-the-input verdict). **Fix locus UNRESOLVED** — L1 extraction-shape variance (favored by 5/5 claim-count↔outcome), L2 validator-flip (→ resample), L3 `selfContradicted` code-check false-positive — **needs a discriminator** (more Hydrogen runs + fixed-set re-validation) before committing. The debate's "edit Rule 3" fix is misdirected. EN `4ba5e6e5…` = standing Stage-2⊥Stage-4 debt: `evaluateEvidenceSufficiency` (research-orchestrator.ts:1910) ignores `applicability`, Stage-4 (verdict-stage.ts:2092) requires `direct`; **verdict UNVERIFIED is correct** — **reject Gemini's re-research gate** (loop ran 5 full iterations; direct evidence doesn't exist). PT `a8b3…` healthy (7 direct citations). **KEEP `ac3b33da`.** Fix surfaces: §6.1 pin Hydrogen; §2 align Stage-2 sufficiency w/ Stage-4 publishability (deferred).
