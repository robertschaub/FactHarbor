# Agent Outputs Log
---
### 2026-03-31 | Senior Developer | Claude Code (Opus 4.6) | DistinctEvents Event Granularity — Prompt Fix
**Task:** Implement approved prompt-only fix for Stage 1 `distinctEvents` event granularity to improve multi-proceeding coverage.
**Files touched:** `apps/web/prompts/claimboundary.prompt.md` (2 sections), `Docs/AGENTS/Handoffs/2026-03-31_Senior_Developer_DistinctEvents_Event_Granularity_Fix.md`, `Docs/AGENTS/Agent_Outputs.md`
**What changed:** (1) Plurality override: replaced "one AtomicClaim per instance" with "keep claims broad, represent per-instance coverage in distinctEvents." Added "Multiple distinctEvents do not by themselves require multiple AtomicClaims." (2) DistinctEvents Include rules: replaced milestone-friendly line ("Temporal episodes of the same phenomenon") with peer-proceedings guidance + granularity rule. Lifecycle exception preserved for single-process chronology inputs. (3) Question exception updated to match.
**Old wording replaced (not appended):** "Decompose into one atomic claim per explicitly named or clearly implied distinct instance" and "Temporal episodes of the same phenomenon (e.g., '2022 trial' and '2024 appeal')."
**Verification:** 1494 tests pass. Build clean. Prompt reseeded.
**Pending:** 8-run validation batch (3 positive + 5 guardrail). Not yet executed — requires live LLM runs.
**For next agent:** Run the validation batch per the handoff. Check MT-5(C), claimCount stability, distinctEvents content. If ≤1/3 positive, escalate to deferred LLM repair step.
...
---
### 2026-03-31 | Lead Architect | Codex (GPT-5) | LLM Expert Consolidation on `distinctEvents` Fix
**Task:** Re-debate the adjusted `distinctEvents` proposal with an LLM Expert and consolidate the final prompt-level fix shape.
**Files touched:** `Docs/AGENTS/Agent_Outputs.md`
**Key decisions:** The LLM Expert now fully agrees with the adjusted proposal as the smallest safe LLM-first next step, but only if the old conflicting prompt lines are **replaced**, not supplemented. Final consolidated shape: (1) replace the Stage-1 plurality/per-instance decomposition guidance so collection-style inputs keep direct claims broad unless the input explicitly asks to assess instances separately, with proceeding coverage living primarily in `distinctEvents`; (2) replace the `distinctEvents` milestone-friendly include rule with a top-level event-cluster rule keyed to different institutional authorities, governing processes, or primary evidence bodies; (3) keep an explicit lifecycle/article exception; (4) add the anti-conflict sentence `Multiple distinctEvents do not by themselves require multiple AtomicClaims.` The expert also confirmed the validation gate should include explicit checks for `claimCount` stability and whether `MT-5(C)` fired.
**Open items:** If prompt-only validation fails, the next step should be a narrow LLM pass that rewrites only `distinctEvents`, not one that changes AtomicClaims.
**Warnings:** There is still one non-prompt caveat: if `MT-5(C)` fires, its current reprompt guidance in `claim-extraction-stage.ts` still leans toward one-claim-per-event. That is why the validation gate must record whether `MT-5(C)` fired before deciding prompt-only failed.
**For next agent:** Replace, do not supplement, these prompt areas: the plurality override around `claimboundary.prompt.md` line 135, the follow-on plural question guidance around line 169, and the `distinctEvents` include rule around line 202. Keep the jurisdiction guardrails and add the explicit sentence `Multiple distinctEvents do not by themselves require multiple AtomicClaims.`
**Learnings:** no

---
### 2026-03-31 | Lead Architect | Codex (GPT-5) | Attribution Check for Plastic and SRG `UNVERIFIED` Regressions
**Task:** Determine which change caused the `UNVERIFIED` outcomes in jobs `1cb64ac240d44987994e65f0057afeb2` (Plastic) and `0a34ef530fcd424798e1d88d30a2ba8b` (SRG compound).
**Files touched:** `Docs/AGENTS/Agent_Outputs.md`
**Key decisions:** The two regressions do **not** share one clean new culprit. For Plastic, the current `UNVERIFIED` result is not caused by the new `distinctEvents` prompt patch (the run has `distinctEvents = null`, no plurality path, and this family was already unstable across earlier commits with opposite verdict bands). The strongest shared upstream issue is the older Stage-1 broad-evaluative decomposition/contract-validation behavior introduced around `fff7a508`: the pipeline keeps split claims even when contract validation says the input should remain unified. For SRG, the same input was already `UNVERIFIED` before the current prompt patch in run `f80b6b41719d48f69732bc0eb0d45153` (`0758bef1...+d00e639d`), so the latest prompt change did not create the problem. The current worsening from one unresolved claim to all-claims-unverified is explained mainly by a weaker evidence run (20 evidence / 6 boundaries / 4 supporting → 12 evidence / 3 boundaries / 0 supporting), not by a clearly attributable later code change. The March 31 fetch short-circuit (`5e948594`) is the only later change that could plausibly shrink evidence, but the stored warnings show no direct proof that domain short-circuiting actually fired, so it should not be blamed as the cause without stronger evidence.
**Open items:** If needed, the next focused work items are separate: (1) broad evaluative claim unification for Plastic-like inputs, and (2) SRG-specific Stage-1 / retrieval validation to explain why the same bad split sometimes ends with 6 boundaries and sometimes only 3.
**Warnings:** Do not blame the current `distinctEvents` prompt patch for these two `UNVERIFIED` outcomes. It is inert on both families (`distinctEvents = null`). Also do not blame article adjudication (`03387283`) — the Plastic family already had non-UNVERIFIED outcomes under that same article path, and SRG had a partially unresolved but still `MOSTLY-TRUE` run under it.
**For next agent:** Use these anchors: Plastic `1cb64ac2` vs `3a46cf96` vs `fd1c1bfc` vs `3c67b3ac`; SRG `0a34ef53` vs `f80b6b41` vs `1b32f39a` vs `9ca3393c`. The key proof points are in `resultJson.understanding.contractValidationSummary`, `claimCount`, `claimVerdicts`, and `meta.boundaryCount/evidenceBalance`. `0a34...` and `1cb64...` both ran on prompt hash `e231a667`, but the changed prompt sections do not apply to these families, so correlation is not causation.
**Learnings:** no

---
### 2026-03-31 | Lead Architect | Codex (GPT-5) | Stage 1 Hardening for `fff7a508` Contract-Validation Fail-Open
**Task:** Patch the Stage-1 claim-extraction path so the `fff7a508` contract-validation package no longer lets broad evaluative multi-claim splits survive unchanged when contract validation returns no usable output.
...
**Key decisions:** Added a narrow contract-safety retry in `extractClaims()` for the specific failure mode where contract validation returns `undefined` on a multi-claim Pass 2 output. Instead of pure fail-open, the pipeline now retries Pass 2 once with merge-biased guidance and re-validates the result; it adopts the retry when it validates cleanly or at least reduces claim count. Also strengthened the existing contract-correction guidance for `single_atomic_claim` inputs so the retry explicitly asks for exactly one claim unless the input itself contains multiple independent assertions. Added focused regression tests covering both the SRG-style fail-open path and the Plastic-style single-claim correction path.
**Open items:** This hardens the Stage-1 regression path but does not yet prove live-job recovery on the real SRG/Plastic families; re-run those inputs after commit to confirm the broad-evaluative split is actually reduced in production LLM behavior.
**Warnings:** The safety retry adds extra LLM calls only on a narrow failure path. It does not change verdict logic or retrieval. `contractValidationSummary` still records the initial fail-open condition, so the stored summary may still say `fail-open` even when the safety retry subsequently improved the candidate set.
**For next agent:** The two new regression tests are in `claimboundary-pipeline.test.ts` under `Stage 1: extractClaims reprompt loop`. Verification passed: `npm -w apps/web exec vitest run test/unit/lib/analyzer/claimboundary-pipeline.test.ts`, `npm -w apps/web exec vitest run test/unit/lib/analyzer/claim-contract-validation.test.ts`, `npm test`, and `npm -w apps/web run build`.
**Learnings:** no

---
### 2026-03-31 | Lead Architect | Codex (GPT-5) | Stage 1 Candidate-Selection Refinement for Plastic/SRG
**Task:** Refine the `fff7a508` hardening patch after deeper review of the Plastic and SRG regressions showed that contract-validation failure is only one branch of the broader Stage-1 problem.
**Files touched:** `apps/web/src/lib/analyzer/claim-extraction-stage.ts`, `apps/web/test/unit/lib/analyzer/claimboundary-pipeline.test.ts`, `Docs/AGENTS/Agent_Outputs.md`
**Key decisions:** Updated the diagnosis to two branches under the same Stage-1 regression: SRG fails open when contract validation returns `undefined`, while Plastic returns a valid contract rejection but the retry-selection logic had been wrongly preferring a smaller still-failing candidate. Refined `extractClaims()` so the normal contract-correction path no longer uses “fewer claims” as a tiebreaker when the retry still fails re-validation, and the safety-retry branch now adopts a retry only if it validates cleanly. Kept the explicit `single_atomic_claim` retry directive. Added focused regression tests for both “keep original when smaller retry still fails” branches.
**Open items:** The remaining follow-up is live confirmation on the real SRG and Plastic inputs after commit. A later diagnostics pass can still improve `validateClaimContract()` by distinguishing `undefined` failure modes instead of collapsing them all into the same fail-open summary.
**Warnings:** This patch is intentionally conservative. It favors preserving the original Pass 2 output over adopting a smaller retry unless the retry clearly validates better. It does not yet add `result.text` fallback parsing for structured-output failures; that remains a separate evidence-gated follow-up.
**For next agent:** Regression coverage now includes four relevant tests in `claimboundary-pipeline.test.ts`: the safety-retry success case, the safety-retry keep-original case, the explicit single-claim retry-guidance case, and the normal retry keep-original case. After commit, rerun the live anchors `0a34ef530fcd424798e1d88d30a2ba8b` and `1cb64ac240d44987994e65f0057afeb2` equivalents to verify real-model behavior matches the mocks.
**Learnings:** no
