...
---
### 2026-05-25 | Senior Developer | Codex (GPT-5) | Daily Bug Scan No Confirmed Runtime Regression -- [Standard] [open-items: no]
**For next agent:** Scanned seven commits after `2026-05-24T17:27:53Z`: `4a757122`, `d2d06f83`, `4b01b0ee`, `d4114bd3`, `1eaf8b69`, `2dccb322`, and `6164ef8e`. The only product-code surface was Stage 1.5 automatic claim selection plus the wrapper root fix already covered by yesterday's scan. Verification passed: selector/filter/warning/config/pipeline focused Vitest suite (112 tests), wrapper `node --check`, `npm -w apps/web run build`, and full safe `npm test`. GitHub Actions had no run for HEAD `6164ef8e`; latest observed runs were Dependabot successes on `d4114bd3`. No concrete likely runtime bug was found. `git diff --check 4a757122^..HEAD` still reports one non-runtime whitespace issue: `Docs/AGENTS/Handoffs/2026-05-25_Senior_Developer_Claim_Auto_Selection_Root_Cause_Fix.md:81` has a blank line at EOF.
**Warnings:** Existing untracked `Docs/WIP/2026-05-25_Local_vs_Deployed_Regression_Investigation.md` was left untouched and was not part of the committed-change scan.
**Learnings:** No new role learning appended.
...
---
### 2026-05-25 | LLM Expert | Codex (GPT-5) | Bolsonaro EN Corrected Expectation Regression Fix -- [Standard] [open-items: yes]
**For next agent:** Implemented the corrected `bolsonaro-en` quality bar: docs now reject `MIXED`, require true-side labels (`LEANING-TRUE` or stronger), confidence minimum 65, and keep `3dab7adc2d514c2fae2e2a5003adc97e` as the direction/confidence/structure comparator while noting its state.gov residue. Prompt contracts now generically reject any thesis-direct claim that internally bundles separable evaluands, require direct applicability to share both target object and evaluated dimension, and require non-fallback verdicts to cite direct directional evidence. Stage 4 validation now treats publishable zero-directional-citation verdicts as integrity failures, includes that issue in repair/normalization paths, and normalizes verdict labels plus confidence tiers before returning.
**Warnings:** Live Bolsonaro rerun was not submitted because the plan requires commit plus prompt reseed/restart first. Build poststep reseeded local UCM prompt `claimboundary` with hash prefix `b2bf94d5cade`; runtime services still need a deliberate refresh before any live job. Pre-existing dirty files in `Docs/AGENTS/Agent_Outputs.md`, generated indexes, and `Docs/WIP/2026-05-25_Local_vs_Deployed_Regression_Investigation.md` were not evaluated as part of this fix.
**Learnings:** The controlling Bolsonaro EN expectation was stricter in archived Phase 2/scorecard/Phase 7 docs than in current benchmark docs; current baseline files had drifted too permissive by still allowing `MIXED`.
### 2026-05-26 | Codex | Local DB Pipeline_V2 Job Deletion — [Standard] [open-items: no]
**For next agent:** Backed up `apps/api/factharbor.db` with an online SQLite backup before deletion, verified `integrity_check=ok`, then deleted only jobs whose creation-time `GitCommitHash` base was reachable from local `Pipeline_V2` and not reachable from local `main`. Guardrails excluded any main-created jobs, execution-only V2 matches, and active `QUEUED`/`RUNNING` jobs; all three exclusion counts were zero before deletion. Deleted `576` `Jobs`, `32315` `JobEvents`, and `346` `AnalysisMetrics`; cleared `126` `ClaimSelectionDrafts.FinalJobId` references that pointed to deleted jobs, without deleting draft rows or changing invite quota counters. Post-delete verification: `Jobs=1437`, `JobEvents=111684`, `AnalysisMetrics=1475`, `ClaimSelectionDrafts=266`, remaining eligible V2-only creation-hash jobs `0`, `integrity_check=ok`.

**Warnings/Learnings:** Backup is `backups/factharbor-db/20260526-071536/factharbor.db` with manifest `backup-manifest.json` and SHA-256 `d84722178d34395a1f1d0b2eefb2dae552ec0e6fb7744b6de27e0dfcca68c588`. Deletion artifacts are `deleted-job-candidates-20260526-071759.csv`, `delete-summary-20260526-071759.json`, and `test-output/job-audits/delete-pipeline-v2-only-jobs-20260526-071759.summary.json`. Four dangling `ClaimSelectionDrafts.FinalJobId` references remain, but the verified backup had the same four, so they predate this cleanup.

---
### 2026-05-26 | Lead Developer + LLM Expert | Codex (GPT-5) | Bolsonaro EN d04587/dea2cc Regression Reassessment -- [Standard] [open-items: yes]
**For next agent:** Reassessed commit `d04587ba7e4036e208446bbb9c42f508ebdd9351` and job `dea2cc985b5a4c8d92650d4e33836a9b`. Do not fully revert `d04587ba`: it corrected the documented `bolsonaro-en` bar and `dea2cc` shows the generic claim split and no-visible-state.gov goals working (`3` claims, `6` boundaries, `0` state.gov evidence). But `dea2cc` is still a hard failure against Captain's corrected expectation: `UNVERIFIED 53.6/24`, with AC_02 downgraded to `verdict_integrity_failure` and the existing Stage 5 integrity cap reducing aggregate confidence to `24`. Best comparator remains `3dab7adc` for verdict/confidence/structure (`LEANING-TRUE 60.1/65`, but visible state.gov citations) plus `ec9840ff` for clean final citations (`LEANING-TRUE 64.2/50`, no cited state.gov). Immediate plan: keep docs/claim-splitting/no-citation/label-normalization changes, quarantine the Stage 4/Stage 5 integrity interaction for focused redesign, and investigate generic prompt handling for rule-governed evaluative claims whose direct evidence is component-based rather than whole-claim wording.
**Warnings:** Claude Opus sidecar agreed with "keep, validate, then tune" and noted the `dea2cc` failure is not the May 25 selector/V2 stale-runtime failure. Gemini sidecar correctly identified expectation/comparator docs but misdiagnosed `dea2cc` as selector failure; local DB inspection disproves that (`PipelineVariant=claimboundary`, `GitCommitHash=ExecutedWebGitCommitHash=d04587ba`, 3 selected claims, 65+ LLM calls). During sidecar work, an unrelated local DB cleanup appeared in the worktree and modified `apps/api/factharbor.db`, appended an Agent_Outputs entry, and created `backups/factharbor-db/20260526-071536/`; the three Bolsonaro comparator jobs still exist and DB integrity is `ok`, but restoration was not attempted without Captain approval.
**Learnings:** A single claim-level `verdict_integrity_failure` currently caps the whole article confidence through `aggregation-stage.ts`, so a local Stage 4 downgrade can dominate the final report even when the other claims have directional evidence. Future diagnosis must distinguish zero-directional-citation failures introduced by `d04587ba` from the older direction-validator/safe-downgrade path, because `dea2cc` triggered the latter.

---
### 2026-05-26 | Lead Developer + LLM Expert | Claude Opus 4.7 | Bolsonaro EN d04587 Surgical Follow-up -- [Standard] [open-items: yes]
**For next agent:** Implemented the consolidated plan that picked up from the prior Codex (GPT-5) reassessment. Verified independently with `git blame` that the article-wide integrity cascade (`hasIntegrityDowngrade`) was introduced in commit `6e347f095` on 2026-03-23, NOT by `d04587ba`. Cross-family proof: job `7b3c4cc3` (plastic-en, commit `0482c962`, dated 2026-05-01) already hit the cascade with AC_01 carrying 18 supporting/17 contradicting cites and conf=24 yet capping article to UNVERIFIED 51/24 — the cascade is a pre-existing cross-family defect. Rejected full revert; applied three surgical changes: (1) removed the article-wide INSUFFICIENT cap in `aggregation-stage.ts:259-264, 351-355, 367-370` so a per-claim integrity_failure's already-capped confidence flows through the weighted average proportionally rather than poisoning the whole article; (2) loosened `claimboundary.prompt.md:2278` with a generic two-form bridge — (a) target-bridge (same target + same dimension) OR (b) adjudicator-bridge (authoritative external evaluator of the responsible adjudicator's recurring practice on the same procedural/substantive integrity dimension), explicitly excluding advocacy, partisan commentary, and foreign-government assessments; (3) tightened `claimboundary.prompt.md:1461` so the LLM must emit `confidence < 25` (INSUFFICIENT tier) with truth in 40-60 when zero-direct-citation, avoiding the safeDowngradeVerdict path entirely. Added two cascade unit tests in `claimboundary-pipeline.test.ts` and one prompt-contract test for the dimensional bridge. All 1831 safe tests pass; `npm -w apps/web run build` succeeds; postbuild reseeded `claimboundary` UCM hash to `88b57fc0d77c`.
**Warnings:** No live Bolsonaro rerun submitted yet — needs dev-server restart, then exact Captain-defined `bolsonaro-en` input. Even with these three changes, hitting the corrected bar (LEANING-TRUE+ / conf 65-85) is not guaranteed: the closest pre-`d04587ba` comparator `ec9840ff` was `LEANING-TRUE 64.2/50`, so a third intervention on evidence acquisition or direct-source coverage may still be needed to close the conf-65 gap. The predecessor's flagged unrelated DB-cleanup state still persists: `backups/` directory in worktree is now also `.gitignore`d (auto-added by tooling during this session), so it stays out of version control. AC_01's unexplained UNVERIFIED label at truth=58/conf=48 (default bands say LEANING-TRUE) remains an open puzzle; flag for follow-up.
**Learnings:** Pre-existing systemic defects can wear the costume of recent regressions. Without `git blame`-grade attribution, an adversarial reviewer's "full revert" recommendation would have removed a working improvement (3-claim split, label normalization, doc corrections) without touching the actual root cause. The dimensional-bridge formulation generalizes cleanly to other rule-governed evaluative claims: same adjudicator + same procedural integrity dimension qualifies even when episode differs — multilingual-safe, no topic keywords.

---
### 2026-05-26 | Lead Developer + LLM Expert | Claude Opus 4.7 | Bolsonaro Follow-up + Verdict-Tier Evaluation + Plastic-EN Bar Correction -- [Standard] [open-items: yes]
**For next agent:** Extended the Bolsonaro EN session into a broader verdict-tier evaluation. Eight commits landed: `ca510f4c` (cascade + directness bridge + INSUFFICIENT-fallback), `9b2c6039` (surface-pattern recognition for bundled evaluative claims), `48b34617` (zero-citation INSUFFICIENT exclusion from aggregation), `2d07162e` (narrative confidence cap bounded to 5pp default), `37cf92bf` (disabled Anthropic prompt caching — observed 0 hits across 65 calls), `074fa27c` (GPT-5.x pricing entries), `7008b33f` (corrected Claude Opus family pricing $15/$75 → actual $5/$25), `5b95eaa3` (plastic-en bar corrected to TRUE side per Phase 2 R3.1 — analogous to bolso correction in d04587ba). Live three-way verdict-tier evaluation on Sonnet/GPT-5.4-mini/Opus-4-6 across four families (bolso, hydrogen, bundesrat, plastic). Sonnet is the only model that clears the bar consistently: bolso LEANING-TRUE 59/39 (conf gap), hydrogen MOSTLY-FALSE 18/65 (clean), bundesrat MIXED 55/88 (HIGH tier), plastic LEANING-TRUE 64/74 (clean on corrected bar). Captain decided to stay with Sonnet 4-6 as default verdict tier; GPT-5.4-mini and Opus matrices showed mixed/regressed outcomes. Total session cost ~$26 across exploratory matrices.
**Warnings:** (1) The plastic-en bar drift is the SECOND known case (after bolso); a broader audit of all 8 family bars against the Phase 2 archive may be worth doing in a follow-up session to surface any other silent drifts. (2) Bolsonaro EN conf 65-bar gap remains an open evidence-availability problem — none of the four verdict-tier models cleared the conf-65 threshold; the gap is independent of model choice. (3) The Anthropic prompt-caching disable is total — call sites still have the `getPromptCachingOptions(...)` wiring as a no-op; re-enabling later requires only removing the early `return undefined` once the prefix-sharing root cause is investigated. (4) The pre-existing dirty working-tree `backups/factharbor-db/20260526-071536/` directory is now also `.gitignore`d. (5) The verdict-tier experiment temporarily redirected `ANTHROPIC_VERSIONS.standard` and `OPENAI_VERSIONS.standard` in `model-resolver.ts`; both reverts are in the worktree (uncommitted experiment scaffolding) — final state is the Sonnet baseline. (6) The structural debate-tier homogeneity warning surfaced during the GPT-5.4-mini experiment is a known concern any future verdict-tier swap must address (preserve cross-provider diversity by keeping challenger on OpenAI). (7) Open puzzle from prior session: AC_01 UNVERIFIED label at truth=58 conf=48 — investigated and not reproduced this session; still worth checking the percentageToClaimVerdict path in a quiet moment.
**Learnings:** No single verdict-tier model wins universally across families. GPT-5.4-mini cleared the bolso conf gap most cleanly but failed catastrophically on hydrogen (0 boundaries) and bundesrat (wrong direction). Opus 4-6 had the strongest single-claim decisive verdicts (hydrogen FALSE 5/88, bundesrat LEANING-FALSE 35/88) but under-utilized evidence on bolso (MIXED 45/45). Sonnet's per-family calibration consistency is its biggest practical advantage — the bar was tuned with Sonnet in mind, and even when Sonnet has a conf gap (bolso), it's stable and predictable. Prompt caching was a paper savings only — disabling it on main matches the Pipeline_V2 stance. The Anthropic Opus family is now affordable enough ($5/$25 vs the previously-encoded stale $15/$75) that future hybrid routing experiments are economically defensible if quality warrants. The plastic-en bar drift caught here mirrors the bolsonaro-en drift that d04587ba corrected — Phase 2 archive remains the authoritative calibration source when current JSON disagrees with documented historical expectations.
