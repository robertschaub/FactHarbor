...
| V2-RL-021 | HighJump temporary bar-lowering prompt/gate loosenings | keep | Captain Deputy / Lead Developer | First internal V2 report result is produced and reviewed, or two focused HighJump attempts fail to move the active stop, or a leak/schema/provenance failure appears | HJ36 lowers the W3-B source-material strong-anchor bar for bounded Serper search-preview records only, because HJ33-HJ35 repeated a zero Source Material coverage cliff. Re-tighten, merge, or replace with a provenance-grade source-strength signal before public cutover or after report review shows the lower bar harms EvidenceItem quality. Do not let HighJump loosenings become public report policy by default. |
| V2-RL-022 | W7-B boundary/verdict schema diagnostics projected through W8-B upstream stop attribution | retire | Lead Developer | Remove or fold into stable W7-B telemetry after two successive HighJump boundary/verdict canaries no longer need schema diagnostics, or at the end of the current HighJump tranche | Keep bounded path/code-only diagnostics while repairing the HJ-2 W7-B schema stop. Do not add raw provider output, schema messages, prompt text, source text, EvidenceItem text, or a new route. |
| V2-RL-023 | W8-G internal Alpha report draft projection route/sink | merge | Lead Developer | Merge into stable report writer/report-review surface after internal report review accepts the draft output shape, or quarantine if review finds the deterministic draft misleading or not useful | Keep default projection hash/length/provenance-only and inspection-only text access. Do not let W8-G become public report behavior without a separate cutover/report package. |
| V2-RL-024 | HJ37 bounded Serper linked-page Source Material | merge | Lead Developer | First internal reports prove linked-page text is useful, or report review shows noisy/unsupported extraction, or before public cutover | Temporary HighJump depth repair after HJ36 proved snippet-only Source Material was too shallow. Merge into a unified source-material fetch policy or replace with provenance-grade source-strength handling; do not let provider-link dereference become unbounded public source policy. |
| V2-RL-025 | V2 canary evidence capture helper (`scripts/capture-canary-evidence.mjs`) | keep | Captain Deputy / Lead Developer | Retire or merge once capture is stable in normal validation tooling or no longer used; review after 8 weeks (2026-07-17) | Standalone automation wrapping `capture-v2-artifacts.ps1` with job metadata fetch, containment verification, and ledger-consumption stub generation. Not wired into package.json or general procedures. |
