...
- **Watch (4/8):** `asylum-235000-de`, `asylum-wwii-de`, `bolsonaro-en`, `plastic-en` — asylum-current now has two post-fix in-band canaries, but remains watch-listed because one prior stability rerun flipped false-side and the latest pass is at the upper truth-band edge; asylum-WWII now has a first exact current-stack false-side pair but needs Captain acceptance / later spot-check before closure; Bolsonaro EN has a current exact true-side pass and no-edit comparator review, but remains watch-listed because low-confidence fair-trial/verdict claims and claim-local citation warning are acceptable residuals rather than closure proof; plastic improved from high `MIXED` 52 to false-side edge `LEANING-FALSE` 37/62, but remains watch-listed until Captain decides whether `LEANING-FALSE` belongs in the accepted label set
...
**Plastic current-state caution:** Current diagnostic job `939563ecbea14a4c90249eb13c9743ef` moved the exact canonical input to false-side (`LEANING-FALSE` 37/62) with 3 clean dimension claims, 6 boundaries, and 144 evidence items. This is a material improvement over `MIXED` 52 runs, but it is not promoted to best comparator because the label is a strict Q-BE1 mismatch under the current JSON and the truth score remains just above the nominal 10-35 band. Captain decision is needed before changing the accepted label set.
...
| `asylum-235000-de` | Must surface ONE clean official SEM aggregate total, not stitch from components. Must be true-side; `MIXED` is not acceptable. Latest current stability canary `aaaa8f6572c14b2bb3593866e1eefde5` passed in band at the upper edge (`MOSTLY-TRUE` 75/68); still watch residual component-reconstruction caveats. | `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` | LEANING-TRUE / MOSTLY-TRUE; truth 58–75; conf 40–70; min 1 boundary |
...
| `plastic-en` | No fallback UNVERIFIED collapse from the old Stage 4 parse failure or the 2026-05-10 stale-watchdog failure. Operational failure is fixed; latest diagnostic job `939563ecbea14a4c90249eb13c9743ef` improved to false-side edge `LEANING-FALSE` 37/62. Calibration remains open because `LEANING-FALSE` is not currently an accepted label and truth 37 is only inside the 8-point tolerance, not the nominal 10-35 band. | `Plastic recycling is pointless` | MOSTLY-FALSE / FALSE / MIXED; truth 10–35; conf 55–80; min 2 boundaries |
...
| `plastic-en` | `32f00bb32d644a909f0c99521e800536` — MOSTLY-FALSE 21/68 | No exact deployed match found | Local exact report remains the best comparator: 3 AtomicClaims and broad evidence. Current diagnostic `939563ecbea14a4c90249eb13c9743ef` is better than prior current `MIXED` 52 reports (`38655e2b`, `8e3c9b9d`) but is not a best comparator because `LEANING-FALSE` is not in the current expected label set and truth 37 is only tolerance-in-band. Best visible deployed family variant is German `800431527e254d2888ef56ba23af4688` (`Plastik Recycling bringt nichts`, LEANING-FALSE 29/59), but it is not the canonical English input and should remain a variant control. |
...
| `plastic-en` | No more immediate jobs. Current diagnostic `939563ecbea14a4c90249eb13c9743ef` is false-side edge (`LEANING-FALSE` 37/62), while current JSON still accepts only `MOSTLY-FALSE` / `FALSE` / `MIXED`. Captain must decide whether `LEANING-FALSE` is acceptable for this family before agents update the mechanical band or spend another same-stack repeat. | No — control lane only |
| `asylum-235000-de` | Latest current stability canary `aaaa8f6572c14b2bb3593866e1eefde5` passed at `MOSTLY-TRUE` 75/68. Captain clarified that this current-total input is the better asylum validation target than the WWII comparison. Next work is no-edit comparator review only; do not spend another asylum job unless Captain wants closure despite the upper-edge truth score. | Yes — active watch lane; two current passes exist but not broad closure |
