...
**Bolsonaro current-state caution:** Current exact job `aedb3a05046441aba3eb2f6047ca0e22` on `9b0e8efd` recovered the corrected true-side expectation overall (`LEANING-TRUE` 64/43) with the intended three AtomicClaims and no user-visible warnings. AC_02 and AC_03 are both verifiable and true-side, but confidence remains low (`32` and `30`) and overall confidence is 2pp below the canonical band; this is current-watch partial recovery, not broad closure or a replacement for the stronger `91bf6083` / deployed `85812d61` comparators.
...
**Plastic current-state note:** Captain accepted `LEANING-FALSE` as acceptable for plastic reports in English and other languages, and accepted current exact job `939563ecbea14a4c90249eb13c9743ef` as a good report. It moved the exact canonical input to false-side (`LEANING-FALSE` 37/62) with 3 clean dimension claims, 6 boundaries, and 144 evidence items. Best historical exact comparator `32f00bb32d644a909f0c99521e800536` remains useful for stronger false-side shape, but `939563ec` is now an accepted current-stack exact report.
...
| `bolsonaro-en` | Must be true-side with caveats: legal basis strongly supported; procedural fairness largely supported with legitimate concerns. Latest exact current canary `aedb3a05046441aba3eb2f6047ca0e22` has the correct three-claim true-side shape and no user-visible warnings, but confidence is still below band (`64/43` overall; AC_02 `62/32`, AC_03 `62/30`). Treat as partial recovery / watch, not comparator replacement. | `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?` | LEANING-TRUE / MOSTLY-TRUE; truth 58–85; conf 45–75; min 3 boundaries |
...
| `plastic-en` | No fallback UNVERIFIED collapse from the old Stage 4 parse failure or the 2026-05-10 stale-watchdog failure. Operational failure is fixed; Captain accepts `LEANING-FALSE` for plastic reports in English and other languages. Current exact job `939563ecbea14a4c90249eb13c9743ef` is a good current-stack report at `LEANING-FALSE` 37/62. | `Plastic recycling is pointless` | FALSE / MOSTLY-FALSE / LEANING-FALSE / MIXED; truth 10–42; conf 55–80; min 2 boundaries |
...
| `plastic-en` | `32f00bb32d644a909f0c99521e800536` — MOSTLY-FALSE 21/68; accepted current-stack exact `939563ecbea14a4c90249eb13c9743ef` — LEANING-FALSE 37/62 | No exact deployed match found | Local exact `32f00bb` remains the strongest historical comparator: 3 AtomicClaims and broad evidence. Captain accepts `939563ec` as a good current-stack exact report, and accepts `LEANING-FALSE` for plastic reports in English and other languages. Best visible deployed family variant is German `800431527e254d2888ef56ba23af4688` (`Plastik Recycling bringt nichts`, LEANING-FALSE 29/59), but it is not the canonical English input and should remain a variant control. |
...
| `plastic-en` | Captain accepted `LEANING-FALSE` as acceptable for plastic reports and accepted `939563ecbea14a4c90249eb13c9743ef` as a good report. Do not spend another immediate plastic job; use `939563ec` as current-stack exact reference and `32f00bb` as the stronger historical exact comparator. | No — control lane only |
...
| `bolsonaro-en` current partial recovery/watch | Current exact canary `aedb3a05046441aba3eb2f6047ca0e22` gives AC_02/AC_03 true-side, verifiable verdicts after the Stage-4 direction-summary and repair-band fixes. Next work is review/simplification, not another immediate prompt patch: compare against `91bf6083` / `85812d61`, decide whether low confidence is acceptable watch debt, and only then spend another stability job. | No — current direction is corrected, but confidence remains below band and below best comparators |
