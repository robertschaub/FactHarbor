...
**Status:** Living reference · **Last updated:** 2026-06-08 · **Purpose:** Captain's one-page human-readable quality bar for FactHarbor analysis reports.
...
|---|---|---|---|
| `bundesrat-rechtskraftig` | Must not collapse to UNVERIFIED on Stage 1; the `rechtskräftig` anchor must survive extraction. Current isolated rerun `f8e72c84fb004f23945e23c81973fc26` is accepted as good: the legal-force anchor dominates while the chronology remains true-side. An explicit secondary reading ("signed text fixed/authenticated, but treaty/package not yet in force") is valid only through the v0.2 dossier's frame-aware route, not by averaging chronology into the primary band. | `Der Bundesrat unterschrieb den EU-Vertrag rechtskräftig bevor Volk und Parlament darüber entschieden haben` | MIXED / LEANING-FALSE; truth 29–57; conf 55–85; min 2 boundaries |
...
- **No quality-gate / hard-failure error (baseline floor — precedes every other check).** Every report must reach `SUCCEEDED` with no hard-failure warning (`report_damaged`, `analysis_generation_failed`, `llm_provider_error`) — the PHASE-BLOCKER floor (`report-quality-expectations.json` Q-HF1). A verdict-band pass on a `report_damaged` job does **not** count. This is a **population-scale** expectation: the 8 benchmark families are a curated band-check set, **not** the input population. `report_damaged` is a recognized serious failure mode (the Q-HF1 PHASE-BLOCKER type). **Measured 2026-05-31** (`scripts/diag/checkworthy-unverified-census.cjs`, n=1550 SUCCEEDED): **8.6% hard-failure** (report_damaged 107 / analysis_generation_failed 19 / llm_provider_error 7) and **14.8% top-level UNVERIFIED**. Band conformance is necessary but **not sufficient**; gate-error rate is a first-class expectation.
