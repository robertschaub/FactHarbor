...
- **Watch (2/8):** `asylum-235000-de`, `plastic-en` — asylum now has a current in-band canary after component rows stayed neutral, but remains watch-listed because one prior stability rerun flipped false-side and the latest narrative still uses component reconstruction as caveat context; plastic evidence variance remains open
- **Not validated (1/8):** `asylum-wwii-de` — no current-stack rerun yet
- **Release-blockers:** none formally declared in this file; `asylum-235000-de` is no longer cleanup-only and should stay in the active report-improvement lane until one more review confirms component reconstruction remains caveat-only rather than decisive evidence.
...
| `asylum-235000-de` | Latest current canary `fd93d0de531243a18d2097b38351f4d4` passed at `LEANING-TRUE` 70/60: component rows stayed neutral and the only contradiction was the dated 2024 SEM aggregate. Next work is a short no-edit comparator review to confirm component reconstruction is caveat-only; avoid spending another asylum job unless that review identifies a focused stability question. | Yes — active watch lane; current pass exists but not broad closure |
