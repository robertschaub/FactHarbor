...
- **New Jobs columns need a startup entry.** `Program.cs` runs `Migrate()` for the registered EF migrations, then `EnsureJobsColumn` adds Jobs columns whose migrations are not registered. Add a new column there, or as a registered migration after fixing the model snapshot (it lacks `AdminAnnotation`). Never delete `factharbor.db` to rebuild the schema; see `apps/api/AGENTS.md`.
