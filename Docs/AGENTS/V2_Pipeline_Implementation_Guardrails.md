...
- Slice 6B.3c-4B is a provider-boundary ownership contract slice, not provider wiring. It adds an inert contract under `apps/web/src/lib/analyzer-v2-runtime/` plus tests/guards so the future product-owned provider callback factory can be reviewed without using V1 provider helpers, importing provider SDKs into Analyzer V2, creating callbacks, flipping approvals, adding product callers, exposing public results, enabling cache IO, or spending live jobs. Real factory construction remains blocked until a later source-wiring gate resolves the executable approval source, V2 config snapshot ownership, SDK import location, direct-text-only runtime entry, rollback/failure behavior, and live-job smoke plan.
