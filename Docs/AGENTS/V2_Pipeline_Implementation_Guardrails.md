...
- Slice 6B.3c-4C is a docs-only provider source-wiring approval package, not source approval. It proposes that the next possible source work be 4C1 approval-authority cleanup before any provider factory implementation: product/live dispatch must not rely on the 4A scaffold approval snapshot or a private executable gateway-task clone. Provider factory implementation, provider SDK imports, public exposure, cache IO, ACS/direct URL execution, live jobs, approval/status flips, prompt/config changes, and V1 cleanup remain blocked until separate approval.
