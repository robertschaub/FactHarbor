...
- Next Claim Understanding dispatch gate: product runtime dispatch remains blocked. The active gate package is `Docs/WIP/2026-05-14_V2_Slice_6B3c_Product_Runtime_Dispatch_Review_Package.md`; it approves no source code. A later reviewed slice must define the executable approval source, prompt/config/cache hash construction, cache no-read/no-write posture, provider boundary, URL resolution prerequisite, and the guard that replaces the product-path model-adapter import ban before any orchestrator dispatch code is allowed.
