...
- Slice 6B.3c-4C2 is currently a docs-only provider factory approval package at `Docs/WIP/2026-05-14_V2_Slice_6B3c4C2_Provider_Factory_Approval_Package.md`. It does not approve source. The proposed sequence is 4C2a inert provider runtime config/provenance contract, then 4C2b clean-room provider factory under `apps/web/src/lib/analyzer-v2-runtime/`, then later 4C3 product-hidden direct-text runtime artifact. Provider SDK imports, if ever approved, must be allowlisted to exactly one reviewed runtime factory file and remain forbidden inside Analyzer V2 and public/product surfaces.
