...
- Slice 7I is a docs-only Evidence Lifecycle prompt/model task design gate at `f58373a5` from `Docs/WIP/2026-05-15_V2_Slice_7I_Evidence_Prompt_Model_Task_Contract_Design.md`. It assigns future semantic ownership for query planning, applicability, extraction, probative value/evidence strength, source-language strategy, and supplementary-lane decisions to LLM-owned tasks under later Captain-approved prompt/model/UCM policy. It authorizes no source/test edits, prompts, schemas, UCM/default changes, gateway/model approval flips, prompt loader/model adapter/runtime wiring, hidden artifact expansion, provider/search/fetch execution, cache IO, SR integration, public exposure, live jobs, or V1 cleanup. Slice 7J is prepared at `c5da518d` from `Docs/WIP/2026-05-15_V2_Slice_7J_Evidence_Prompt_Schema_Model_UCM_Approval_Package.md` after Gemini/Senior Developer/Claude Opus review. It remains an approval package only; 7J-1 source work requires direct Captain approval with the exact package wording and remains limited to non-executable prompt/schema/task-policy metadata plus inert verifier tests.
