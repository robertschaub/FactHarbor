...
- Next Claim Understanding dispatch gate: 6B.3c-3B3 is complete at `d615b699` as an internal direct-text runtime-dispatch owner only. Post-3B3 product wiring review returned `MODIFY/BLOCK`, and follow-up review of `Docs/WIP/2026-05-14_V2_Slice_6B3c4_Product_Runtime_Dispatch_Wiring_Gate.md` also lacked source-wiring consent. Product runtime wiring requires explicit approval before source changes; Captain clarified that risky but meaningful progress should be escalated for confirmation rather than postponed indefinitely. Only guard/test hardening that preserves the current no-product-reach topology is low-risk. The first such hardening split cache policy metadata into `gateway/cache-policy-registry.ts`, so product paths can reach policy metadata without transitive reach into cache-governance behavior. Before any product wiring source change, a deputy-reviewed source package must define product-owned runtime approval authority, provider callback factory ownership outside Analyzer V2, API acceptance mode, direct-text-only limits, public leak guards, rollback, failure classification, live-job discipline, and V1-removal separation. API/UI/report/export changes, ACS/direct URL execution, cache read/write, provider SDK imports in Analyzer V2, approval/status flips, prompt/config changes, live jobs, and V1 cleanup remain forbidden without separate approval.
