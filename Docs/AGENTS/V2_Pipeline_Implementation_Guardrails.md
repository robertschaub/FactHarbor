...
- Live-job rule: live jobs become meaningful only after a committed, runtime-refreshed source-wiring slice can produce a real hidden V2 direct-text artifact without scaffold-only injection. The current live-job authority is the machine-readable tranche ledger at `Docs/AGENTS/V2_Live_Job_Tranche_Ledger.json`; this guardrail file does not grant live-job budget by itself. As of the W5-G/W5-H stop-line state on 2026-05-20, that ledger records `currentRemaining = 0`, and W6-C0 authorizes no live jobs. A future live job requires a new or replenished ledger allowance plus a reviewed slice package with tranche impact, commit/runtime-refresh discipline, and pass/stop criteria.
