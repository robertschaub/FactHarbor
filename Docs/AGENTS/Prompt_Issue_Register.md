...
- **Status:** unconfirmed
- **Description:** The reconciliation stage comment said the reconciler saw the full evidence pool, but the runtime payload only included advocate verdicts, challenges, self-consistency data, and source portfolios. The prompt also restricted final citation arrays to advocate/challenger IDs. This allowed the LLM to accept a missing-evidence challenge even when an acquired source contained relevant official or oversight-body material that was omitted or narrowly characterized by earlier stages.
- **Observed behavior:** Job `86462618095c431ca7a84e82630b296a` acquired the OAS/IACHR report as source `S_033` and extracted evidence from it, yet AC_02/AC_03 reconciliation reasoned as though no direct international or official assessment was present, instead of distinguishing no final case-specific adjudication from narrower acquired oversight material.
- **Recommended fix:** Pass the full acquired evidence pool to `VERDICT_RECONCILIATION`; require a full-pool check before accepting missing-evidence or source-absence challenges; allow the reconciler to inspect full-pool evidence while restricting directional citations to unmapped/current-claim evidence; and tighten generic extraction/applicability wording so international or regional oversight material is classified by what it actually assessed.
- **Mitigation applied:** 2026-05-29 revision passes compact `evidenceItems` and canonical `atomicClaims` into `VERDICT_RECONCILIATION`, updates the prompt to require a full-pool evidence check and to distinguish final adjudication absence from assessment absence, and structurally filters reconciled directional citation arrays against `relevantClaimIds` to prevent sibling-claim contamination. Prompt-contract/unit tests cover the payload and citation-scope guard. Active local claimboundary prompt hash after final reseed: `f0957eb946c151a5e54bd54fc70fb4c2cb4ec4b0c97f49947df3f1c762b0caf8`. No post-fix live rerun yet, so the issue remains `unconfirmed`, not `resolved`.
