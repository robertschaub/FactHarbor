...
### 2026-06-04 — When asked to "measure & rate report quality," the criteria already exist as Q-codes — design the reference model, not new rubrics
**Role:** Lead Architect  **Agent/Tool:** Claude Code (Opus 4.8, 1M)
**Category:** useful-pattern
**Learning:** A request to "measure/rate report quality and compare builds, decomposed into isolatable parts" looked like a new-rubric task; it wasn't. The Q-code catalog (`report-quality-expectations.json`) *already* partitions along the exact decomposition asked for — `Q-S1.*`=claim extraction, `Q-EV*`=evidence, `Q-V*`=per-claim verdict, `Q-BE*`=overall-verdict gold bands, `Q-ST*`=stability, `Q-HF1`=integrity gate. The real missing pieces were a *rating model*, a *build-comparison methodology*, and — the load-bearing one — an *honest reference model*. Four design decisions the advisor sharpened and that generalize: (1) **The reference gradient is the spine.** "Correct answer" availability is NOT uniform across a pipeline — gold lived only at the aggregate verdict (`benchmark-expectations.json` bands) + coarse structural floors; every sub-stage (claims, evidence, per-claim verdict) has none, so it's intrinsic-structural + cross-run-stability + judge. Frame this as the organizing principle, not a caveat. (2) **Build comparison is ordinal, not cardinal.** Where there's no gold, *do not* score-then-subtract (the judge's weakest mode on the highest-stakes layer) — judge **pairwise A-vs-B** on the same input (blind, order-balanced), which is far more reliable and reuses the existing `/report-review` debate panel. Keep absolute scores only where anchored. (3) **The composite gates; it never ranks.** A flat scalar hides a Stage-1 regression offset by a Stage-4 gain — the same masking forbidden for upstream/downstream attribution. Keep the per-component *vector* authoritative; rank builds only by Pareto-dominance or an explicit (Captain-owned) weighting. (4) **Run-to-run drift is the primary isolation confound** — claim-set Jaccard ≥0.6 is tolerated, so per-claim-verdict quality has no stable target; it must be scored conditional on the realized claim set and reported as a distribution, and cross-build it needs semantic claim alignment or must step up to the aggregate. Also: **calibration is a first-class, distinct build-layer dimension** (a build can get more correct AND worse calibrated — the plastic-en overconfidence pattern), and **anchor any judge on gold-band families first** (does its A/B ordering match band agreement?) before trusting it on ungolded layers. And true cross-build *per-stage causal* attribution needs cross-version claim injection (era-study conditions ③/④) — don't over-claim it from native runs alone.
**Files:** `Docs/WIP/2026-06-04_Report_Quality_Measurement_And_Build_Comparison_Concept.md`, `Docs/AGENTS/Handoffs/2026-06-04_Lead_Architect_Report_Quality_Measurement_Concept.md`

### 2026-06-04 — A late-arriving fact + a scoped second-opinion review can correctly flip a "locked" decision — verify the new approach's cost assumption before committing
**Role:** Lead Architect  **Agent/Tool:** Claude Code (Opus 4.8, 1M)
**Category:** useful-pattern
**Learning:** A multi-variant-pipeline architecture was designed, debated, Codex-reviewed, and had 5 decisions Captain-"locked" — then a single new fact ("the variants are *deeply code-divergent historical tags*", learned mid-design) plus a deliberately-scoped Gemini second opinion flipped the core decision from "in-tree re-implementation registry" to "run the original commits natively via `git worktree`." Lessons: (1) **A locked decision made before a key fact arrived is not safe** — when the fact lands, re-examine the decision it undercuts; the in-tree decision predated knowing the eras were 42/21/16-file-divergent rebuilds, and re-implementing them in-tree risks *silent semantic fidelity loss* (compiles, prompts quietly degrade) that a code-grounded review misses by construction. (2) **Scope a second-opinion model to a DISTINCT question** (experimental validity + simpler-architecture) rather than re-running the first reviewer's checklist — that's where a different model family adds non-overlapping value. (3) **Verify the challenger's cost assumption before pivoting:** Gemini called worktrees "light glue on different ports"; a `git diff --stat` of deps/schema/contract showed each era needs its own `npm install`, 2/3 need their own DB schema, all need their own API → *full-stack-per-era*, not trivial (still likely cheaper than faithful re-implementation, but the gap narrows). (4) **External-world drift (live LLM + search) means NEITHER native-run nor re-implementation recovers historical reports** — both measure "era code on today's world"; say so explicitly so the study's claim is honest. (5) The **2×2 ablation** (Era→Era, HEAD→HEAD, HEAD-claims→Era-downstream, Era-claims→HEAD-downstream) is the right attribution framing, and its cross-conditions ③&④ are exactly what *need* the in-tree claim-injection machinery — so worktrees (①&②) and in-tree (③&④) are complementary, not either/or. (6) When a review reopens a locked decision, surface it to the Captain as an explicit priority call (study vs durable capability) — don't silently fold it in, and don't pre-edit the superseded docs until the direction is set. Outcome: pivoted to a worktree study plan; the in-tree docs preserved as a decoupled deferred capability.
**Files:** `Docs/WIP/2026-06-04_Pipeline_Era_Comparison_Worktree_Study_Plan.md`, `Docs/WIP/2026-06-04_Multi_Variant_Pipeline_{Architecture,Specification,Implementation_Plan}.md` (deferred), `Docs/WIP/2026-06-04_Multi_Variant_Pipeline_{Codex,Gemini}_*Review*.md`
...
### 2026-05-24 — Failed Pipeline V2 shows process machinery can become the product
...
**Role:** Lead Architect  **Agent/Tool:** Claude Code (Opus 4.8 1M)
**Category:** gotcha
**Learning:** A `report_damaged` on the hydrogen benchmark was read by two prior debates (Claude Opus 4.6, Gemini 3.1) as a "Rule 2/Rule 3 proxy-drift contradiction" in `CLAIM_CONTRACT_VALIDATION`, with the fix being a prompt edit to Rule 3. Static-reading the current code/prompt made that look right. But the case was a `SOLVED` benchmark (`FALSE 12/82`), so it was a **regression** — and `git log -L` showed Rule 2/Rule 3 were **unchanged since `4f7d3850` (03-20)**, i.e. identical when the case passed. The regression therefore could not live in those rules; deeper diffing then exonerated *every* in-window static suspect too (the self-consistency check `b5b6ce4e` and `report_damaged` termination `f5cf31ee` predate last-good; `0d97a7a6` only *added* recovery; comparison-side rules don't apply to dimension splits). **Then a live re-run test settled it: 4 fresh runs at the same source all PASSED with in-band verdicts (4 pass / 1 fail) — so it was never a deterministic regression at all; it was a FLAKY contract-validation judgment** that occasionally flags a borderline measurement-frame decomposition as "material proxy drift" and emits a *terminal* `report_damaged`. **Strongest lesson: a single observed `report_damaged` on a SOLVED benchmark is not evidence of a regression — confirm determinism by re-running 3-5× *before* attributing it to any commit or proposing a revert/bisect.** A flaky LLM-judgment abort needs *gate hardening* (don't terminally abort on an occasional borderline contract judgment — drop/repair the offending claim and proceed), not a rule edit or revert. Lesson on method: when a previously-passing case breaks, bracket last-good→current from the benchmark's `latestVerifiedJobId` and diff the window; don't attribute from a static read; and don't edit a rule whose history shows it predates the break. Corollary: persisted job traces may omit raw LLM booleans (`preservesOriginalClaimContract`, per-claim `proxyDriftSeverity`) — if a "self-contradiction"/override claim depends on them, you cannot isolate the load-bearing trigger from the trace alone; say so rather than over-attributing.
...
### 2026-05-31 — A prompt tiebreaker can't stabilize a stochastic LLM classification; and never launch the FH dev server from the agent shell
**Role:** Lead Architect  **Agent/Tool:** Claude Code (Opus 4.8 1M)
**Category:** gotcha
