...
**Key decisions:** Active V2 workspace: `C:\DEV\FactHarbor`. Git branch: `main`. Preserved prior workspace: `C:\DEV\FactHarbor-main-before-v2-rehome`. Preserved Git branch: `codex/main-before-v2-rehome` at `31b3ea90`, including commit `31b3ea90` for the daily bug scan note that had been dirty on `main`. The retired duplicate V2 workspace was detached, verified clean, and removed from Git worktree registration before the V2 branch was checked out at the canonical path.
**Open items:** None for the workspace rehome. After reboot, the retired duplicate V2 workspace directory was deleted.
**Warnings:** Current V2 workspace: `C:\DEV\FactHarbor`. Current Git branch: `main`. Do not continue V2 work from any retired duplicate path.
