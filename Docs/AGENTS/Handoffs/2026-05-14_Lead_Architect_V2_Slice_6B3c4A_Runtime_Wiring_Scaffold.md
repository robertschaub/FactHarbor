...
**For next agent:** Start from `runClaimUnderstandingRuntimeStage(...)` in `apps/web/src/lib/analyzer-v2/claim-understanding/runtime-stage.ts`. The safe next implementation step should either keep hardening the scaffold or design the next reviewed gate for provider-boundary ownership; do not add API/UI/report/export exposure or V1 cleanup in the same slice.
