...
**For next agent:** Start from current `main` HEAD `21e9a817`. The next decision is Captain-owned: accept `LEANING-FALSE` as an allowed `plastic-en` label or keep the stricter current label set. If accepted, update the mechanical expected labels and optionally spend one same-stack repeat only if deployment approval requires a fresh job hash. If rejected, do not stack prompt changes; inspect contradiction-query generation/admission for broad absolute claims first.
