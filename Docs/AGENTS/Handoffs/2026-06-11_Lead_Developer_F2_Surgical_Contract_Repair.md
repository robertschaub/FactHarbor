...
3. **Coverage boundary (intentional, documented in the F2 block comment):** challenge-driven violations (single-claim atomicity, binding challenge, anchor-provenance, normative injection) carry no per-claim critique → surgical pass skips them gracefully; they fall through to completion. If single-claim bundling (the comparative "more efficient" shape) should also be surgically repairable, the extension point is synthesizing a flagged assessment from `atomicityRetryReason` — a design decision for Captain, not landed.
