...
>
> Captain assignment overrides these defaults. See `Docs/AGENTS/Active_Team_Composition.md` for current model-to-role assignments (Leader, executor, reviewers). The table below describes **tool** capabilities, not active role assignments.

| Task Type | Best Tool | Model Tier | Why |
|-----------|-----------|------------|-----|
| Primary implementation, bug fixes, repo-local execution | Codex / GPT-5.5 | High | Reads `AGENTS.md` natively, supports repo-local skills, shell work, and direct verification |
...
| Deep investigation, consolidation | Codex/GPT-5.5 or Claude Opus 4.6 | High | Use Codex when implementation/verification is likely; use Claude when the task is review-only or adversarial synthesis |
