# V2 Slice X7-W2-LS4 DIAG3 Transport Phase Live-Observation Package
...
**Parent implementation:** `5b04bc3a` (`feat: add v2 w2 transport phase diagnostics`)
**Parent package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-DIAG3_Sanitized_Transport_Phase_Diagnostics_Source_Package.md`

## 1. Purpose

X7-W2-LS4 is a one-job diagnostic live observation after DIAG3. It must answer one narrow question:

```text
When the committed/refreshed product V2 route reaches W2, which enum-only
transport phase, error shape, and code category are recorded on the hidden W2
network attempts?
```

LS4 is not a provider-success gate, source-quality gate, report-quality gate, public-readiness gate, W2 completion-semantics repair, provider-portfolio decision, Wikimedia endorsement, or V1-cleanup gate.

## 2. Authorization

After this docs-only package is committed and all pre-live checks pass, LS4 authorizes exactly one live job using the exact Captain-defined input in section 4.
...
LS4 passes its diagnostic objective if:

- the job reaches `SUCCEEDED`;
- the first preparation event shows `pipeline: claimboundary-v2`;
- package/runtime provenance is clean and matches the committed LS4 package revision;
...
- every W2 network attempt includes bounded values for the DIAG2 fields:
  - `dnsAddressCount`;
...
- every W2 network attempt includes bounded values for the DIAG3 fields:
...
`candidateProviderNetwork.status` may still be `candidate_provider_network_damaged_structural`. That remains a W2 provider-network completion failure, but LS4 can pass as `PASS_X7_W2_LS4_DIAG3_TELEMETRY_CAPTURED` if the diagnostic objective above is met. If W2 completes successfully, record that as extra evidence; do not treat it as source-material or report-quality approval.

## 4. Exact Input

Use the same exact Captain-defined input as LS2 and LS3 so the diagnostic result is comparable:
...
LS4 is docs-only before live execution. Allowed package files:

- `Docs/WIP/2026-05-18_V2_Slice_X7-W2-LS4_DIAG3_Transport_Phase_Live_Observation_Package.md`
...
if ($beforeIdle) { throw "X7-W2-LS4 pre-submission worktree is dirty before idle checkpoint" }
Start-Sleep -Seconds 60
$afterIdle = git status --short --untracked-files=all
if ($afterIdle) { throw "X7-W2-LS4 pre-submission worktree became dirty during idle checkpoint" }
```
...
- any W2 network attempt lacks one of the DIAG2 or DIAG3 fields;
- any DIAG2 or DIAG3 field value is outside the approved enum/count contracts;
...
Do not repair inside LS4. Create a separate reviewed fix package if needed.
...
- all W2 DIAG2 and DIAG3 field values by attempt;
...
Review `Docs/WIP/2026-05-18_V2_Slice_X7-W2-LS4_DIAG3_Transport_Phase_Live_Observation_Package.md`.

Return `approve`, `modify`, or `reject`.

Check whether LS4 is a safe one-job diagnostic live observation after DIAG3. Pay attention to:

- exact Captain-defined input choice and comparability to LS2/LS3;
...
- no retries or second canary inside LS4;
- hard-fail behavior on V1 fallback, hidden leaks, endpoint drift, missing DIAG2/DIAG3 fields, or any need for code/prompt/config edits.

## 15. Review Decision

| Role | Reviewer | Date | Decision | Notes |
|---|---|---:|---|---|
| Claude Opus 4.6 senior architect/security | Claude Opus 4.6 via `scripts/agents/invoke-claude.cjs` | 2026-05-18 | APPROVE | Reviewer confirmed exact input/comparability, strict pre-live gate ordering, one provider/one job/no retry, diagnostic pass bar separated from W2 completion, forbidden behavior coverage, hidden/public leakage controls, docs-only file envelope, and hard-fail criteria. Execution-time notes: verify focused test file list, route existence, and LS3 ledger-id shape before submission. |
