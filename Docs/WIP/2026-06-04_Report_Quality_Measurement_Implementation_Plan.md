...
| # | Decision | Status |
|---|---|---|
| ③ | Cross-provider judge (GPT-5.5 vs in-house multi-vote panel) | Phase-2 implementation choice only; the v0.2 judge input/output contract is fixed in the reference-data model. |
...
- **P0b.1** Instantiate dossier files from `Docs/WIP/2026-06-06_AtomicClaim_Reference_Data_Model.md`: `expectedInputClassification`, dossier-level `ambiguityPolicy`, `frameRole`, frame-scoped `atomicityProfile` including `determinabilityStatus`, truth-condition dominance roles/weights, required reference assertions, truth/confidence bands, source snapshots, and `validityWindow` for current-snapshot assertions.
- **P0b.2** Use the manual-alignment rubric and score artifact contract in the reference-data model: active-frame selection, frame-role fit, assertion coverage, Stage-1-only dominance-weighted atomicity fidelity, disclosure fidelity, C3 verdict/evidence equivalence, tie handling, and `needs_human_review`.
...
- **P0b.6** Instantiate strict-JSON C1 alignment and C3 evidence-equivalence judge prompts from the fixed v0.2 judge output contracts; do not add labels, fields, or scoring rules, and do not run them until Captain approves a cap.
