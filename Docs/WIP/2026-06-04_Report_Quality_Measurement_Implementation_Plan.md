...
2. **The reference gradient.** "Right answer" availability is *not* uniform: aggregate gold exists by default at the overall verdict (C4) (`benchmark-expectations.json`) and a few coarse floors. Deeper AtomicClaim-level gold exists only for families with independently reviewed reference dossiers, scored through frame-scoped N:M semantic alignment with per-frame atomicity fidelity, explicit primary/secondary/caveat frame roles, and explicit `frameAggregationMode` (`dominance_weighted`, `balanced_composite`, or `no_topline`) — never through strict claim-string matching, never by forcing ambiguous terms into a false binary, never by assuming every scorable frame has a dominant AtomicClaim, never by letting ambiguity hide clearly distinct truth conditions, never by raw-count averaging true supporting claims over a dominant false claim, and never by rescuing C1 with downstream verdict/evidence. Until such dossiers exist and pass Phase 0b gates, C1/C2/C3 remain structural checks + cross-run stability + judge. Current-snapshot dossiers are time-relative gold and require dossier-version + run-window pinning. See `Docs/WIP/2026-06-06_AtomicClaim_Reference_Data_Model.md`.
...
| **C4 harm-adjusted gold-band** | **RANK** | ResultJson + benchmark bands; linked dossier primary `topLineAssertionIds` provide the machine-checked top-line contract where available; `alternativeTopLineRoutes` are frame-aware/manual until Phase 0b alignment can select them per report | **[−100,100]**, raw mean (no clip) |
...
| # | Decision | Status |
|---|---|---|
| ③ | Cross-provider judge (GPT-5.5 vs in-house multi-vote panel) | Phase-2 implementation choice only; the reference-data model fixes the judge input/output contract. |
...
- **P0b.1** Instantiate dossier files from `Docs/WIP/2026-06-06_AtomicClaim_Reference_Data_Model.md`: `expectedInputClassification`, dossier-level `ambiguityPolicy`, `frameRole`, `frameAggregationMode`, frame-scoped `atomicityProfile` including `determinabilityStatus`, truth-condition dominance roles/weights, required reference assertions, truth/confidence bands, source snapshots, and `validityWindow` for current-snapshot assertions.
- **P0b.2** Use the manual-alignment rubric and score artifact contract in the reference-data model: active-frame selection, frame-role fit, frame-aggregation-mode fit, assertion coverage, Stage-1-only atomicity fidelity under the declared aggregation mode, disclosure fidelity, C3 verdict/evidence equivalence, tie handling, and `needs_human_review`.
- **P0b.3 draft produced:** `Docs/AGENTS/Reference_Dossiers/bundesrat-rechtskraftig.v0.3.json` is a validator-clean full draft. It makes legal-force / entry-into-force the primary `rechtskräftig` route, records the Captain-confirmed 3-AtomicClaim C1 shape, records signed-text/authentication as an explicit secondary route, and keeps limited/provisional effects as guards/caveats rather than co-equal top-line scoring. It remains `status: draft` until independent curator/adjudicator/reviewer passes upgrade it.
...
- **P0b.5** Run manual alignment on stored reports first. Score per axis: input-classification fit, frame admissibility, frame-role fit, assertion coverage, dominance-weighted atomicity fidelity, and disclosure fidelity.
- **P0b.6** Instantiate strict-JSON C1 alignment and C3 evidence-equivalence judge prompts from the fixed reference-data judge output contracts; do not add labels, fields, or scoring rules, and do not run them until Captain approves a cap.
...
- **P1.8 built:** reference-dossier routing consumer. Linked dossiers provide the primary C4 top-line expected labels/truth/confidence bands through `benchmarkCoherence.topLineAssertionIds`; `alternativeTopLineRoutes` are exposed for frame-aware/manual review but are not selected by the legacy zero-spend scorer; `coverageGuardAssertionIds` and `contextAssertionIds` are exposed as diagnostic colour and are not averaged into C4. C1/C3 dossier alignment remains rank-ineligible until Phase 0b gates pass.
