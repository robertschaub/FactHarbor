# V2 Slice X7-W2-LS5 DIAG4 Taxonomy Live-Observation Package
...
**Parent package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-DIAG4_Node_Error_Category_Taxonomy_Source_Package.md`

## 1. Purpose

X7-W2-LS5 is a one-job diagnostic live observation after DIAG4. It must answer one narrow question:

```text
After DIAG4 taxonomy closure, does the same product-route W2 transport failure
map to one of the new bounded categories, or does it still land in other_known?
```

LS5 is not a provider-success gate, source-quality gate, report-quality gate, public-readiness gate, W2 completion-semantics repair, provider-portfolio decision, Wikimedia endorsement, raw-code probe, or V1-cleanup gate.

## 2. Authorization

After this docs-only package is committed and all pre-live checks pass, LS5 authorizes exactly one live job using the exact Captain-defined input in section 4.
...
- raw Node error-code exposure in product/admin artifacts, public responses, route JSON, durable logs, or committed runtime artifacts;
...
LS5 passes its diagnostic objective if:

- the job reaches `SUCCEEDED`;
- the first preparation event shows `pipeline: claimboundary-v2`;
- package/runtime provenance is clean and matches the committed LS5 package revision;
...
- every W2 network attempt includes bounded DIAG2/DIAG3/DIAG4 values:
  - `dnsAddressCount`;
...
`candidateProviderNetwork.status` may still be `candidate_provider_network_damaged_structural`. That remains a W2 provider-network completion failure, but LS5 can pass as a diagnostic observation if the classification evidence is captured and containment holds.
...
Use the same exact Captain-defined input as LS2, LS3, and LS4:
...
LS5 is docs-only before live execution. Allowed package files:

- `Docs/WIP/2026-05-18_V2_Slice_X7-W2-LS5_DIAG4_Taxonomy_Live_Observation_Package.md`
...
if ($beforeIdle) { throw "X7-W2-LS5 pre-submission worktree is dirty before idle checkpoint" }
Start-Sleep -Seconds 60
$afterIdle = git status --short --untracked-files=all
if ($afterIdle) { throw "X7-W2-LS5 pre-submission worktree became dirty during idle checkpoint" }
```
...
- any W2 network attempt lacks one of the DIAG2/DIAG3/DIAG4 fields;
- any DIAG2/DIAG3/DIAG4 field value is outside the approved enum/count contracts;
...
Do not repair inside LS5. Create a separate reviewed fix package if needed.
...
- all W2 DIAG2/DIAG3/DIAG4 field values by attempt;
- whether any attempt maps to `network_unreachable`, `host_unreachable`, `address_family_failure`, `tls_protocol`, or still `other_known`;
...
Review `Docs/WIP/2026-05-18_V2_Slice_X7-W2-LS5_DIAG4_Taxonomy_Live_Observation_Package.md`.

Return `approve`, `modify`, or `reject`.

Check whether LS5 is a safe one-job diagnostic live observation after DIAG4. Pay attention to:

- exact Captain-defined input choice and comparability to LS2/LS3/LS4;
...
- no retries or second canary inside LS5;
- hard-fail behavior on V1 fallback, hidden leaks, endpoint drift, missing DIAG2/DIAG3/DIAG4 fields, or any need for code/prompt/config edits.

## 15. Review Decision

| Role | Reviewer | Date | Decision | Notes |
|---|---|---:|---|---|
| Claude Opus 4.6 senior architect/security | Claude Opus 4.6 via `scripts/agents/invoke-claude.cjs` | 2026-05-18 | APPROVE | Reviewer confirmed LS5 is safe/useful after DIAG4, keeps exact input continuity with LS2/LS3/LS4, separates diagnostic pass from W2 provider completion, has comprehensive hard-fail criteria, preserves raw-code/privacy/leak controls, forbids repair/probes/provider expansion/source material/parser/cache/SR/storage/EvidenceCorpus/report/verdict/public behavior/V1 work/retries/second canary, and has correct package-commit/runtime-refresh/preflight sequencing. |
