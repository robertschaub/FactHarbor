...
Prepared parser provenance package: `Docs/WIP/2026-05-16_V2_Slice_7N3B3-2D-C0-S2_Parser_Admission_Provenance_Source_Package.md` is a docs-only package drafted after deputy consensus that C0-S2 source implementation should not start until a reviewed package exists. It proposes future process-local runtime-owned C0-S1 admission provenance only; it authorizes no source edits now and does not approve parser execution, worker spawn, byte consumption, packet/frame consumption, parsed material, source material, Evidence Lifecycle consumption, product/public/live wiring, cache/SR/storage, or 2D-C.
