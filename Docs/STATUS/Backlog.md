...
Current V2 source-acquisition state: 7A through X7-F plus C0-S1/C0-S2/C0-S3 are implementation-complete, and C0-S3A has hardened the audit-only gate register around the parser adjuncts. Detailed earlier source-acquisition and parser proof packages remain in their WIP packages and handoffs; the active direct-text chain is X5 hidden integration, X6 hidden test-injected candidate acquisition, X6-P in-process candidate-acquisition provenance, X7-A candidate-to-source-material readiness, X7-B source-material/evidence-corpus negative guard, X7-C provider-network readiness, X7-D hidden source-acquisition readiness composition, X7-E copied-X6 provenance gating, and X7-F hidden no-IO source-acquisition execution-denial gate. C0-S1 is a parser-path adjunct under `Docs/WIP/2026-05-16_V2_Slice_7N3B3-2D-C0-S1_P0_Parser_Worker_Admission_Source_Package.md`; it admits only structural fixture/control or synthetic inert P0 metadata while keeping parser execution blocked. C0-S2 adds process-local runtime-owned admission provenance, and C0-S3 consumes only that provenance to deny parsed-material creation. X7-F is implemented under `Docs/WIP/2026-05-16_V2_Slice_X7-F_Hidden_No_IO_Source_Acquisition_Execution_Gate_Source_Package.md`; it consumes only runtime-owned X7-E readiness composition and returns `gate_closed_no_io` with `research_acquisition_gateway_not_implemented`, or fails closed for blocked/copied/malformed/non-no-IO readiness. X3-A deliberately limits the executable state to hidden/internal query planning metadata and input-envelope scope. X4 does not approve public cutover. X5 does not approve product/orchestrator/runner/API/UI/report/export wiring. X6 does not approve product/orchestrator/runner/API/UI/report/export wiring or real source IO. X6-P, X7-E, X7-F, and C0-S1/C0-S2/C0-S3 do not approve source execution or durable provenance. X7-A, X7-B, X7-C, X7-D, X7-E, X7-F, and C0-S1/C0-S2/C0-S3 still do not approve source-material population, extraction, evidence-corpus building, provider-network execution, product/public wiring, live jobs, cache IO, Source Reliability, ACS/direct URL runtime dispatch, prompt edits, meaningful public result exposure, parser consumption of real fetched bytes, parsed-material creation, evidence/report generation, or V1 cleanup.

V2 gate register state: X8 is implemented under `Docs/WIP/2026-05-16_V2_Slice_X8_Source_Acquisition_Gate_Register_Refresh_Source_Package.md`, X7-F updates the current `gate.research_acquisition` audit anchor to the closed no-IO execution gate, C0-S1 adds the P0 parser-worker admission source package as tracked parser context, and the register now also tracks C0-S2 parser-admission provenance plus C0-S3 parsed-material denial as parser adjunct context. The register remains audit-only, does not approve execution, is not runtime-consumed, and keeps live jobs blocked. `scripts/validate-v2-gate-register.mjs` now self-tests that the row cannot silently drop X7-F, the B2 parser blocker, the C0-S1/C0-S2/C0-S3 parser package refs, `parser-worker execution`, `parsed-material creation`, `C0-S1`, `C0-S2`, `C0-S3`, `P0`, or the `2D-C remains blocked` note.

Next V2 action: X3-B prompt frontmatter/text alignment now has a reviewed docs-only approval package at `Docs/WIP/2026-05-17_V2_Slice_X3-B_Query_Planning_Prompt_Frontmatter_Text_Approval_Package.md`. It remains blocked until explicit Captain approval plus LLM Expert prompt approval/review and Architect scope acceptance; the package itself authorizes no prompt/source edits. X7-F is the latest direct-text hidden Evidence Lifecycle/source-acquisition state: it makes the readiness-to-execution boundary explicit and closed, but it still does not authorize product/public/live execution, real source IO, source-material population, extraction, or evidence-corpus building. Do not infer source execution, candidate acquisition, source material, or corpus building from X6-P, X7-D, X7-E, or X7-F. C0-S1/C0-S2/C0-S3 are the latest parser-path adjuncts: they create a P0 admission seam, prove runtime-owned admission provenance, and deny parsed-material creation, but they do not authorize parser execution, worker spawn, packet/frame consumption, fixture/control parsing, real fetched-byte parsing, parsed material, or 2D-C. Separately, 7N-3B3-2D-B2 remains the parser proof-only source slice: hidden structural isolation-proof contract, OCI proof owner, default `parser_isolation_unavailable`, approval checks, denial-probe result mapping, focused tests, and boundary guards. Docker and Podman are not available on this machine, so the positive sandbox verifier was not run locally and local proof remains fail-closed/unavailable. The next parser path action is not 2D-C source implementation; it is either a positive deployment-candidate proof on a provisioned rootless OCI host or a reviewed follow-up package that keeps 2D-C blocked until that proof exists. B2 and C0-S1/C0-S2/C0-S3 do not authorize live jobs, product/public cutover, parser consumption of real fetched bytes, fixture/control parsing, packet consumption, cache/SR integration, evidence/report generation, prompt/model/config/schema changes beyond approved gates, ACS/direct URL execution, or V1 cleanup. 2D-C parser work remains blocked until a positive deployment-candidate rootless OCI sandbox proof passes on a provisioned host. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a reviewed executable/live-smoke gate makes live jobs meaningful. `Plastic recycling is pointless` `blocked/no_valid_claim` remains a separate Claim Understanding policy-quality review item.
...
| **PHASE7B-1** | **Next bounded Shape B slice**: move from prompt/test hardening to explicit claim-to-anchor preservation mapping and validator evolution from discovery to audit, while preserving clear audit-vs-binding separation. | Analyzer / Prompt / Architecture | high | high | DEFERRED | LLM Expert / Senior Developer | Reactivate only if V1 maintenance is explicitly reopened; first refresh the Phase 7b charter to post-implementation state before prompt/source edits. | `Docs/WIP/2026-04-15_Phase7b_Prompt_Blocker_Implementation_Charter.md`, `Docs/WIP/2026-04-14_Phase7_Step1_Pains_Issues_Needs.md` |
...
| **ACS-1** | **Atomic Claim Selection v1**: pre-job draft/intake flow over the exact final `CBClaimUnderstanding.atomicClaims`, with prepared Stage-1 snapshots, draft confirm/restart flow, job-side selection provenance, interactive default, and automatic override. This is the first delivery surface for the new claim-selection track. | Analyzer / API / Web | med | high | [2026-04-22_Atomic_Claim_Selection_Implementation_Spec.md](../WIP/2026-04-22_Atomic_Claim_Selection_Implementation_Spec.md), `Docs/AGENTS/Handoffs/2026-04-22_Lead_Architect_Clarified_Order_ACS_CheckWorthiness_TopLevelProposition.md` |
| **ACS-CW-1** | **Check-worthiness recommendation inside ACS**: add the batched post-Gate-1 triage/ranking module over the final candidate set. In `automatic` mode it becomes the effective post-Stage-1 selection filter because `recommendedClaimIds` become `selectedClaimIds`, but it must not become Gate 1 authority. Do **not** ship this as a standalone pre-ACS external contract; pre-ACS work stays internal telemetry/prototyping only. | Analyzer / LLM / UX | med | high | [2026-04-22_Atomic_Claim_Selection_Implementation_Spec.md](../WIP/2026-04-22_Atomic_Claim_Selection_Implementation_Spec.md) §8.2, `Docs/AGENTS/Handoffs/2026-04-22_Lead_Architect_Debate_CheckWorthiness_Before_ACS.md` |
...
| **TOPPROP-1** | **`topLevelProposition` Phase A detection-only**: after ACS v1 and the recommendation layer land, add optional parent detection outside `atomicClaims`, validate `componentClaimIds` against the final accepted child set, and keep aggregation default-off until null-controls and approved benchmark families are stable. | Analyzer / Architecture | low | high | [2026-04-20_Dominant_Proposition_Architecture_Plan.md](../WIP/2026-04-20_Dominant_Proposition_Architecture_Plan.md) |
