...
**Last Updated**: 2026-09-13 (EGA-LINK now points to public working drafts)

**Ordering**: Rows are **grouped by related item family**, not strictly sorted — completed and closed rows sit next to the open work they came from, so their history stays readable. The **Urgency** and **Importance** columns are authoritative for prioritization; row position is not.

**Phase note**: **POC is complete** (tagged `v1.0.0-poc`). FactHarbor is in **Alpha phase**, with *engineering* **paused since 2026-07-02 for funding reasons** — but **production stays live**: `app.factharbor.ch` keeps serving and keeps accepting jobs (see `Docs/STATUS/Current_Status.md` §Why development is paused). Read the urgency column accordingly:

- **Development items**: urgency = order of work *on resume*. The separately authorized local F-35 repair and verification on 2026-09-09 did not restart the broader plan. Further work follows the [restart conditions](Current_Status.md#why-development-is-paused).
- **Operational and security items**: urgency = judged per item against *actual* exposure, which is low — the deployed alpha is invite-gated and very rarely used, so it accrues little per-job spend on its own. The residual risk is anomaly rather than baseline.

The older rule "security items are low urgency during Alpha, reclassify before production exposure" is **spent** — that exposure already happened. It has not been replaced by a blanket escalation: the maintainer has ruled on the two live-exposure items individually (`SEC-INVITE` declined, `OPS-MONITOR` deferred in favour of manual checks — see their rows). Provider-side spend caps and billing alerts carry that risk instead, and are tracked in `Current_Status.md` §Current Priorities rather than here.

**See Also**:
- `Docs/STATUS/Current_Status.md` for the current high-level snapshot
- `Docs/WIP/2026-06-18_Analysis_Quality_Consolidated_Execution_Plan.md` — the active engineering plan and its phase order

---

## Immediate Priorities (2026-08-10)

The active engineering track is the **[Analysis Quality Consolidated Execution Plan](../WIP/2026-06-18_Analysis_Quality_Consolidated_Execution_Plan.md)** (2026-06-18). Phases 0–3 and the Phase-4 scaffold are shipped; the Phase-4 LLM retrieval-language planner slice is unstarted. That plan supersedes the earlier Phase 7 / Shape B framing of this section — no active handoff in `Docs/AGENTS/Handoffs/` references Phase 7 or Shape B, and the Phase 7b charter itself now closes with "do not open a fresh prompt-implementation slice from this charter."

Two consequences for this table:

1. Rows whose *only* rationale was "next Phase 7 slice" are re-labelled below against what the code actually contains, not against the old plan.
2. Both production-runtime verifications (`F2-PROD`, Serper→P1 re-seed) are **resume-time items, not live ones** — production runs at roughly **1–2 jobs per week**, so neither has meaningful blast radius right now: a stale F2 blob would cost 1–2 imperfect reports a week, and the search-priority lever is volume-proportional (worth ~$170/month at 2026-05 development volume, far less at current rates). They matter as *prerequisites*: both are shipped changes a stale admin-owned DB blob silently ignores, so any later validation stacked on an unverified blob would be meaningless. Verify them when volume returns, before spending on validation.

| Item | Description | Domain | Urgency | Importance | Status | Notes |
|------|-------------|--------|---------|------------|--------|-------|
| **F35-FOLLOWUP** | **F-35 diagnosis review closed; follow-up deferred.** Local repair `e29773c71` and its verification are recorded; independent code review, deployment decision, clustering output ceiling, claim-contract schema failures and the remaining diagnosis findings are open. | Analyzer / Quality / Cost | on resume | high | DEFERRED — PAUSED | [Evidence and open items](../AGENTS/Handoffs/2026-09-09_LLM_Expert_F35_Fixed_Price_Retrieval_Diagnosis.md). Select any work within one budgeted cycle of the existing plan. No repeated wording experiment or new paid validation is authorized by this closure. |
...
| **PHASE7B-1** | **Anchor-preservation mapping / validator audit evolution**: NOT the active track and not an open implementation slice. Verified 2026-08-10 against code: `preservedByQuotes` is persisted (`claim-extraction-stage.ts:3497`, `:3932`), audit-vs-binding separation is live (`salienceCommitment.mode`, default `audit`; binding appendices gated in `claimboundary.prompt.md:446`, `:722`), anchor escalation to `binding_merged_anchor` exists (`claim-extraction-stage.ts:4249`), and `success=false` coverage exists in both `claim-contract-validation.test.ts` and `claim-extraction-prompt-contract.test.ts`. The charter's own executive decision is "do **not** open a fresh prompt-implementation slice from this charter." Residual is verification-only. The one substantive remainder — the Stage-1 truth-condition anchor-preservation *override* still being deterministic — is already tracked as `LLMINT-2` hotspot (1); do not double-track it here. | Analyzer / Prompt / Architecture | low | med | CLOSED-SUPERSEDED | `Docs/WIP/2026-04-15_Phase7b_Prompt_Blocker_Implementation_Charter.md` §1; superseded by `Docs/WIP/2026-06-18_Analysis_Quality_Consolidated_Execution_Plan.md` |
| **PHASE7B-OBS** | **Prompt-rollout hygiene (generic risk)**: the underlying risk is confirmed and permanent — runtime prompt resolution is DB-first/UCM-managed, not file-first, so a shipped prompt change can be silently inert per environment. Partially closed since 2026-04: prompt hashes are surfaced by `scripts/diag/current-build-failure-attribution.cjs`, and per-job prompt-hash provenance is persisted. What remains is the *generic* check discipline, now carried concretely by `F2-PROD` (prompt 1.0.12) and the Serper→P1 re-seed. Keep the row as the standing rule; track instances in the concrete items. The separate claim that persisted telemetry understates Stage-1 pruning / recovery provenance was **not** re-verified on 2026-08-10 — treat it as unconfirmed rather than as a known gap. | Analyzer / Config / Observability | med | high | STANDING | `Docs/AGENTS/Handoffs/2026-04-15_Unassigned_ClaimBoundary_Prompt_UCM_Propagation_Review.md`; instances: `F2-PROD` |
| **PHASE7B-MODEL** | **Dedicated salience model routing**: still true as written. Verified 2026-08-10: `TaskType` in `model-tiering.ts:20-26` has no salience entry, and `runSalienceCommitment()` calls `getModelForTask("understand", …)` (`claim-extraction-stage.ts:1712`), which maps to the **budget** tier (`DEFAULT_TASK_TIER_MAPPING`). Salience commitment is on by default in `audit` mode. If salience quality needs a stronger model, add isolated `modelSalience` / task routing instead of broadening `modelUnderstand` or reusing `context_refinement`. No evidence has been gathered that salience quality *is* the constraint, so this stays a design option, not a queued fix. | Analyzer / LLM Routing | low | med | DESIGN | `Docs/AGENTS/Handoffs/2026-04-15_Unassigned_Salience_Model_Tier_Design_Review.md` |
...
| **NEUTRALITY-1** | **Cross-linguistic neutrality gap**: Plastik recycling shows 58pp max spread across languages (DE 33% / EN 72% / FR 13%). Proposal 2 implementation is now shipped and review-clean: `LanguageIntent`, report-language threading, and an experimental default-off EN supplementary lane. Next gate is live A/B validation and a promotion decision. Mechanism reference: `Docs/ARCHITECTURE/Multilingual_Language_Handling.md`. | Quality / Policy / Analyzer | high | high | VALIDATION | `Docs/ARCHITECTURE/Multilingual_Language_Handling.md`, `Docs/ARCHIVE/2026-03-30_Report_Quality_Evolution_Deep_Analysis.md`, `Docs/ARCHIVE/Handoffs/2026-04/2026-04-01_LLM_Expert_Multilingual_Output_Search_Review_Validation_Status.md` |
...
| **DIR-1** | **Direction-integrity citation-carriage**: code fix shipped in `e1f2c551`. Citation arrays now carried through reconciliation. Remeasurement on post-fix stack pending. | Analyzer / Verdict | low | high | MONITOR | `Docs/ARCHIVE/2026-03-29_1bfb_Direction_Integrity_Architect_Review.md` |
...
| **NARR-1** | **Stage-5 Cross-Boundary Tension calibration**: first-pass cleanup is shipped and post-fix canaries are good enough to defer further narrative work. `08220154` wires the stale `${aggregation}` / `${evidenceSummary}` variables and tightens `boundaryDisagreements` to material directional divergence only; `2acc4545` closes the review follow-ups by counting unique `sourceUrl` values for `sourceCount` and adding a dedicated Stage-5 prompt-contract test. Swiss and misinformation-tools now sit at `0` tensions; remaining Bolsonaro / Plastik tensions look substantive. Keep Fix 2 only as fallback path B if future runs again show misclassified caveat noise. | Analyzer / Aggregation / Quality | low | high | MONITOR | `Docs/ARCHIVE/2026-04-06_Cross_Boundary_Tensions_Current_vs_Previous_Deployment_Investigation.md`, `Docs/ARCHIVE/2026-04-06_Cross_Boundary_Tension_Investigation.md` |
| **UPQ-1** | **Stage 2/3 upstream report quality stabilization**: Phase A (both sub-phases) and Phase B are complete; the strongest root cause was seeded-evidence dominance (addressed in Phase B, `442a5450`). Phase C (Stage 3 boundary-concentration stabilization) is confirmed **unstarted** — verified 2026-08-10 that boundary concentration is detection-only, emitting the `boundary_evidence_concentration` warning above a UCM threshold (`boundary-clustering-stage.ts:215-241`) with no stabilizing behavior. But it is no longer "the confirmed next step": the 2026-06-18 consolidated plan sequences Phase 4 (retrieval-language planning) and Phase 5 (D5/Stage-4 sufficiency) ahead of it, and its non-negotiables forbid starting D5-adjacent behavior work before stage isolation. Reopen only after plan Phases 4–5. | Analyzer / Quality | low | high | OPEN-DEFERRED | `Docs/WIP/2026-04-06_Upstream_Report_Quality_Workstream_Proposal.md` §Phase C, `Docs/WIP/2026-04-07_UPQ1_Phase_B_Canary_Measurement.md`; deferred by `Docs/WIP/2026-06-18_Analysis_Quality_Consolidated_Execution_Plan.md` |
...
| **METRICS-2** | **Anthropic standard-tier pricing parity**: was already done when this row was last written. `'claude-sonnet-4-6': { input: 3, output: 15 }` is present at `apps/web/src/lib/analyzer/metrics.ts:612`, landed in `8d0098d7` (2026-04-15). Cost estimates do not understate Anthropic standard usage. | Metrics / Observability | — | — | DONE | `8d0098d7`, `apps/web/src/lib/analyzer/metrics.ts:612` |
| **F2-CENSUS** | **F2 surgical-repair efficacy at volume**: F2 shipped `bb4de5f7` + atomicity extension `33ed6f74`, and is confirmed working on a single firing (job `b84ebbfe`, 2026-06-12 — collapsed 3 proxy-drift claims → 1, adopted, in band). The design claim is report_damaged 13%→~0, which one firing cannot establish. Re-run `scripts/diag/checkworthy-unverified-census.cjs` over accumulated jobs and read the `contract_surgical_repair_fired` / `_diagnostic` telemetry (fields `outcome`, `critiqueSource`, `claimsBefore/After`) for fire and rescue rates. | Analyzer / Quality / Measurement | med | high | OPEN | `Docs/AGENTS/Handoffs/2026-06-11_Lead_Developer_F2_Surgical_Contract_Repair.md` |
| **F2-PROD** | **Verify prompt 1.0.12 active in production**: F2 depends on the `CLAIM_CONTRACT_SURGICAL_REPAIR` section, and admin-owned prompt blobs do NOT auto-refresh from file. On a stale prod blob F2 silently no-ops — visible as `contract_surgical_repair_diagnostic` with `outcome=rejected`, reason "prompt section not found". Confirm the active prod claimboundary prompt is 1.0.12 via the Admin UI (or re-seed), same class of check as the pending Serper-priority config verification. **Urgency lowered to low 2026-08-10** (maintainer): production runs at roughly 1–2 jobs/week, so a stale blob costs 1–2 imperfect reports a week — not worth treating as urgent. It stays **high importance** as a prerequisite: it is a minutes-long Admin-UI check, and any paid validation run on an unverified blob is wasted spend. Do it before Phase 6, or whenever volume returns. | Config / Deployment | low | high | OPEN | Maintainer decision 2026-08-10; `Docs/AGENTS/Handoffs/2026-06-11_Lead_Developer_F2_Surgical_Contract_Repair.md` |
| **CI-DOTNET** | **No `dotnet test` in CI**: `.github/workflows/ci.yml` runs `npm run test:ci` plus `dotnet build`, so the .NET layer — auth (`AuthHelper`), invite-code claim/quota logic, rate limiting, runner client — has zero automated coverage in the gate. Add a `dotnet test` step; meaningful test cases can follow incrementally. (The JS safe suite is green: 1,986 passing as of 2026-08-06.) | CI / Testing | med | high | OPEN | 2026-06-10 advisory audit |
| **SEC-ADMIN-SR** | **`/admin/source-reliability` bypasses the admin login gate** (`apps/web/src/app/admin/layout.tsx:46`). Read-only and no paid calls, but inconsistent with the rest of the admin area. See KNOWN_ISSUES S2. | Web / Security | med | med | OPEN | `Docs/STATUS/KNOWN_ISSUES.md` §S2 |
| **SEC-INVITE** | **No invite-code brute-force lockout** — **DECLINED by the maintainer 2026-08-10; do not re-raise without new evidence.** The gap is real and stays documented in KNOWN_ISSUES S4: invalid codes return a distinguishable 404 with no failed-attempt tracking, and per-IP rate limiting slows probing without stopping it. What is accepted is the consequence — a guessed code buys analysis jobs at roughly $1 each. The judgement behind the decision: severity is cost amplification, not data exposure; the per-IP limiter caps the rate at 5 attempts/min/IP; and the tail is better bounded provider-side by a low monthly spend limit plus a billing alert than by building lockout logic. Reopen only on evidence of actual probing, or before open (non-invite) signup. | API / Security | — | med | DECLINED | Maintainer decision 2026-08-10; `Docs/STATUS/KNOWN_ISSUES.md` §S4 |
| **OPS-MONITOR** | **No uptime monitoring or error aggregation**: production failures are discovered when a user reports them. Corrected 2026-08-10 — the earlier "`MetricsCollector` exists but is not wired to the pipeline" rationale was **wrong**; per-job metrics are wired (`metrics-integration.ts` is imported by 10 analyzer/search modules) and `pipelineTelemetry` + `qualityHealth.d5` are persisted and aggregated by `/api/fh/metrics/summary` and `/api/fh/metrics/quality-health`. The real gap is external: no liveness probe and no cross-job error alerting. **Deferred by maintainer decision 2026-08-10**: not needed while paused — the maintainer will check manually when appropriate. That is the compensating control, and it is a deliberate choice, not a gap nobody noticed. Accepted consequence: between manual checks, an outage or a spend anomaly stays unnoticed until someone looks or a tester reports it. Justified by very low alpha traffic — there is little to be down *for*. The `/api/health` route already exists, so if the calculus changes (traffic returns, a paid pilot starts, or an anomaly actually occurs) a free-tier uptime ping is cheap to add. | Operations / Observability | low | high | DEFERRED | Maintainer decision 2026-08-10; 2026-06-10 advisory audit; `apps/web/src/app/api/health/route.ts` |
...
| **EGA-LINK** | **Evidence-Gated Agents proposed integration** — [user needs and requirements](https://github.com/robertschaub/our-ai-charter/blob/main/docs/wip/evidence-gated-agents-requirements.md) and [specification](https://github.com/robertschaub/our-ai-charter/blob/main/docs/wip/evidence-gated-agents-spec.md) describe the proposed isolated batch, baseline and bundle/export work alongside possible later extensions. These are working drafts, not an adopted implementation baseline. | Analyzer / Batch / Export | — | high | STANDING | If funded and separately activated; no restart of paused Alpha development or production work. |
...
| **EVD-1** | **Acceptable-variance policy** — **APPROVED**, operative Alpha-phase quality governance. 5 input classes (A–E) with green/amber/red bands. Analyzer work in monitor mode — new implementation only if a red threshold is breached. Revisit before Beta. | Quality / Governance | — | high | `Docs/ARCHIVE/2026-03-25_EVD1_Acceptable_Variance_Policy.md` |
...
| **EVACT-1** | **Minimum no-facilitator Estuarine map (concept gap)**: design the smallest Estuarine map that still changes a decision **without expert facilitation** — the sophistication-vs-adoption crux of the evidence-based action tools concept. The 3-tool design (Evidence Compass, Honest Inquiry Engine, Agent Provenance Contract) is now rich (actors, constraints, constructors, energy/time, counterfactual/volatile boundaries, intervention portfolios), which pulls against the concept's own adoption finding ("teammate, not watchdog"; completeness ≠ use). Concept-only — no build sequence is committed; the FactHarbor build stays gated on defer-and-dogfood (quality tripwires calibrated to the n=1550 UNVERIFIED census + a ≤2-founder-day no-UX internal probe). Related open items: KG1 demand and KG4 shared human/agent substrate, both unmeasured. | Product / Concept / Decision Support | low | med | `Docs/WIP/2026-07-01_Evidence_Based_Action_Tools_Product_Concept.md`, `Docs/AGENTS/Handoffs/2026-07-01_Product_Strategist_LLM_Expert_Evidence_Action_Debates_Wizard_Consolidation.md` |

---

## Recently Completed (June 2 – July 2, 2026)

Last code commit before the funding pause is `5c76e0e3` (2026-07-02). Commit hashes verified against git on 2026-08-10.

| Description | Domain | Completed | Reference |
|---|---|---|---|
| ✅ **F2 surgical per-claim contract repair**: replaced the all-or-nothing Stage-1 contract-gate abort — the root cause of `report_damaged` — with per-claim repair, plus claimboundary prompt 1.0.12, UCM knobs, and `contract_surgical_repair_fired` / `_diagnostic` telemetry. `33ed6f74` extends it to single-claim atomicity bundling. Live-validated on exactly one firing (job `b84ebbfe`, 2026-06-12: 3 proxy-drift claims collapsed to 1, adopted, in band). The design claim of `report_damaged` 13%→~0 is **unmeasured** — see `F2-CENSUS` — and the fix silently no-ops on a stale production prompt blob — see `F2-PROD`. | Analyzer / Stage 1 / Quality | 2026-06-11 | `bb4de5f7`, `33ed6f74`, `Docs/AGENTS/Handoffs/2026-06-11_Lead_Developer_F2_Surgical_Contract_Repair.md` |
| ✅ **Stage-4 citation publication contract**: verdicts must cite publishable evidence, repaired verdicts must carry plausible citations, and invalid challenge citations are separated rather than silently absorbed. Chain spans `ca143468` + `ac3b33da` (2026-05-30) and completes at `6b9c562f`. | Analyzer / Verdict | 2026-06-17 | `ca143468`, `ac3b33da`, `6b9c562f`, `Docs/AGENTS/Handoffs/2026-06-17_Senior_Developer_Stage4_Citation_Grounding_Fix.md` |
| ⚠️ **Cycle-1 selective prune reverted after gate failure**: the six Cycle-1 prune commits (`227c69ac`, `c5388aa6`, `29b58a0a`, `a4e3d1b0`, `68fd6925`, `350a867f`, 2026-06-12) were undone wholesale by `a56fed6f4` when the G1 gate failed. `main` was left green. Recorded as a failed attempt with baseline, per the Failed-Attempt Recovery rule; the ratified selective-prune approach itself is unchanged, and the non-negotiable "do not prune `claimboundary.prompt.md` before measurement and stage isolation exist" still stands. | Analyzer / Refactoring / Debt | 2026-06-17 | `a56fed6f4`, `Docs/WIP/2026-06-18_Analysis_Quality_Consolidated_Execution_Plan.md` §Non-Negotiables |
| ✅ **Phase-1 report-quality scorer (zero-spend)**: read-only scorer plus reference dossiers, manual alignment artifacts, a frame-dominance contract and frame-aggregation modes for the Captain benchmark families (Bundesrat, Bolsonaro, plastic, hydrogen, asylum). Signed off `0120fa0d`. | Quality / Measurement | 2026-06-05 | `7296f4a7` … `3b420f32`, `Docs/AGENTS/Handoffs/2026-06-05_Lead_Developer_Report_Quality_Phase0_Phase1_Scorer.md` |
| ✅ **Failure-attribution + stage-isolation fixtures (plan Phases 1–2)**: added `scripts/diag/current-build-failure-attribution.cjs` and `scripts/diag/export-stage-isolation-fixtures.cjs`, plus frozen fixtures (`phase2-current-failures.0396ea47.json`, set hash `b94b0b68…`) and the consuming D5 replay harness `analysis-stage-fixtures.test.ts`. All read-only; no live jobs. Phase 1 is **not** a behavior greenlight — every `6b9c562f` family/build cell is `n=1` low confidence, which blocks downstream behavior edits for those failure classes. | Measurement / Diagnostics | 2026-07-01 | `66ddc0cf`, plan §Phase 1 Status / §Phase 2 Fixture Rule And Status |
| ✅ **Default-off source-native supplementary lane scaffold (plan Phase 4 landing surface)**: `source_native` retrieval lane type, UCM-visible default-off `sourceNativeSupplementaryLane`, and shared supplementary-lane executor. Scaffold only — with default config it returns immediately; if enabled before a planner exists it records zero-count telemetry with `source_native:planner_unavailable` and performs no search, no query generation, no LLM call and no budget consumption. **The LLM planner behind it is unstarted** and is the next step of the paused plan. | Analyzer / Search / Multilingual | 2026-07-01 | `b72b5111`, `Docs/AGENTS/Handoffs/2026-06-18_Senior_Developer_Phase4_Source_Native_Scaffold.md`, plan §Phase 4 Status |
| ✅ **Plan Phase 3 ownership/schema fixes**: documented the split source-reliability prompt ownership (TypeScript-owned evaluation prompts in `sr-eval-prompts.ts` vs runtime-read `source-reliability.prompt.md` sections) and normalized input-policy-gate LLM output so generic snake_case `reasonCode` values survive and `messageKey: "unknown"` cannot reach the API response. | Analyzer / Prompt / Config | 2026-07-01 | `78890516`, plan §Phase 3 Status |
| ⚠️ **Stage2/D5 recovery chain classified as a failed attempt**: `0396ea47` (preserve research coverage before D5) shipped, but both Captain inputs returned `UNVERIFIED` 50/0. Baseline recorded; the plan's Phase-0 rule requires logging `ATTEMPT 2` against baseline `0396ea47` before the next behavior edit in this chain. | Analyzer / Quality | 2026-06-18 | `0396ea47`, `Docs/AGENTS/Handoffs/2026-06-18_Senior_Developer_Stage2_D5_Recovery.md` |
| ✅ **Longitudinal quality-over-time tooling**: per-branch quality attribution, build/commit-date x-axis (replacing job-submission time), production DB folded into coverage, reason-coded failure census by era. Established that `report_damaged` is 94% Stage-1 contract gate. | Measurement / Diagnostics | 2026-06-03 | `0811d9aa` … `a1b43c13`, `ccb526e2` |
| ✅ **Slim `/debt-guard` and `/doc-guard` skills restored** after a repair-drift pile-up recurred despite the auto-loaded AGENTS.md rule. | Process / Tooling | 2026-06-16 | `3fee2839`, `d6804bef`, `12f1e492` |
| ✅ **Harness environment leak fixed**: `restart-clean.ps1` strips the harness-injected `ANTHROPIC_BASE_URL`/`MODEL` that made every agent-started pipeline LLM call return 404. | Tooling / Reliability | 2026-06-11 | `3fc2b26b` |
| ✅ **Legal documents rewritten to match the live product**: the prior Privacy Policy and ToS described a community wiki and omitted all third-party LLM/search sub-processors. Both remain DRAFT with open `[NEEDS DECISION]` items. | Org / Legal | 2026-06-11 | `62f894a8`, `45df14fd` |
| ✅ **Dependency patching**: 7 npm vulnerabilities across the web app and the xWiki preview extension. | Security / Deps | 2026-06-11 | `beae2e16`, `064b7240` |
...
| Description | Domain | Completed | Reference |
|---|---|---|---|
| ✅ **Search priority swapped Serper→P1, Google CSE→free-tier fallback**: roughly a $170/month lever. Local DB live; **production re-seed still pending** — see Current_Status §Current Priorities. | Search / Cost / Config | 2026-06-01 | `362a9312` |
...
| ✅ **CB Architecture Design**: 9/9 decisions closed, 2 review rounds, 13 v1 features. | Architecture | 2026-02-16 | [Architecture Doc](../ARCHIVE/ClaimBoundary_Pipeline_Architecture_2026-02-15.md) |
...
| ✅ **UCM Phase 1: Analyzer Integration**: Analyzer now loads pipeline/search/calc from UCM (hot-reloadable). Env-based analysis settings removed. `LLM_PROVIDER` deprecated. | Architecture / Config | 2026-02-02 | [UCM Implementation Review](../ARCHIVE/Unified_Configuration_Management_Implementation_Review.md) |
...
| ✅ **UCM Phase 2: Job Config Snapshots**: `job_config_snapshots` capture pipeline/search + SR summary per job for auditability. | Architecture / Config | 2026-02-02 | [UCM Implementation Review](../ARCHIVE/Unified_Configuration_Management_Implementation_Review.md) |
| ✅ **UCM Phase 3: SR Modularity Interface**: SR config is separate UCM domain; SR service config wired and isolated from pipeline config. | Architecture / Config | 2026-02-02 | [UCM Implementation Review](../ARCHIVE/Unified_Configuration_Management_Implementation_Review.md) |
| ✅ **UCM Phase 4: Admin UI Polish**: Split routes to `/admin/quality/*` (FactHarbor) and `/admin/sr/*` (SR standalone). Added validation warnings for dangerous config combos and snapshot viewer. | Web UI / Config | 2026-02-02 | [UCM Implementation Review](../ARCHIVE/Unified_Configuration_Management_Implementation_Review.md) |
...
| ✅ **Terminology cleanup**: Context vs EvidenceScope naming cleanup in admin UI + schema (legacy keys supported). | Analyzer / Docs | 2026-02-02 | [Terminology Migration Summary](../ARCHIVE/Terminology_Migration_SUMMARY.md) |
...
| ✅ **Unified Configuration Management Foundation (v2.9.0)**: Extended config system to all core types (prompt, search, calculation, pipeline, sr) plus lexicons. Added prompt import/export/reseed APIs. 158 unit tests. Admin UI complete. | Architecture / Testing | 2026-01-30 | [Implementation Review](../ARCHIVE/Unified_Configuration_Management_Implementation_Review.md) |
| ✅ **LLM Text Analysis Pipeline**: Implemented 4 analysis points with hybrid LLM/heuristic architecture. Bug fix in v2.8.1 removed counter-claim detection from verdict prompt. | Analyzer / LLM | 2026-01-30 | [Deep Analysis](../ARCHIVE/LLM_Text_Analysis_Pipeline_Deep_Analysis.md) |
...
| **AtomicClaim extraction validation**: Validate with real LLM calls (target ~40% cost reduction). | Analyzer / Quality | low | high | `WIP/AtomicClaim_Extraction_Improvements_2026-02-17.md` (referenced file not present in repo or git history; verified 2026-08-10) |
...
- **Quick wins** — the original four are settled, so this list is retired. Quality Gate UI display shipped (`QualityGatesPanel`); invite-code access control and admin authentication shipped (one exempt route left, `SEC-ADMIN-SR`); parallel verdict generation was removed from the codebase rather than integrated (`apps/web/src/lib/analyzer/parallel-verdicts.ts` no longer exists — see KNOWN_ISSUES P1).

- **Cost optimization** — current state, verified 2026-08-10:
  - ✅ Budget defaults reduced: 25-40% savings (2026-02-13); `maxTotalIterations` later retuned 20→10
  - ❌ **Anthropic prompt caching is DISABLED on `main`**, not active: `getPromptCachingOptions()` returns `undefined` globally (`apps/web/src/lib/analyzer/llm.ts:394-398`), locked off by UCM default `anthropicPromptCachingEnabled: false`. Disabled in `b692ae17` (2026-05-26) after job `cf40490e` recorded zero cache reads and zero cache creations across 65 LLM calls / 366k input tokens. Do not re-propose it without new measurement.
  ...
  - Claim caching: 30-50% savings on repeat claims — not implemented; note that *result* caching is deliberately off during Alpha so per-run variance stays visible
  - See: [LLM API Cost Reduction & NPO Discounts](../WIP/2026-06-01_LLM_API_Cost_Reduction_and_NPO_Discounts.md)
...
- **Active plan**: `Docs/WIP/2026-06-18_Analysis_Quality_Consolidated_Execution_Plan.md`
- **Current Status**: `Docs/STATUS/Current_Status.md` · **Known issues**: `Docs/STATUS/KNOWN_ISSUES.md`
- **Detailed Analysis** (historical): `Docs/ARCHIVE/Improvement_Recommendations.md`
