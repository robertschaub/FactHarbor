...
| **MON-ACS-DIST** | Investigate selected-claim Stage 2 research distribution for ACS jobs: selected claims must not silently receive zero targeted Stage 2 iterations unless this is explicitly explained by protected budget behavior and surfaced in review output. | Analyzer / ACS / Stage 2 | high | high | ACTIVE | SVP ACS job `3a95e38c39ba4104b97e6cd0ff512b52` selected `AC_17`, but Stage 2 telemetry reported zero targeted iterations for that claim while contradiction ran. Treat as a pipeline-quality finding, not a tooling blocker. |
| **MON-STG1-QUAL** | Investigate Stage 1 quality only from concrete failing packets: omitted thesis branches, bundled consequences, one-claim collapse, and contract-preservation failures. | Analyzer / Stage 1 / Quality | high | high | ACTIVE | Bundesrat one-claim validator-context patch landed; independent status-plus-branch prompt fix added after duplicate-run verdict divergence; first live canary showed validator self-contradiction still forced retry, so validator-prompt repair is pending live validation. SVP PDF broad-input packet remains open. Prompt edits require Captain approval and topic-neutral wording. |
...
| **PROMPT-SPLIT-1** | **Prompt split / runtime prompt efficiency reopened as telemetry/design track**: the AGENTS large-file targeted-read exception landed and the original April 20 plan remains archived as the historical baseline. Current follow-up is telemetry first: prompt section/runtime metrics, cache-boundary diagnostics, retry-cause accounting, and manifest/composite-hash design before any physical split or prompt-behavior change. | Tooling / Prompt Governance | low | med | `Docs/WIP/2026-04-28_Prompt_Split_Runtime_Efficiency_Execution_Plan.md`, `Docs/ARCHIVE/2026-04-20_Prompt_Split_Plan.md` |

---

## Recently Completed (April 28, 2026)

| Description | Domain | Completed | Reference |
|---|---|---|---|
| ✅ **Atomic Claim Selection product path documented as implemented**: pre-job drafts, prepared Stage 1 reuse, selected-ID validation, interactive default, automatic mode, and residual quality findings now have a living xWiki architecture page. | Docs / ACS / Architecture | 2026-04-28 | `Docs/xwiki-pages/FactHarbor/Product Development/Specification/Architecture/Deep Dive/Atomic Claim Selection and Validation/WebHome.xwiki` |
| ✅ **ACS validation tooling thin waist implemented**: supported validation automation uses `apps/web/scripts/automatic-claim-selection.js`, emits schema v2 metadata, preserves historical direct-reference quality/status, and treats zero-iteration selected claims as reported pipeline findings. | Validation / ACS / Tooling | 2026-04-28 | `apps/web/scripts/automatic-claim-selection.js`, `scripts/validation/captain-approved-families.json` |
...
| ~~**ACS-1**~~ | **DONE - Atomic Claim Selection v1**: pre-job draft/intake flow over final Stage 1 `AtomicClaim` candidates, prepared Stage 1 snapshots, draft confirm/restart/cancel/retry, job-side selection provenance, interactive default, automatic override, and hidden/admin draft controls are implemented. Current living reference: `Docs/xwiki-pages/FactHarbor/Product Development/Specification/Architecture/Deep Dive/Atomic Claim Selection and Validation/WebHome.xwiki`. | Analyzer / API / Web | — | high | Implemented; remaining quality/performance follow-ups are tracked under `MON-STG1-LAT`, `MON-STG1-QUAL`, and `MON-ACS-DIST`. |
| ~~**ACS-CW-1**~~ | **DONE / MONITOR - Check-worthiness recommendation inside ACS**: the batched post-Gate-1 ACS recommendation/ranking module is implemented and can drive automatic selected IDs. Budget-aware recommendation metadata exists, but selected-claim research distribution remains under active monitoring before promoting automatic mode for routine product use. | Analyzer / LLM / UX | — | high | Current living reference: `Docs/xwiki-pages/FactHarbor/Product Development/Specification/Architecture/Deep Dive/Atomic Claim Selection and Validation/WebHome.xwiki`; follow-up: `MON-ACS-DIST`. |
