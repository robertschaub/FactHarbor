...
| **SEC-INVITE** | **No invite-code brute-force lockout**: invalid codes return a distinguishable 404 with no failed-attempt tracking; per-IP rate limiting slows but does not stop probing, and a guessed code costs real LLM spend (~$1/job). See KNOWN_ISSUES S4. | API / Security | med | med | OPEN | `Docs/STATUS/KNOWN_ISSUES.md` §S4 |
| **OPS-MONITOR** | **No uptime monitoring or error aggregation**: production failures are discovered when a user reports them. `MetricsCollector` exists but is not wired to the pipeline (see issue #2). A free-tier uptime ping on `/api/health` plus error aggregation would close the basic gap. | Operations / Observability | med | high | OPEN | 2026-06-10 advisory audit |
