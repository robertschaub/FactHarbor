...
Current V2 source-acquisition state: 7A through 7N-3B2 are committed. 7N-3A source-IO authority/runtime-owner contracts are implemented at `18f0fa02`; 7N-3B source-IO preflight gate is documented at `05387362`; 7N-3B1 hidden/internal candidate-acquisition runtime shell is implemented at `878ba6ba`, post-review hardened at `3d05583e`, and consolidated in `Docs/WIP/2026-05-16_V2_Slice_7N3B1_Post_Implementation_Consolidation.md`; 7N-3B2 hidden/internal SDK-free candidate-provider network boundary is implemented at `54b8af1a` under `Docs/WIP/2026-05-16_V2_Slice_7N3B2_Candidate_Provider_Network_Source_Package.md`. 7N-3B2 is deliberately limited to a private provider-network authority, raw-URL-free endpoint and budget snapshots, Node-core HTTPS transport, SSRF/final-address checks, redirect denial, streaming byte/decompression caps, timeout/abort handling, hidden diagnostics, and adapter wiring back into the injected 7N-3B1 provider boundary. It does not approve product/orchestrator/runner/API/UI/report/export wiring, live jobs, cache IO, Source Reliability, content packet dereference, parser execution, ACS/direct URL runtime dispatch, prompt/config/model/schema changes, public result exposure, or V1 cleanup.

Next V2 action: re-review `Docs/WIP/2026-05-16_V2_Slice_7N3B3-1_Content_Dereference_Source_Package.md` after initial deputy review returned `MODIFY` from security and test/ops/cost. The amended draft may approve only content-dereference authority, raw-URL-free target envelope, and content transport under exact file/test guards; it must not include parser/sink implementation, live jobs, product/public wiring, cache/SR integration, evidence/report generation, or V1 cleanup. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a reviewed executable/live-smoke gate makes live jobs meaningful. `Plastic recycling is pointless` `blocked/no_valid_claim` remains a separate Claim Understanding policy-quality review item.
