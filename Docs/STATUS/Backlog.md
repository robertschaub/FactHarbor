...
Current V2 source-acquisition state: 7A through X7-E are implementation-complete. Detailed earlier source-acquisition and parser proof packages remain in their WIP packages and handoffs; the active direct-text chain is X5 hidden integration, X6 hidden test-injected candidate acquisition, X6-P in-process candidate-acquisition provenance, X7-A candidate-to-source-material readiness, X7-B source-material/evidence-corpus negative guard, X7-C provider-network readiness, X7-D hidden source-acquisition readiness composition, and X7-E copied-X6 provenance gating. X7-E is implemented under `Docs/WIP/2026-05-16_V2_Slice_X7-E_Hidden_Source_Acquisition_Composition_X6_Provenance_Gate_Source_Package.md`; X7-D now consumes the X6-P sidecar reader and rejects copied/non-runtime-owned X6 inputs as `x6_not_runtime_owned`. X3-A deliberately limits the executable state to hidden/internal query planning metadata and input-envelope scope. X4 does not approve public cutover. X5 does not approve product/orchestrator/runner/API/UI/report/export wiring. X6 does not approve product/orchestrator/runner/API/UI/report/export wiring or real source IO. X6-P and X7-E do not approve source execution or durable provenance. X7-A, X7-B, X7-C, X7-D, and X7-E still do not approve source-material population, extraction, evidence-corpus building, provider-network execution, product/public wiring, live jobs, cache IO, Source Reliability, ACS/direct URL runtime dispatch, prompt edits, meaningful public result exposure, parser consumption of real fetched bytes, evidence/report generation, or V1 cleanup.

V2 gate register state: X8 is implemented under `Docs/WIP/2026-05-16_V2_Slice_X8_Source_Acquisition_Gate_Register_Refresh_Source_Package.md`. `Docs/AGENTS/V2_Gate_Register.json` now mirrors X7-E as the current `gate.research_acquisition` audit anchor and explicitly records B2/B3/C0 parser blockers. The register remains audit-only, does not approve execution, is not runtime-consumed, and keeps live jobs blocked. `scripts/validate-v2-gate-register.mjs` now self-tests that the row cannot silently drop X7-E, the B2 parser blocker, or the `2D-C remains blocked` note.

Next V2 action: X3-B prompt frontmatter/text alignment remains blocked until explicit Captain/LLM Expert prompt approval. X7-E is the latest direct-text hidden Evidence Lifecycle composition state: it updates X7-D to consume the X6-P reader and reject non-runtime-owned X6 inputs, but it still does not authorize product/public/live execution, real source IO, source-material population, extraction, or evidence-corpus building. Do not infer source execution, candidate acquisition, source material, or corpus building from X6-P, X7-D, or X7-E. Separately, 7N-3B3-2D-B2 remains the parser proof-only source slice: hidden structural isolation-proof contract, OCI proof owner, default `parser_isolation_unavailable`, approval checks, denial-probe result mapping, focused tests, and boundary guards. Docker and Podman are not available on this machine, so the positive sandbox verifier was not run locally and local proof remains fail-closed/unavailable. The next parser path action is not 2D-C source implementation; it is either a positive deployment-candidate proof on a provisioned rootless OCI host or a reviewed follow-up package that keeps 2D-C blocked until that proof exists. B2 does not authorize live jobs, product/public cutover, parser consumption of real fetched bytes, fixture/control parsing, packet consumption, cache/SR integration, evidence/report generation, prompt/model/config/schema changes beyond approved gates, ACS/direct URL execution, or V1 cleanup. 2D-C parser work remains blocked until a positive deployment-candidate rootless OCI sandbox proof passes on a provisioned host. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a reviewed executable/live-smoke gate makes live jobs meaningful. `Plastic recycling is pointless` `blocked/no_valid_claim` remains a separate Claim Understanding policy-quality review item.
