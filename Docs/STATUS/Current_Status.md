...
- **V2 implementation state:** 4C3b through 7N-3B2 are committed. The latest source slice is 7N-3B2 (`54b8af1a`): it adds only a hidden/internal candidate-provider network boundary with a private provider-network authority, structural raw-URL-free endpoint and budget snapshots, SDK-free Node-core HTTPS transport, SSRF/final-address checks, redirect denial, streaming byte/decompression caps, timeout/abort handling, hidden diagnostics, and an adapter back into the 7N-3B1 injected provider boundary. It does not add product/orchestrator/runner/API/UI/report/export wiring, live jobs, cache IO, Source Reliability, content packet dereference, parser execution, ACS/direct URL execution, prompt/config/model/schema changes, public result exposure, or V1 cleanup. The public result remains the damaged pre-cutover envelope.
- **Latest V2 gate:** 7N-3B2 is implementation-complete under `Docs/WIP/2026-05-16_V2_Slice_7N3B2_Candidate_Provider_Network_Source_Package.md`. Post-implementation deputy review returned `MODIFY`; blockers were fixed before `54b8af1a`: no raw parsed provider JSON in transport outcomes, decompression cap enforced during streaming inflate, DNS/provider calls timeout/abort bounded, and provider/query/total budgets enforced before dispatch.
- **Next V2 gate:** Re-review `Docs/WIP/2026-05-16_V2_Slice_7N3B3-1_Content_Dereference_Source_Package.md` after initial deputy review returned `MODIFY` from security and test/ops/cost. The amended draft may approve only content-dereference authority, raw-URL-free target envelope, and content transport under exact file/test guards; it must not include parser/sink implementation, live jobs, product/public wiring, cache/SR integration, evidence/report generation, or V1 cleanup. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a later reviewed live-smoke gate makes live jobs meaningful. Separately review the `Plastic recycling is pointless` `blocked/no_valid_claim` outcome before treating broad benchmark claims as quality evidence.
