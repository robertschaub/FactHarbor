...
- **Prepared X7-U1 Query Planning diagnostic live-smoke package:** `Docs/WIP/2026-05-17_V2_Slice_X7-U1_Query_Planning_Diagnostic_Live_Smoke_Package.md` is a docs-only execution gate for one exact German direct-text canary on committed X7-U0 diagnostics code. It is only meant to capture sanitized `adapterAttemptDiagnostics`; it does not authorize prompt/schema/config/model/provider repair, retries, source execution, public output, report/verdict/evidence behavior, ACS/direct URL, or V1 cleanup.
