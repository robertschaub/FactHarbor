...
- **V2 implementation state:** 4C3b through X4 are implementation-complete. The latest source slice is X4: V2 result contracts now carry `meta.publicCutoverStatus`, and TypeScript/C# public compatibility projections fail closed unless the schema is `4.0.0-cb` and the status is explicitly `approved`. Blocked V2 results still detect as V2 and surface primary issues, but public quick fields, legacy report surfaces, API list/detail quick fields, non-admin detail JSON, non-admin markdown, and HTML metadata no longer project V2 verdict/truth/confidence. X4 does not authorize public cutover; it prevents accidental exposure before cutover.
- **Latest V2 gate:** X4 is implementation-complete under `Docs/AGENTS/Handoffs/2026-05-16_Lead_Architect_V2_X4_Public_Cutover_Guard.md`. Reviewer consensus favored fail-closed null public metrics for all blocked V2 results, including the damaged pre-cutover shell. X3-B prompt frontmatter/text drift remains blocked and unresolved.
- **Next V2 gate:** X3-B prompt frontmatter/text alignment remains blocked until explicit Captain/LLM Expert prompt approval. If prompt approval is not provided, select the next implementation gate from the reviewed V2 action plan only after checking that it is still low-risk and does not imply live jobs, product/public cutover, parser consumption of real fetched bytes, cache/SR integration, evidence/report generation, prompt/model/config/schema changes beyond approved gates, ACS/direct URL execution, or V1 cleanup. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a later reviewed live-smoke gate makes live jobs meaningful. Separately review the `Plastic recycling is pointless` `blocked/no_valid_claim` outcome before treating broad benchmark claims as quality evidence.
