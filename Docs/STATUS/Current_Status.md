...
**Current X7-W4-H review package:** `Docs/WIP/2026-05-19_V2_Slice_X7-W4-H_Bounded_Extraction_Input_Authorization_Review_Package.md` is prepared for Steering Board review only. It proposes one hidden/admin-only bounded extraction-input packet derived only from the runtime-owned W4-G bounded text sidecar, with max text bytes inherited from W3-B/W4-G (`4096`). It authorizes no implementation and no live job. Default admin artifacts/routes must remain hash/length/provenance-only; retained internal packet text must not appear in public JSON, UI, reports, exports, compatibility projections, logs, errors, or default admin responses. Extraction execution, EvidenceItems, parser, LLM extraction calls, report/verdict/warning/confidence behavior, public behavior, cache/SR/storage, retries, provider expansion, W2/W3 widening, ACS/direct URL, V1 work, and V1 cleanup remain blocked. Captain reset/increased the next live-job tranche to `6`; the package recommends at most one later W4-H canary only after a separate accepted implementation package.
