...
**Current V2 default/manual UI state:** Commit `79a5c31f` makes `claimboundary-v2` the default manual submission pipeline through UCM defaults, web/API job and draft creation defaults, and runner missing-variant fallback. Explicit legacy `claimboundary` requests still run V1. Authenticated admin job pages can now render V2 report markdown in a `V2 Report` section when the internal V2 report writer produces it, while public/default result JSON remains `4.0.0-cb-precutover` / `blocked_precutover` / `report_damaged` with no public verdict/truth/confidence exposure. The active lane projection is `Docs/STATUS/V2_Current_Lane.md`; the next HighJump step is one HJ21/post-default canary from the default manual path after clean provenance/runtime preflight.
