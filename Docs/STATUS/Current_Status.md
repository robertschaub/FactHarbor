...
- **Prepared X7-W2-QC2 diagnostic source package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-QC2_Query_Planning_Distribution_Diagnostic_Source_Package.md` is the reviewed next implementation package. It allows one local Query Planning-only diagnostic harness plus boundary test to collect query-count evidence from Captain-defined inputs, while forbidding product route changes, live jobs, Source Acquisition/W2/provider-network/content/parser/cache/SR/storage/EvidenceCorpus/report/verdict/public behavior, prompt/config/schema/model/provider policy edits, ACS/direct URL, V1 work, and W2 cap changes.
