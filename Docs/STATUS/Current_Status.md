...
**Current X7-W4-G review package:** `Docs/WIP/2026-05-19_V2_Slice_X7-W4-G_Bounded_Corpus_Text_Authorization_Review_Package.md` is prepared for Steering Board review only. It proposes the first positive bounded corpus-text authorization: use the runtime-owned W3-B Wikimedia page-summary Source Material text as the only text parent, use W4-F/W4-C/W4-D/W4-E same-ledger state as corpus closure and extraction-denial evidence, then create one hidden/admin-only linked EvidenceCorpus bounded-text sidecar/record with the existing `4096` byte cap and no extraction. The W4-G admin artifact route must be hash/length/provenance-only by default; bounded text may be returned only behind an explicit authenticated admin inspection flag if included at all, and public result JSON, UI, reports, exports, compatibility projections, logs, and errors must never expose the text. W4-G must not supersede or mutate the W4-D shell or W4-E denial state. It authorizes no implementation, no live job, no extraction input, no EvidenceItems, no parser, no report/verdict/warning/confidence behavior, no public behavior, no cache/SR/storage, no retries, no provider expansion, no W2 endpoint migration, no W3-C widening, no full page/source/html fetch, no ACS/direct URL, and no V1 work or cleanup. The package recommends at most one later canary after implementation review, counted against the remaining `3`-job tranche.
