...
- **Approved X7-Q Claim Understanding language-metadata repair package:** `Docs/WIP/2026-05-17_V2_Slice_X7-Q_Claim_Understanding_Language_Metadata_Repair_Package.md` is a deputy-approved docs-only implementation package. It authorizes a narrow prompt/schema/test repair so accepted direct-text ClaimContracts cannot carry blank or `und` language metadata into downstream Query Planning prerequisites. Enforcement is global `ClaimContractSchema` first; X7-O and the Query Planning input envelope remain downstream defense-in-depth and should block raw invalid direct-input language as `claim_contract_invalid`. X7-Q does not authorize live jobs, Query Planning execution, source/provider/parser execution, public cutover, or V1 cleanup.
