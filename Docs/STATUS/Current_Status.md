...
**Phase**: **Alpha**
**Status**: V2 pipeline rebuild is the current strategic execution track. `Docs/STATUS/Backlog.md` is the authoritative active queue; this file is a high-level snapshot and historical context. V1 ClaimAssessmentBoundary work is maintenance-only until V2 cutover readiness. The V2 hidden direct-text runtime path remains product-reachable only through the approved triple gate. Evidence Lifecycle work has advanced through hidden Query Planning and Source Acquisition preparation into X7-W2: one product-internal, hidden candidate-provider network loop through the approved Wikimedia Core REST Search page-search endpoint, with bounded admin-only artifacts and cost/timing/outcome/byte telemetry. Broader live-job expansion, meaningful public V2 verdict exposure, source material, content dereference, parser consumption of real fetched bytes, cache IO, durable storage, Source Reliability integration, EvidenceCorpus/evidence/report/verdict/warning/confidence generation, ACS/direct URL runtime dispatch, prompt edits without explicit approval, public cutover, and V1 cleanup remain blocked by later reviewed gates.
**Current source-acquisition note:** X7-W2 is now the latest implemented product-route hidden Source Acquisition slice. It consumes accepted Query Planning handoff plus X7-V intake, X7-W1A admission, and the X7-W1B candidate-runtime prerequisite, then runs one product-owned hidden candidate-provider network loop through the 7N-3B2 boundary for the W2-approved Wikimedia Core REST Search page-search endpoint only. It records bounded admin-only artifacts with cost/timing/outcome/byte telemetry and no raw query/provider payload exposure. X7-W2 keeps source material, content dereference, EvidenceCorpus, parser/cache/SR/storage, evidence/report/verdict/warning/confidence generation, public output, ACS/direct URL, V1 reuse, and V1 cleanup blocked. X7-W2-LS1 is spent as a hard-fail/partial defensive observation; X7-W2-QC2 then measured accepted Query Planning query-entry counts of `2`, `5`, and `3` on three Captain-defined inputs. X7-W2-QC3 is implementation-complete at `c2fdcd9c`: W2 hidden provider-network admission is aligned to the reviewed Query Planning maximum of `6` entries with directly dependent `9000ms` total timeouts. X7-W2-LS2 has now run once and is classified as `HARD_FAIL_X7_W2_LS2_PROVIDER_NETWORK_DAMAGED_STRUCTURAL`: the job cleanly reached V2/W2 and public containment held, but W2 ended `candidate_provider_network_damaged_structural` with `candidate_runtime_query_coverage_invalid` and three sanitized `transport_failure` attempts. X7-W2-DIAG1 is docs-only complete, X7-W2-DIAG2 added bounded sanitized transport diagnostics, and X7-W2-DIAG3 added enum-only transport phase/error-shape/code-category diagnostics without raw URL/query/payload/IP/error leakage. X7-W2-LS4 has now run once and is classified as `PASS_X7_W2_LS4_DIAG3_TELEMETRY_CAPTURED`: the job cleanly reached W2 and captured DIAG2 plus DIAG3 fields on all three attempts, while W2 itself still failed structurally as `candidate_runtime_query_coverage_invalid` / `unknown_transport_failure` with `transportFailurePhase: unknown_phase`, `transportErrorShape: node_error_code_present`, and `nodeErrorCodeCategory: other_known`. Do not rerun LS2, LS3, or LS4. X7-W1C remains the pre-IO fence context; `research_acquisition` gateway policy remains `notImplemented`. C0-S1/C0-S2/C0-S3 are parser-path adjuncts only: they can admit structural P0 metadata, mark runtime-owned admission provenance, and deny parsed-material creation, but parser execution, parsed material, source material, Evidence Lifecycle consumption, product/public/live wiring, and 2D-C remain blocked.
**Current X7-W2 diagnostic addendum:** X7-W2-DIAG5 is implementation-complete as a narrow RP1-observed taxonomy mapping. Hidden source-acquisition network diagnostics now map the RP1-observed standard Node-style address-validation failure to `address_validation_failure` / `address_selection` / `node_error_code_present`, while W2 damaged/completion semantics and public pre-cutover output remain unchanged. The raw observed code remains confined to approved source/test literals; committed narrative docs and status use `[RAW_CODE_OBSERVED_LOCALLY_NOT_RECORDED]`. `Docs/WIP/2026-05-18_V2_Slice_X7-W2-LS6_DIAG5_Taxonomy_Live_Observation_Package.md` is Claude Opus-approved as the next one-job diagnostic live-observation package after DIAG5, but only after package commit, runtime refresh, endpoint re-check, route preflight, and verifiers. LS6 does not authorize endpoint/client repair, source material/content/parser/cache/SR/storage, EvidenceCorpus/evidence/report/verdict/warning/confidence/public behavior, provider expansion, ACS/direct URL, V1 work/cleanup, retries, prompt/config/model edits, or additional probes.
...
- **Prepared X7-W2-LS6 DIAG5 taxonomy live-observation package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-LS6_DIAG5_Taxonomy_Live_Observation_Package.md` is Claude Opus-approved as a docs-only one-job live-observation package after DIAG5. It uses the exact Captain-defined input `Using hydrogen for cars is more efficient than using electricity` to confirm whether the hidden W2 product-route failure now maps to `address_validation_failure` / `address_selection` instead of the old unknown category/phase. It authorizes no live job until package commit, runtime refresh, endpoint re-check, route preflight, and verifiers pass, and it authorizes no repair, provider expansion, source material/content/parser/cache/SR/storage, EvidenceCorpus/evidence/report/verdict/warning/confidence/public behavior, ACS/direct URL, V1 work/cleanup, retry, or second canary.
