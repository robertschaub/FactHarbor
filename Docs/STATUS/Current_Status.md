...
- **V2 implementation state:** 4C3b hidden direct-text source wiring, bounded hidden artifact sink, P1 product-reachability fix, API V2 variant admission, internal artifact inspection route, 4C3c smoke ledger-scope fix, 4C4 internal Claim Understanding stage handoff, 4C5 hidden adapter-attempt diagnostics (`4b36aab5`), 4C6 strict fenced-JSON output coercion (`57dc2308`), and 7A Evidence Lifecycle accepted-only intake (`08c7ddae`) are committed. The public result remains the damaged pre-cutover envelope.
- **Latest V2 gate:** 7A adds contract-only `buildEvidenceLifecycleIntake(...)`: accepted Claim Understanding handoffs with a non-null `ClaimContract` produce an internal `EvidenceLifecycleIntake`; blocked/damaged/malformed handoffs fail closed with no fabricated intake. No orchestrator wiring, provider/source work, prompt/model/schema/UCM change, cache IO, public exposure, ACS/direct URL execution, approval flip, or live job was added.
- **Next V2 gate:** prepare a reviewed Evidence Lifecycle source-acquisition/task-policy contract package before source work. Separately review the `Plastic recycling is pointless` `blocked/no_valid_claim` outcome before treating broad benchmark claims as quality evidence. Do not broaden live jobs, public exposure, cache IO, ACS/direct URL dispatch, prompt/config/model/schema changes, or V1 cleanup without a later reviewed gate.
