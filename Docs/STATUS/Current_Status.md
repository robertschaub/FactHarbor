...
- **Approved X7-R post-X7-Q live-smoke package:** `Docs/WIP/2026-05-17_V2_Slice_X7-R_Post_X7Q_X7O_Live_Smoke_Package.md` is a deputy-approved docs-only execution package for exactly one post-repair product-route job using the same Captain-defined legal-question input as X7-P. Objective: prove X7-Q lets X7-O observe structural prerequisites with `sourceLanguageSignal=present`, while public V2 remains damaged/precutover and hidden artifacts do not leak. It allows only the approved hidden Claim Understanding runtime/model call for that one job; Query Planning/source/provider/search/fetch/parser/cache/SR/report/verdict/public/V1 work remains blocked.
