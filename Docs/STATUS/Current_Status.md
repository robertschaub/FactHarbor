...
- **Prepared X7-W2-QC3 W2 cap-alignment source package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-QC3_W2_Cap_Alignment_Source_Package.md` is deputy-approved for implementation after Claude Opus, Security, and Performance/Cost review. QC3 may change only W2's hidden candidate-provider-network query cap from `2` to the reviewed Query Planning maximum `6`, and directly dependent total candidate/network timeouts from `3000ms` to `9000ms`. It must keep one provider, no credentials, per-query timeout `1500ms`, max candidates per query `3`, byte cap `32768`, retry policy `none`, redirect `deny`, proxy `none`, hidden/admin-only artifacts, no source material/content/parser/cache/SR/storage/EvidenceCorpus/evidence/report/verdict/warning/confidence/public behavior, no ACS/direct URL, no V1 work/cleanup, and no live jobs. A later LS2-style package is required before any live provider-network canary.
