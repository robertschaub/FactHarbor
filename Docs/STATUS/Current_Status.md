...
**Current W6-F1 repair result:** Superseding the stale W6-F1 repair-in-progress wording above, repair commit `7e3dafe8ce674765e77737e3b3aa007be02a7a06` ran one canary job `130bfc4c8be94fffadf780e7a0dd3f3f`. Classification is `PARTIAL_X7_W6_F1_OPENALEX_SOURCE_MATERIAL_MATERIALIZED_W4H_LINEAGE_BLOCKED`: public containment held, OpenAlex materialized through Source Material and W4-G bounded corpus-text sidecar, but W4-H blocked before an extraction-input packet because it still assumed Wikimedia-only/single-provider lineage. The current focused repair amends the existing W4-H/W4-I/W5 packet lineage contract to preserve multi-provider Source Material fan-in with hash/length/provenance default projections. The machine ledger now records `currentRemaining = 6` after the repair canary.
**Current W6-F1 W5E repair addendum:** Lineage repair commit `778be4b2403de8cfe60b0d21d72239c96180f772` ran canary job `0389a60644f749fb86208bb7d8e2c4ce`. W4-H produced one bounded extraction-input packet with three source-content packets and W5 accepted `4` hidden EvidenceItems with `schemaDiagnostics = null`, proving the OpenAlex/Wikimedia lineage repair through W5. W5E then blocked `lineage_mismatch` because it still validated against one scalar parent pair. The active local repair amends W5/W5E to carry and validate approved source-content packet identities, remains verifier-clean locally, and awaits focused commit/runtime refresh/canary. The machine ledger records `currentRemaining = 5` after job `0389a60644f749fb86208bb7d8e2c4ce`.
...
**Current W6-E / W6-F / W6-F1 state:** W6-E canary job `6a09d149d5d046cb95d0cdd67e02c095` is classified as `PASS_X7_W6_E_QUERY_BALANCED_SELECTION_VERIFIED_PROVIDER_DIVERSITY_REQUIRED`. Runtime `f1a45285f2d2b22fceb03d91007f9d24c47fdce8` selected W3-B records `SOURCE_CANDIDATE_PREVIEW_1_1`, `SOURCE_CANDIDATE_PREVIEW_2_1`, and `SOURCE_CANDIDATE_PREVIEW_3_1`, proving three distinct provider-attempt groups. W5 accepted `3` EvidenceItems and containment held, but W6-C still completed accepted with `schemaDiagnostics = null` and `reportStopRecommendation = refine_retrieval`. W6-F set the OpenAlex-first provider/source diversity direction, and W6-F1 is implemented as one bounded OpenAlex academic Source Material path plus the minimal closed two-endpoint/two-kind contract widening. W6-F1 canary job `3ec1c0c48ff84dd88484580967380320` did not materialize OpenAlex Source Material; source-material route showed three Wikimedia records, public leak checks stayed clean, and W8-B still reported `refine_retrieval`. That OpenAlex-not-materialized stop has been superseded by the W6-F1 authority-repair canary: OpenAlex materialized through Source Material and W4-G, then W4-H blocked on Wikimedia-only/single-provider lineage. The active local repair is the W4-H/W4-I/W5 multi-provider packet lineage amendment; the machine ledger currently records `currentRemaining = 6`. Do not weaken W6 prompts, relax W7 gates, add another provider, or rerun W6-F1 until the lineage repair is committed, runtime-refreshed, route/default-admin preflighted, and OpenAlex no-key posture is confirmed again.
