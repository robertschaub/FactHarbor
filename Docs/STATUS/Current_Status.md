...
**Current HighJump HJ15 implementation state:** Steer-Co consented to `Docs/WIP/2026-05-22_V2_HighJump_HJ15_OpenAlex_Source_Material_Fan_In_Repair.md` as an `incomplete-existing-mechanism` repair after HJ14 artifact triage. HJ15 is locally implemented and verifier-clean: the existing OpenAlex Source Material collector now continues through projected candidates, structurally de-duplicates by Source Material identity, skips invalid/no-abstract candidates without consuming slots, and stops at the existing cap. It adds no new provider, endpoint, prompt/model/config/schema edit, parser/full source/html fetch, retry, cache/SR/storage, public behavior, ACS/direct URL, V1 work, or V1 cleanup. Verifiers passed: focused Source Material tests (`3` files / `19` tests), full `analyzer-v2-runtime` (`74` files / `353` tests), full `analyzer-v2` (`142` files / `861` tests), V2 gate validation, gate-register self-test, debt sensors (`advisory_warn`), build, and diff check. Next action after commit/runtime refresh is exactly one HJ15 canary under explicit Steer-Co budget reconciliation; HighJump tranche remains exhausted and no second HJ15 canary is authorized.
