...
- **Two production config verifications are still pending**, both the same class of risk — a shipped change that a stale admin-owned DB blob silently ignores: the F2 prompt blob above, and the Serper→P1 search-priority re-seed (`362a9312`, 2026-06-01; local DB live, production outstanding). The first affects every job that runs; the second only pays off once search volume returns.
...
[Backlog.md](Backlog.md) is the canonical prioritized list. Priorities 1–4 apply **now** — they keep the paused state cheap, safe and reversible. Priorities 5 onward are the engineering sequence on resume:
...
2. **Cap the tail while unattended** — lower the provider monthly spend limit to something an alpha bill can justify, and add a billing alert. With usage this thin there is no legitimate volume to compare against and no monitoring watching (`OPS-MONITOR`), so an anomaly would run unnoticed: invite-code probing (`SEC-INVITE`), a drifted premium model pin, a runaway loop. A free uptime ping on `/api/health` closes the outage half cheaply.
...
4. **Verify prompt blob 1.0.12 is active in production** (`F2-PROD`) — an Admin-UI check, minutes of work. This is a correctness item, not a cost one: it does not matter how rarely the app is used, only that the jobs which do run get the F2 repair rather than a `report_damaged` abort.
