...
**Current X7-W3-A implementation review package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W3-A_Safe_Locator_Materialization_Preview_Source_Package.md` is prepared for Steering Board review only. It narrows W3-A to Tier 0 safe locator materialization from provider-owned Wikimedia search candidates already present during W2 response handling, using bounded hidden/admin-only source-candidate preview diagnostics. It proposes a projection-hook architecture so raw provider candidate objects do not cross runtime boundaries, forbids raw page keys/URLs/provider JSON, and keeps `extraHttpCall: false` plus `sourceMaterialCreated: false`. It authorizes no implementation and no live job. W3-B page-summary fetch, Source Material records, parser, EvidenceCorpus, EvidenceItems, report/verdict/warning/confidence, public exposure, second provider, retries, cache/SR/storage, ACS/direct URL, V1 work, and V1 cleanup remain blocked.
