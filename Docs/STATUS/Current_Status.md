...
**Phase**: **Alpha**
**Status**: V2 pipeline rebuild is the current strategic execution track. `Docs/STATUS/Backlog.md` is the authoritative active queue; this file is a high-level snapshot and historical context. V1 ClaimAssessmentBoundary work is maintenance-only until V2 cutover readiness. The V2 hidden direct-text runtime path has advanced through accepted Claim Understanding, Query Planning, bounded Source Acquisition, Source Material, corpus/readiness gates, W5 EvidenceItem extraction/admission/handoff, X7-W5-H retirement of the standalone W4-I route, W6-B contract-only sufficiency intake, W6-C hidden/internal sufficiency assessment implementation at `8f7856b5`, W7-A contract-only Boundary/Verdict Candidate implementation at `00bd1674`, W8-A internal Alpha report stop candidate implementation at `63f1042a`, W7-B hidden/internal Boundary/Verdict LLM execution at `94b171b7`, W6-C2 product runtime ownership at `ca0a96fa`, W7-B2 product runtime ownership at `ae7b681b`, W7-C product chain integration at `1bba8d8f`, W8-B internal Alpha report-result observability at `f468a73d`, W8-C post-W8B convergence implemented at `9dca1af8`, W8-D product-route canary result `STOP_X7_W8_D_INTERNAL_ALPHA_RESULT_BLOCKED_BY_SUFFICIENCY_ASSESSMENT` on job `828aacb298ed43a885b8d2f41379f25e`, W8-E implementation committed at `dfe430b5`, W8-F product-route attribution canary passed on job `5a9f11c1b3e34be18b6bf49ed6fc4d65`, and W6-F1 OpenAlex bounded academic Source Material diversity is implemented under `Docs/WIP/2026-05-21_V2_Slice_W6-F1_OpenAlex_Bounded_Academic_Source_Material_Diversity_Review_Package.md`. Public V2 remains intentionally blocked/precutover/report_damaged. The first W6-F1 canary job `3ec1c0c48ff84dd88484580967380320` ran on runtime `731ef0e595c59f678e4d50f461c1ce6ca8cb9715` and is classified `STOP_X7_W6_F1_OPENALEX_NOT_MATERIALIZED_REFINEMENT_REMAINS`: containment held, but Source Material stayed Wikimedia-only and W6-C still recommended `refine_retrieval`. That first W6-F1 stop has been superseded by the W6-F1 authority-repair canary and the current W4-H multi-provider lineage repair state below; the machine live-job ledger is the current budget authority.
**Current V2 default/manual UI state:** Commit `79a5c31f` makes `claimboundary-v2` the default manual submission pipeline through UCM defaults, web/API job and draft creation defaults, and runner missing-variant fallback. Commit `2e12b254` also makes the preparation page show the active draft pipeline (`Pipeline V2` / `Pipeline V1` / default) and improves V2 report-writer citation rendering. Explicit legacy `claimboundary` requests still run V1. HJ24 stronger report gauntlet now has two complete hidden/internal V2 report-writer drafts: German asylum job `b943c31f416941c9b46887e5b996c901` on runtime `366dce54` and hydrogen job `0a769c00825e48e5933cb1b1286b85c1` on runtime `beb23bca`. Public/default containment held. The active open gap is Bolsonaro-family generalization: after W3-B/W4-A repairs it reaches W5 but still returns `hidden_no_extractable_evidence`. Captain reset/extended the live-job budget to `18`; HJ24 consumed `7`, leaving `11`.
**Current HighJump bar-ladder plan:** `Docs/WIP/2026-05-22_V2_HighJump_Bar_Ladder_Plan.md` remains the active core-plan addendum for the current V2 HighJump phase. Captain authorized topic-neutral prompt edits, schema changes naturally needed to complete the plan, and a fresh `12` live-job tranche after HJ73. HJ74 consumed `1`, so `11` remain. The working rule is to lower only report-blocking bars enough to obtain complete internal V2 report results across Captain-defined inputs, then raise bars from observed report evidence. Public V2 remains `4.0.0-cb-precutover` / `blocked_precutover` / `report_damaged`; V1 cleanup remains blocked.
**Current HighJump latest result:** HJ74 canary job `b5dc2c0d4f3e47a6aa2bd82ff3c617e5` ran on runtime `7f24b11905bf9e945934ee634fd5aa7d0c6f7520` and is classified `PARTIAL_PASS_X7_HJ74_ATTRIBUTION_CAPTURED_SOURCE_MATERIAL_GAP_REMAINS`. It stayed on `claimboundary-v2`, finished `SUCCEEDED`, and preserved public/default containment. Admin raw output included HJ73 source-chain attribution: Query Planning accepted `4` entries, candidate provider network completed, Source Material produced `7` records, W5 extracted `4` EvidenceItems, and the internal report writer created a draft. The report remained `UNVERIFIED` because supplied evidence still lacked a comprehensive current asylum-domain stock count for the `235000` threshold. Result artifact: `Docs/WIP/canary-evidence-hj74-source-chain-attribution.json`. HJ75 implementation commit `522beebb` now amends the existing Source Material selector to prefer already-materialized Serper source-native records (XLSX, linked-page text) over preview fallback under existing caps. First HJ75 submission `bdde6d4ad58544bcbf07576c7cf89968` is `INVALID_X7_HJ75_RUNTIME_AUTH_TRIGGER_MISS`: it failed before analyzer execution because API-to-Web `/api/internal/run-job` returned `401`, so no Source Material/report evidence exists. The job is counted against the live tranche; `10` jobs remain. Runtime auth preflight is now fixed, and Steer-Co consented to exactly one replacement HJ75 canary after this invalid-attempt documentation commit and fresh health/auth/provenance preflight.
**Historical HighJump report result:** HJ8 repaired canary job `80e156a93d8442169166ec8b1a951231` ran on runtime `955c65917fe55f300762004473f72e1996307daf` and is classified `PASS_X7_HJ8_INTERNAL_ALPHA_REPORT_DRAFT_CREATED`. The initial W8-G canary job `17e3f40dda2f42749e5528742a497731` is recorded honestly as `STOP_X7_HJ8_W8G_BLOCKED_BY_W7B_RUNTIME_OWNERSHIP_ALLOWLIST_DRIFT`; repair commit `955c6591` amended the existing W7-B runtime ownership guard to include `internalReviewPayload`. The repaired hidden chain reached W8-B `internal_alpha_report_result_candidate_created` with W7-B runtime ownership `owned`, `firstIncompleteStage = none`, `3` boundary candidates, `2` verdict candidates, and `3` cited EvidenceItem refs. W8-G then created `internal_alpha_report_draft_created` with a `5420` byte draft available only through explicit authenticated inspection; the default admin projection stayed hash/length/provenance-only, unauthenticated route access returned `401`, and public V2 stayed `4.0.0-cb-precutover` / `blocked_precutover` / `report_damaged` with no draft leak.
**Current HighJump HJ9-HJ14 addendum:** HJ9 remains `PARTIAL_X7_HJ9_COMPARATOR_PROMPT_REPAIR_OFF_COMPARATOR_TOP_VERDICT_REMAINS`, and HJ10 remains `PASS_X7_HJ10_CLAIM_CONTEXT_REPAIR_NO_OFF_COMPARATOR_TRUE_VERDICT_INTERNAL_ALPHA_UNVERIFIED`. HJ11 source-coverage repair commit `bf1f0011898956bb2efabd2044cfce9be30defb5` carried `9` hidden Source Material records into W4/W5, including OpenAlex plus Wikimedia, but corrective V2 canary `751c0cb864924ec1a2cbe697730a7b70` stopped at W5 `schema_validation_failed` on `integrityEvents`. HJ12 prompt-contract repair commit `40207da59a521041879617f94c349162eb86a273` fixed that event-contract drift: canary `4dfbcd7627414738b0216d200df550c4` ran `claimboundary-v2`, W5 extracted `6` hidden EvidenceItems, and W8-G created a `5387` byte internal Alpha draft. HJ13 repair commit `9d1591154060d46f391aed66f9c713b8002fef2c` then ran canary job `1d85ff88bf6945cb8f7caefcbabc7d9c` and is classified `STOP_X7_HJ13_OPENALEX_BALANCE_NOT_REALIZED_INTERNAL_ALPHA_DRAFT_CREATED`: hidden V2 reached W8-G and created an `8966` byte internal Alpha draft with `4` boundary drafts, `3` verdict drafts, and `4` cited EvidenceItem refs, but Source Material still contained only `1` OpenAlex and `8` Wikimedia records. HJ14 repair commit `92cbc14fd53665e07a80e239b2e1ec6e190be3df` ran canary job `959c0246501c44558cbf8f484f9b6e3b` and is classified `STOP_X7_HJ14_WIKIMEDIA_TEXTEXTRACTS_NO_MEASURABLE_DEPTH_GAIN_INTERNAL_ALPHA_DRAFT_CREATED`: hidden V2 reached W8-G and created a `7908` byte internal Alpha draft with `4` boundary drafts, `3` verdict drafts, and `5` cited EvidenceItem refs, but the W4-A Source Material byte distribution was unchanged from HJ13 (`1` OpenAlex record, `8` Wikimedia records, `3577` total Wikimedia bytes). Public V2 remains blocked/precutover/damaged. The HJ14 exception canary is consumed; do not run another same-provider Wikimedia depth tweak without a new Steer-Co direction.
**Current HighJump HJ15 result:** HJ15 implementation commit `c1bfba1d57dc36b15a1200288246a45faea31fdc` amended only the existing OpenAlex Source Material collector to fan in unique valid OpenAlex abstracts. Canary job `42e9c2a6ce2a4bb5bd551900db230249` ran on runtime `c1bfba1d57dc36b15a1200288246a45faea31fdc` and is classified `PARTIAL_X7_HJ15_OPENALEX_FAN_IN_IMPROVED_W4G_AGGREGATE_CAP_STOP`. Public V2 stayed blocked/precutover/damaged and containment held. W4-A Source Material improved to `3` OpenAlex records (`5512` bytes total) plus `6` Wikimedia records (`7224` bytes total), but W4-G blocked as `blocked_pre_bounded_corpus_text_oversized` because the existing aggregate bounded-text cap is `12288` bytes and the HJ15 material set was `12736` bytes. W4-H then blocked as `blocked_pre_extraction_input_w4g_not_positive`, W5 blocked pre-execution with `w4h_packet_invalid`, and W8-B/W8-G were not created. The standalone W3-B route still returned `404` for the ledger while W4-A/W4-F carried W3-B state downstream; treat this as inspection-coverage caveat. HJ15 consumed exception overrun `5`; no second HJ15 canary is authorized. Next action is Steer-Co consent for the smallest balanced W4-G/W4-H aggregate handling repair; do not add a new provider, source route, readiness layer, or report-prose patch for this stop.
**Current HighJump HJ16 result:** HJ16 implementation commit `06865c3fde5d0432e3a38afd91d4a7645d4c07e3` raised only the existing shared W4-G/W4-H aggregate bounded-text cap from `12288` to `16384` bytes. Canary job `801450beed9b4de184f1a5ae532f00dd` ran on API/Web runtime `06865c3fde5d0432e3a38afd91d4a7645d4c07e3` and is classified `PARTIAL_X7_HJ16_AGGREGATE_CAP_RECALIBRATED_W5_PARSE_FAILURE`. Public V2 stayed blocked/precutover/damaged and containment held. HJ16 closed the cap stop: W4-A admitted `9` Source Material records (`3` OpenAlex / `4235` bytes and `6` Wikimedia / `6860` bytes), and W4-H created one aggregate extraction-input packet with `11111` bytes under the new cap. The new downstream stop is W5 `damaged_execution` / `parse_failure` with schema diagnostic `json_parse_error` and token usage `8253` input, `4000` output, `12253` total. W5E/W5-F blocked because W5 was not accepted, and W8-B/W8-G were not created. HJ16 consumed exception overrun `6`; no second HJ16 canary is authorized. Next action is Steer-Co selection of the smallest balanced W5 parse/truncation repair; do not change source acquisition, aggregate caps, provider selection, public behavior, parser, cache/SR/storage, ACS/direct URL, V1 work, or V1 cleanup for this stop.
**Current HighJump HJ17 result:** HJ17 implementation commit `c63f8a90ce38f419c458c5f4e6cc1d05ec150edc` amended only the existing W5 `evidence_extraction` output budget from `4000` to `8000` and added one topic-neutral compactness instruction to `V2_EVIDENCE_EXTRACTION`; Query Planning and W7-B output budgets remain unchanged. Canary job `ec943a2eeedf41a5890c7a0222286f50` ran on API/Web runtime `c63f8a90ce38f419c458c5f4e6cc1d05ec150edc` and is classified `PASS_X7_HJ17_W5_OUTPUT_BUDGET_REPAIR_INTERNAL_ALPHA_DRAFT_CREATED`. Public V2 stayed blocked/precutover/damaged and containment held. Hidden W5 completed as `hidden_evidence_item_extraction_completed` with `extractionResultStatus = accepted`, `extractionStatus = evidence_extracted`, `3` EvidenceItems, and token usage `8313` input / `1588` output / `9901` total, well below the `8000` output-token ceiling. W8-B created an internal Alpha report result candidate with `firstIncompleteStage = none`, `3` boundary candidates, `2` verdict candidates, and `3` cited EvidenceItem refs; W8-G created a `5413` byte internal Alpha draft available only through explicit authenticated inspection. HJ17 consumed exception overrun `7`; no second HJ17 canary is authorized. Next action is Steer-Co report-quality steering over the internal Alpha draft, not another W5 output-budget repair.
**Historical HighJump HJ18/HJ19 result:** HJ18 implementation commit `ceb6a38395f2a36bc3eeeeba8ab2818524222013` added the first hidden/internal `aggregation_narrative` LLM report-writer path over accepted W8-B/W7-B state. Canary job `c75322fad1e74218b8ee51a54f2307cd` ran on runtime `44395adc13a9b28faa7bec862dafda52682805ed` and is classified `STOP_X7_HJ18_INTERNAL_REPORT_WRITER_PARSE_FAILURE_CONTAINED`: the product route reached HJ18, default admin projection stayed hash/length/provenance-only, public V2 stayed blocked/precutover/damaged, but the report-writer result was `internal_report_writer_damaged` / `parse_failure` after `4000` output tokens and produced no report markdown. HJ19 implemented the smallest in-place repair: HJ19 approval provenance, `aggregation_narrative` model/cache provenance, `maxOutputTokens = 8000`, and topic-neutral compactness guidance. HJ19 canary job `7522df8a2f1647adb80d230efcfafe40` ran on runtime `4e2ed0982d0eecfb8cd5e0098bca0c8342611e77` and is classified `STOP_X7_HJ19_PRE_REPORT_WRITER_W5_EVIDENCE_ITEMS_TOO_BIG`: it reached CU/Query Planning/W2/W5, but stopped before W8-B/HJ19 because W5 failed closed as `damaged_execution` / `schema_validation_failed` with schema issue path `evidenceItems`, code `too_big`. Public V2 stayed blocked/precutover/damaged. Historical tranche remaining was `6`; no second HJ19 canary is authorized. The historical next action was HJ20 W5 output shaping, not another report-writer canary, source/provider expansion, public projection, retry, schema relaxation, parser/cache/SR/storage, ACS/direct URL, or V1 work.
**Historical HighJump HJ20 result:** HJ20 is implemented at `561f65d865f037f1a81b75dd9a2514a5cd988561` as a W5 prompt-only output-shaping repair. It amends only `V2_EVIDENCE_EXTRACTION` with topic-neutral guidance to normally return `2` to `5` complete, materially distinct EvidenceItems and not more than `5` for one HighJump extraction packet. First HJ20 canary job `8fe16cdeef7842058a8a36337a41b82e` is `UNEVALUATED_X7_HJ20_HIDDEN_ARTIFACT_CAPTURE_ROUTE_READINESS_MISS` because hidden artifact capture returned app-level HTML `404` and services stopped before process-local ledgers could be read. The Steer-Co/Claude-approved evaluability rerun job `53f22512b9aa41b5ab23b774e2ddf10f` ran on runtime `a7a73479d62779ad7b22868898fb50d0d09634c6` and is classified `PASS_X7_HJ20_W5_OUTPUT_SHAPING_INTERNAL_REPORT_WRITER_DRAFT_CREATED`: W5 accepted `4` EvidenceItems with schema diagnostics `null`, W8-B created an internal Alpha result candidate with `3` boundary candidates / `2` verdict candidates / `4` cited EvidenceItem refs, W8-G created a `7843` byte internal Alpha draft, and HJ19 internal report writer created an `8759` byte hidden report draft. Public V2 stayed blocked/precutover/damaged, default admin projections stayed hash/length/provenance-only, and public `reportMarkdown` stayed empty. Historical tranche remaining was `4`; no second HJ20 canary is authorized. The historical next action was internal report-quality review over the hidden draft, not another readiness/proof/plumbing layer.

**Current W6-F1 final repair result:** W6-F1 has reached the requested stable handoff point. The earlier OpenAlex-not-materialized canary `3ec1c0c48ff84dd88484580967380320`, authority-repair canary `130bfc4c8be94fffadf780e7a0dd3f3f`, and lineage-repair canary `0389a60644f749fb86208bb7d8e2c4ce` are superseded by W5E repair commit `7f5fe959b2a9c60b3ee86a0d69bc3ad6cee29b37` and canary job `2c60d8e749514f0d84e1158ae7dc9354`. Classification is `PASS_X7_W6_F1_W5E_MULTI_SOURCE_ADMISSION_REPAIR_VERIFIED_RETRIEVAL_REFINEMENT_REMAINS`: public V2 stayed damaged/precutover, Source Material contained one OpenAlex and two Wikimedia records, W4-H carried three source-content packets, W5 accepted `3` EvidenceItems, W5E admitted them without block/damage reason, and W5-F handoff was ready. W6-C completed accepted with `schemaDiagnostics = null` but still recommended `refine_retrieval`; W8-B/W8-F still stopped at `boundary_verdict_candidate_not_ready`. W6-F1 budget accounting is historical and superseded by the current HighJump tranche. No second W6-F1/W5E canary is authorized.
**Current W6-C2 implementation state:** W6-C2 is locally implemented and verifier-clean as one bounded product-owned sufficiency runtime owner. It wraps existing W6-C core sufficiency assessment execution, validates hidden direct-text activation and frozen `evidence_sufficiency` model policy, calls the provider only through the approved owner, marks returned decisions runtime-owned, and keeps evidence-scope/provenance text out of the provider prompt by projecting those fields as hashes and byte lengths. It does not wire the product orchestrator, W7-B, a chain artifact, a route, a sink, public/default-admin projection, live jobs, prompt/model/config/schema/UCM/gateway edits, report/verdict/warning/confidence behavior, parser/cache/SR/storage/provider expansion, ACS/direct URL, V1 work, or V1 cleanup. Next bounded step remains a separate W7-B2 product runtime owner package before any chain integration or canary.
**Current W7-B2 implementation state:** W7-B2 is locally implemented and verifier-clean as one bounded product-owned boundary/verdict execution runtime owner. It wraps existing W7-B core `BoundaryVerdictExecutionDecision`, requires runtime-owned W5 and W6-C parents, validates hidden direct-text activation and frozen `boundary_verdict_execution` model policy, loads only the existing approved `V2_BOUNDARY_VERDICT_EXECUTION` prompt section, calls Anthropic with `maxRetries: 0`, sanitizes provider failures, and marks returned W7-B decisions runtime-owned. No live job was run. It authorizes no product orchestrator wiring, W6-C2-to-W7-B chain integration, route/sink/artifact creation, W8-B report wrapper, public behavior, prompt/model/config/schema/UCM/gateway edit, parser/cache/SR/storage/provider expansion, ACS/direct URL, V1 work, or V1 cleanup. Next bounded step is a separate reviewed chain-integration package.
**Current W7-C / W8-B / W8-C / W8-D state:** W7-C is committed at `1bba8d8f` and wires the product V2 orchestrator from accepted W5 bounded extraction through W5-F/W6-B/W6-C2/W7-A/W8-A/W7-B2 while preserving the public damaged/precutover envelope. W8-B is committed at `f468a73d` under `Docs/WIP/2026-05-20_V2_Slice_W8-B_Internal_Alpha_Report_Output_And_Chain_Observability_Review_Package.md` as one hidden/internal `InternalAlphaReportResultCandidate` plus one bounded hidden/admin-only chain artifact over W7-B2 output. W8-C is implemented/verifier-clean at `9dca1af8`: W8-B `w8aMergeTrigger.status` is `parity_covered`, W8-A/W8-B fail-closed parity is covered by focused tests, W7-A remains `keep` because it is still load-bearing for orchestrator/W7-B/W8-A/W8-B lineage contracts, and boundary-guard split is deferred with a no-safe-split rationale. W8-D ran one authorized product-route canary on job `828aacb298ed43a885b8d2f41379f25e` and is classified as `STOP_X7_W8_D_INTERNAL_ALPHA_RESULT_BLOCKED_BY_SUFFICIENCY_ASSESSMENT`: public V2 stayed damaged/precutover, the hidden W8-B route was present and no-store, but W8-B recorded `internal_alpha_report_result_blocked` with `blockedReason = sufficiency_assessment_not_completed` and zero cited EvidenceItem refs. No second W8-D canary is authorized.
**Current W8-E / W8-F state:** W8-E is committed at `dfe430b5` under `Docs/WIP/2026-05-20_V2_Slice_W8-E_W8B_Upstream_Stop_Attribution_Package.md`. It adds enum-only `upstreamStopAttribution` inside the existing W8-B artifact/projection, using parent decisions already passed to W8-B. W8-F passed exactly one product-route canary on job `5a9f11c1b3e34be18b6bf49ed6fc4d65` using runtime `75c7a786e073db2f099864837ce537d3a859697c`: public V2 stayed damaged/precutover with no W8-E/W8-B leak, the W8-B route returned one no-store text-free artifact, and attribution identified `firstIncompleteStage = sufficiency_assessment` with W6-C parent `sufficiency_assessment_damaged` / `schema_validation_failed`. W8-F authorizes no source changes beyond result documentation, no second canary, no prompt/model/config/schema/UCM/gateway edit, no public/API/UI/report/export/compatibility behavior, no parser/cache/SR/storage/provider expansion, ACS/direct URL, V1 work, V1 cleanup, or W8-B readiness relaxation.
**Current W6-C3 diagnostic result:** W6-C3 is implemented at `cb070a7d`, and the single authorized canary job `0456086280104979b74da724d9d58308` passed as `PASS_X7_W6_C3_SUFFICIENCY_SCHEMA_DIAGNOSTICS_CAPTURED` on runtime `132d90ae141a72f2a492e8acf936d587229b0a03`. Public V2 stayed `4.0.0-cb-precutover` / `blocked_precutover` / `report_damaged`; the W8-B hidden route stayed internal/admin-only and text-free by default. W6-C still stopped as `sufficiency_assessment_damaged` / `schema_validation_failed`, but now exposes bounded schema issue paths/codes: invalid enum values under `sufficiencyAssessment.missingEvidenceDimensions.*` and integrity event shape drift at `integrityEvents.0.type`, `integrityEvents.0.references`, and `integrityEvents.0`. Live-job budget is now `7`; no second W6-C3 canary is authorized. Next action is a narrow W6-C4 prompt/contract repair review package, not direct prompt/schema edits.
**Current W6-C4 package state:** `Docs/WIP/2026-05-20_V2_Slice_W6-C4_Sufficiency_Prompt_Contract_Repair_Package.md` is prepared and Claude Opus-reviewed as `APPROVE_WITH_AMENDMENTS`, with amendments applied. It recommends a narrow W6-C prompt-contract repair only: enumerate existing schema literals for missing-evidence dimension/materiality, blocked/damaged reasons, and task-event fields; add prompt-contract tests including anti-canary-domain leakage checks; run one later canary after implementation. It does not authorize implementation by itself because prompt/model/config/schema work remains approval-gated.
**Current W6-C4 implementation state:** W6-C4 is locally implemented and verifier-clean. It amends only `V2_EVIDENCE_SUFFICIENCY_GATE` in `apps/web/prompts/claimboundary-v2.prompt.md` and adds schema-derived prompt-contract tests. No schema literals, validation behavior, model/config/UCM/gateway policy, runtime provider seam, public surface, or retry behavior changed. One product-route canary remains pending after commit, runtime refresh, route preflight, and clean git status.
**Current W6-C4 canary result:** W6-C4 passed on job `cbb4f6b5ae9d49a3bb6f941c7ae6c231` using runtime `621f8efe062f66deda190c30832a5feaa2dbe74d`. W6-C now completes with `sufficiency_assessment_completed`, `sufficiencyResultStatus = accepted`, `schemaDiagnostics = null`, and `reportStopRecommendation = refine_retrieval`. Public V2 remains `4.0.0-cb-precutover` / `blocked_precutover` / `report_damaged`, and the hidden W8-B route remained text-free by default. The active stop-line moved to `boundary_verdict_candidate_not_ready` / W7-A `closed_until_llm_task_approved`. Live-job budget is `6`; no second W6-C4 canary is authorized.
**Current W6-C5 / W6-D state:** W6-C5 is implemented at `d59b0248bc78965c1a5988b9cea5df15e9590e2a` and the corrected product-route canary job `305176cf9cd34829b08dc826cf850b64` is classified as `STOP_X7_W6_C5_RETRIEVAL_REFINEMENT_REQUIRED`. W6-C completed as `sufficiency_assessment_completed`, `sufficiencyResultStatus = accepted`, `schemaDiagnostics = null`, and `reportStopRecommendation = refine_retrieval`; W8-B remained blocked at `boundary_verdict_candidate_not_ready`. The earlier W6-C5 job `5762f4f74f7c479daca6c6f0290b7ce8` consumed a live-job slot but is unevaluated because the hidden artifact capture used a faulty PowerShell URL interpolation pattern. Live-job budget is now `4`. W6-D is the amended retrieval-refinement review package: W6-D1 contract widening first, then W6-D2 same-provider page-summary fan-in if W6-D1 is clean. Do not continue W6-C prompt retries, relax W7 gates, or run a W6-D canary before both implementation slices are verifier-clean.
**Current W6-D1 implementation state:** W6-D1 is locally implemented and verifier-clean as contract-only bounded fan-in widening. It keeps the one-record path valid while allowing canonical arrays for `1..3` Source Material records, corpus admission inputs, bounded-text sidecars, and one aggregate extraction-input packet capped at `4096` bytes. W6-D1 adds no runtime fan-in fetch, live job, provider expansion, parser, public behavior, prompt/model/config/schema change, cache/SR/storage, report/verdict/warning/confidence behavior, ACS/direct URL, V1 work, or V1 cleanup. Next action after commit is W6-D2 same-provider Wikimedia page-summary fan-in through the existing approved path.
**Current W6-D2 implementation state:** W6-D2 is locally implemented and verifier-clean as bounded same-provider Wikimedia page-summary fan-in inside the existing W3-B Source Material runtime owner. It selects up to `3` structurally eligible distinct W3-A materialized locators in existing candidate order, fetches them sequentially through the approved page-summary transport, and emits multiple bounded Source Material records for the W6-D1 downstream contracts. W6-D2 adds no live job, provider expansion, retries, semantic ranking, parser, public behavior, prompt/model/config/schema change, cache/SR/storage, report/verdict/warning/confidence behavior, ACS/direct URL, V1 work, or V1 cleanup. Next action after commit is a W6-D product-route canary package/preflight; no W6-D canary has run yet.
**Current W6-D canary result:** The one authorized W6-D product-route canary job `cc1700dd7ae544d4877482f93d399474` is classified as `STOP_X7_W6_D_DEFAULT_ADMIN_BOUNDED_TEXT_SIDECAR_ARRAY_LEAK`. The canary proved W6-D product-route reachability through W3-B fan-in with `3` bounded Source Material records, W5 `accepted` with `3` EvidenceItems, and W6-C `accepted` with `schemaDiagnostics = null`, but W6-C still recommended `refine_retrieval`. The stop reason is containment: the W4-G bounded-text default-admin route redacted the singular sidecar projection but exposed text through the plural `boundedTextSidecars[]` projection introduced by fan-in. Repair commit `8dcbb982` amends the existing default projection redaction and focused route test. Remaining live-job budget is now `3`. No second W6-D canary is authorized by the original package; the next action is a narrow containment-repair rerun package if Steer-Co agrees.
**Current W6-D3 containment repair result:** W6-D3 rerun job `be008b8b3bdd4c7fa6edd19c368061db` is classified as `PASS_X7_W6_D3_CONTAINMENT_REPAIR_VERIFIED_RETRIEVAL_REFINEMENT_REMAINS`. Runtime `5ea83b5a67b655d39de2703b970dd9acbc63e3b5` includes repair `8dcbb982`; W4-G default-admin singular and plural bounded-text sidecar projections are now hash/length/provenance-only with no `text` fields returned. W3-B again produced `3` Source Material records and W5 accepted `3` EvidenceItems, while W6-C completed accepted with `schemaDiagnostics = null` and still recommended `refine_retrieval`. Remaining live-job budget is now `2`. Next action is a bounded query/source diversity review package; do not spend another same-provider W6-D canary, weaken W6 prompts, or relax W7 gates.
**Current W6-E implementation state:** W6-E is locally implemented and verifier-clean as a bounded structural query-balanced selection repair inside the existing W3-B Source Material page-summary owner. It selects one eligible locator per W3-A provider-attempt group before filling remaining slots, keeps the hard cap at `3`, keeps same-provider Wikimedia page-summary fetches, and adds no semantic ranking, provider expansion, prompt/model/config/schema change, public behavior, cache/SR/storage, parser, ACS/direct URL, V1 work, or V1 cleanup. No W6-E live job has run yet. Next action is a separate W6-E canary package decision if Steer-Co agrees the remaining budget justifies one job.
**Current W6-E / W6-F / W6-F1 state:** W6-E canary job `6a09d149d5d046cb95d0cdd67e02c095` proved query-balanced same-provider selection across three provider-attempt groups, but W6-C still recommended `refine_retrieval`. W6-F set the OpenAlex-first provider/source diversity direction, and W6-F1 is implemented as one bounded OpenAlex academic Source Material path plus the minimal closed two-endpoint/two-kind contract widening. The current W6-F1 terminal repair evidence is canary job `2c60d8e749514f0d84e1158ae7dc9354` on runtime `7f5fe959b2a9c60b3ee86a0d69bc3ad6cee29b37`: OpenAlex and Wikimedia Source Material reached W4-H/W5/W5E/W5-F successfully, containment held, and the remaining stop is retrieval refinement / `boundary_verdict_candidate_not_ready`. Do not weaken W6 prompts, relax W7 gates, add another provider, or rerun W6-F1/W5E; the next step belongs to the successor team as a reviewed retrieval-quality/source-diversity decision.
