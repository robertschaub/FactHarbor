...
**Current HighJump HJ17 implementation state:** HJ17 is locally implemented and verifier-clean under `Docs/WIP/2026-05-22_V2_HighJump_HJ17_W5_Output_Budget_Repair.md`. The repair amends only the existing W5 `evidence_extraction` output budget from `4000` to `8000` and adds one topic-neutral compactness instruction to `V2_EVIDENCE_EXTRACTION`; Query Planning and W7-B output budgets remain unchanged. Focused W5/policy/prompt tests, full `analyzer-v2-runtime`, full `analyzer-v2`, V2 gate validation, gate-register self-test, debt sensors, build, and diff checks passed. No chunking, retry, schema relaxation, source/provider change, aggregate-cap change, public behavior, parser, cache/SR/storage, ACS/direct URL, V1 work, or V1 cleanup was added. One HJ17 canary is ready after commit, runtime refresh, API/Web runtime hash match, route/runtime preflight, and live-job ledger reconciliation; inspect W5 output tokens, latency/cost, and whether W5 still reaches the `8000` ceiling. No live HJ17 canary has run yet.
