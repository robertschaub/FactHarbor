...
**Current W6-E / W6-F / W6-F1 state:** W6-E canary job `6a09d149d5d046cb95d0cdd67e02c095` is classified as `PASS_X7_W6_E_QUERY_BALANCED_SELECTION_VERIFIED_PROVIDER_DIVERSITY_REQUIRED`. Runtime `f1a45285f2d2b22fceb03d91007f9d24c47fdce8` selected W3-B records `SOURCE_CANDIDATE_PREVIEW_1_1`, `SOURCE_CANDIDATE_PREVIEW_2_1`, and `SOURCE_CANDIDATE_PREVIEW_3_1`, proving three distinct provider-attempt groups. W5 accepted `3` EvidenceItems and containment held, but W6-C still completed accepted with `schemaDiagnostics = null` and `reportStopRecommendation = refine_retrieval`. W6-F set the OpenAlex-first provider/source diversity direction, and W6-F1 is locally implemented and verifier-clean as one bounded OpenAlex academic Source Material path plus the minimal closed two-endpoint/two-kind contract widening. Remaining live-job budget in the machine ledger is currently `1` before any W6-F1 canary. Do not spend a W6-F1 live job, weaken W6 prompts, relax W7 gates, or add another provider before W6-F1 is committed, runtime-refreshed, route/default-admin preflighted, and OpenAlex credential/no-key posture is explicit.
