...
**Current X7-W2 transport repair package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-TR1_Standard_Client_Transport_Repair_Source_Package.md` is implementation-verified and awaiting focused commit plus the single post-repair live canary. TR1 amends only the current W2 production default transport request so it uses the standard Node HTTPS connection path without the custom pinned lookup callback, while retaining DNS pre-resolution/public-address validation, final remote-address validation, redirect-deny, proxy-none, no-credentials, byte/timeout caps, endpoint allowlist, and raw-leak controls. If the post-repair canary still produces zero bytes/candidates, stop and prepare an endpoint/client pivot package; do not patch the custom DNS stack inside TR1.
