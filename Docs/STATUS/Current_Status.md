...
- **V2 implementation state:** 4C3b through 7N-3B1 are committed. The latest source slice is 7N-3B1 (`878ba6ba`, hardened at `3d05583e`): it adds only a hidden/internal candidate-acquisition runtime shell with package-scoped authority, structural allowlist/budget validation, injected provider boundary, hidden candidate records, per-query outcome accounting, no module cache, and boundary guards. It does not add concrete provider SDKs, direct fetch/network, content dereference, parser execution, cache IO, Source Reliability, product/public wiring, or live jobs. The public result remains the damaged pre-cutover envelope.
- **Latest V2 gate:** 7N-3B1 is implementation-complete under the reviewed package `Docs/WIP/2026-05-16_V2_Slice_7N3B1_Candidate_Acquisition_Runtime_Source_Package.md`. Post-review hardening at `3d05583e` binds the child authority to parent 7N-3A snapshot hashes, constrains provider attempt IDs to opaque structural IDs, prevents non-success provider outcomes from carrying candidates, and enforces provider timeouts in the injected runtime shell.
- **Next V2 gate:** Re-review `Docs/WIP/2026-05-16_V2_Slice_7N3B2_Candidate_Provider_Network_Source_Package.md` after the first deputy review returned `modify`. It is a draft package only; implementation remains blocked until deputy review explicitly approves it. 7N-3B2 must stay SDK-free, Node-core-transport-only, candidate-provider network only, and must not combine content dereference/parser execution, live smoke, cache/SR integration, or product/public wiring. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a later reviewed live-smoke gate makes live jobs meaningful. Separately review the `Plastic recycling is pointless` `blocked/no_valid_claim` outcome before treating broad benchmark claims as quality evidence.
