...
- **QLT-4 (per-claim contrarian retrieval) is CLOSED and removed.** The experimental feature was implemented (`b3e85c54`) but never triggered on real data — even after enabling config and lowering thresholds. Preflight found that Plastik EN per-claim evidence already has substantial minority-direction coverage (ratio 0.62, 21 minority items per claim). Remaining variance is driven by evidence content/quality variation, not per-claim direction scarcity. The experimental code, UCM config fields, and tests have been removed from the codebase. Historical design context preserved in `Docs/AGENTS/Handoffs/`. See `Docs/AGENTS/Handoffs/2026-03-26_Senior_Developer_QLT4_Preflight_Verification.md`.
