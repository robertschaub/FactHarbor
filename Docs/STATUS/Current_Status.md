...
**Current X7-W4-F review package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W4-F_Product_Route_Observability_Canary_Package.md` is prepared for Steering Board review only. It recommends a narrow product-route observability bridge after W3-B/W4-A that records one hidden/admin-only W4-C/W4-D/W4-E chain artifact, then proposes exactly one later canary using the known W3-B-successful Captain-defined input only after implementation approval, clean verifiers, runtime refresh, and route preflight. W4-F must not authorize source text, extraction input, EvidenceItems, parser, report/verdict/warning/confidence/public behavior, cache/SR/storage, retries, provider expansion, W2 endpoint migration, ACS/direct URL, V1 work, or V1 cleanup. The remaining live-job tranche stays at `4` until Steering approves and a later implementation package runs the proposed canary.
