...
- **Prepared X7-T Query Planning runtime live-smoke package:** `Docs/WIP/2026-05-17_V2_Slice_X7-T_Query_Planning_Runtime_Live_Smoke_Package.md` is a reviewer-approved docs-only execution package for at most two Captain-defined direct-text jobs after package commit, clean runtime refresh, route preflight, and focused verifiers. It is limited to hidden product-route Query Planning runtime/artifact reachability and public non-leak checks. It does not authorize source/search/fetch/parser/SR/cache IO, EvidenceCorpus/evidence/report/verdict/warning/confidence behavior, prompt/config/model/schema edits, public cutover, ACS/direct URL, V1 reuse, or V1 cleanup.
