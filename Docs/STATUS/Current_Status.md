...
- **V2 implementation state:** 4C3b hidden direct-text source wiring, bounded hidden artifact sink, P1 product-reachability fix, API V2 variant admission, internal artifact inspection route, 4C3c smoke ledger-scope fix, 4C4 internal Claim Understanding stage handoff, 4C5 hidden adapter-attempt diagnostics (`4b36aab5`), 4C6 strict fenced-JSON output coercion (`57dc2308`), 7A Evidence Lifecycle accepted-only intake (`08c7ddae`), 7C non-executable source-acquisition request (`22530936`), 7E shared non-executable task-policy contract (`d9d25ef7`), 7F Evidence Corpus pre-execution build decision (`b8a97413`), 7G source-acquisition execution ownership docs (`47c531f2`), 7H inert source-acquisition port contract (`24ad47fd`), 7I evidence prompt/model task design docs (`f58373a5`), 7J reviewed approval package (`c5da518d`), and 7J-1 non-executable Evidence Lifecycle task contracts (`1a874b8d`) are committed. The public result remains the damaged pre-cutover envelope.
- **Latest V2 gate:** 7J-1 is implemented under direct Captain approval of `Docs/WIP/2026-05-15_V2_Slice_7J_Evidence_Prompt_Schema_Model_UCM_Approval_Package.md`. It added inert V2 Evidence Lifecycle prompt sections, strict task-result schemas, categorical missing-evidence dimensions, and non-executable task-policy metadata. It did not seed prompts, flip approvals, execute prompt/model/provider/search/fetch, add cache IO, integrate Source Reliability, expose public output, run live jobs, execute canaries, or clean V1.
- **Next V2 gate:** A reviewed post-7J-1 package is required before any Evidence Lifecycle execution or source-acquisition implementation. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a reviewed executable gate makes live jobs meaningful. Do not add provider SDK imports, search/fetch calls, prompt/model runtime calls, UCM/default changes, cache IO, SR imports/calls, product wiring, public exposure, live jobs, or V1 cleanup without that gate. Separately review the `Plastic recycling is pointless` `blocked/no_valid_claim` outcome before treating broad benchmark claims as quality evidence.
