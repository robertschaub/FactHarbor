...
**Current X7-W2 transport repair package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-TR1_Standard_Client_Transport_Repair_Source_Package.md` is Claude Opus-reviewed and approved as the next narrow source package after LS6. TR1 proposes trying the standard Node HTTPS connection path without the current custom pinned lookup callback first, while retaining DNS pre-resolution/public-address validation, final remote-address validation, redirect-deny, proxy-none, no-credentials, byte/timeout caps, endpoint allowlist, and raw-leak controls. If the post-repair canary still produces zero bytes/candidates, stop and prepare an endpoint/client pivot package; do not patch the custom DNS stack inside TR1.
...
- **Prepared X7-W2-TR1 standard-client transport repair package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-TR1_Standard_Client_Transport_Repair_Source_Package.md` is Claude Opus-reviewed and approved. It authorizes, after package commit, a narrow repair of the current W2 transport path using the standard Node HTTPS connection path without the custom pinned lookup callback as the primary candidate. It preserves DNS pre-resolution/public-address validation, final remote-address validation, endpoint allowlist, redirect-deny, proxy-none, no-credentials, byte/timeout caps, W2 hidden-only artifacts, and all no-source-material/no-public/no-downstream boundaries. It includes synthetic tests plus exactly one post-repair live canary after verifiers. If that canary does not produce nonzero bytes and candidates, the next step is an endpoint/client pivot package, not further TR1 repair.
