...
- **V2 implementation state:** 4C3b through 7N-3B3-1 are implementation-complete. The latest source slice is 7N-3B3-1: it adds only hidden/internal content-dereference authority, raw-URL-free content target and budget envelopes, branded HMAC-bound ephemeral execution targets, SDK-free Node-core HTTPS content transport, DNS/final-address SSRF checks, IPv4-mapped IPv6 blocking, redirect denial, declared-vs-sniffed content-type mismatch rejection, streaming/compressed/decompressed byte caps, timeout/abort handling, and sanitized hidden diagnostics. It does not add product/orchestrator/runner/API/UI/report/export wiring, live jobs, cache IO, Source Reliability, parser/sink execution, evidence/report generation, ACS/direct URL execution, prompt/config/model/schema changes, public result exposure, or V1 cleanup. The public result remains the damaged pre-cutover envelope.
- **Latest V2 gate:** 7N-3B3-1 is implementation-complete under `Docs/WIP/2026-05-16_V2_Slice_7N3B3-1_Content_Dereference_Source_Package.md`. Initial deputy review returned `MODIFY`; blockers were fixed before commit: parent 7N-3B2 authority hash binding, owner-branded HMAC-bound raw execution targets, IPv4-mapped IPv6 blocking, declared/sniffed content-type mismatch rejection, diagnostic hash sanitation by positive 64-hex validation, and exact production/test file-envelope guards.
- **Next V2 gate:** 7N-3B3-1 post-implementation consolidation is recorded in `Docs/WIP/2026-05-16_V2_Slice_7N3B3-1_Post_Implementation_Consolidation.md`, and 7N-3B3-2A is approved in `Docs/WIP/2026-05-16_V2_Slice_7N3B3-2A_Parser_Sink_Isolation_Package.md` as a docs-only parser/sink isolation boundary. The next package is deputy review of `Docs/WIP/2026-05-16_V2_Slice_7N3B3-2B_Parser_Sink_Source_Package.md`; it is a draft source package only, not implementation approval. 7N-3B3-2B must stay fixture/control-only unless a later 7N-3B3-2C package explicitly approves an owner-only real transport-byte handoff from `source-acquisition-content-transport.ts`. Parser/sink source implementation, real transport-byte handoff, live jobs, product/public wiring, cache/SR integration, evidence/report generation, and V1 cleanup remain blocked. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a later reviewed live-smoke gate makes live jobs meaningful. Separately review the `Plastic recycling is pointless` `blocked/no_valid_claim` outcome before treating broad benchmark claims as quality evidence.
