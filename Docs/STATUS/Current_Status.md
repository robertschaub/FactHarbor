...
- **Next V2 gate:** one committed/refreshed hidden direct-text diagnostic smoke after 4C5, then a reviewed Claim Understanding contract-quality fix package based on the new adapter attempt diagnostics. 4C3c smoke produced `claim_contract_validation_failed`; do not start Evidence Lifecycle until a reviewed gate proves direct-text Claim Understanding can produce an accepted `ClaimContract` or explicitly chooses an ACS-only downstream path. Do not broaden live jobs, public exposure, cache IO, ACS/direct URL dispatch, prompt/config/model/schema changes, or V1 cleanup without a later reviewed gate.
