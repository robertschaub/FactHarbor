...
**Current X7-W2 diagnostic addendum:** X7-W2-LS5 has now also run and is classified as `PASS_X7_W2_LS5_DIAG4_OBSERVED_OTHER_KNOWN_REMAINS`; DIAG4 did not classify the live W2 transport failure. `Docs/WIP/2026-05-18_V2_Slice_X7-W2-RP1_Local_Raw_Code_Probe_Package.md` is Claude Opus-approved as the next diagnostic package: exactly one transient operator-local `https.request` probe outside the repo may observe the raw Node code without recording it in product/admin/runtime artifacts, committed docs, logs, scripts, or chat. RP1 does not authorize live jobs, source material/content/parser/cache/SR/storage, EvidenceCorpus/evidence/report/verdict/warning/confidence/public behavior, provider expansion, endpoint redesign, ACS/direct URL, V1 work/cleanup, source/test/prompt/config/schema/model edits, retries, or additional probes.
...
- **Prepared X7-W2-DIAG5 RP1-observed taxonomy source package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-DIAG5_RP1_Observed_Node_Code_Taxonomy_Source_Package.md` is Claude Opus-approved. It permits source/test-only promotion of the RP1-observed raw literal while keeping narrative docs and product/admin/public artifacts raw-literal-free, adds only the generic `address_validation_failure` taxonomy family, and keeps W2 completion semantics unchanged. It authorizes no live jobs, endpoint/client repair, provider expansion, source material/content/parser/cache/SR/storage, EvidenceCorpus/evidence/report/verdict/warning/confidence/public behavior, ACS/direct URL, V1 work/cleanup, retries, or additional probes.
