...
- **Prepared parser provenance package:** `Docs/WIP/2026-05-16_V2_Slice_7N3B3-2D-C0-S2_Parser_Admission_Provenance_Source_Package.md` is a docs-only review package for future process-local runtime-owned C0-S1 admission provenance. It authorizes no source edits now and does not approve parser execution, worker spawn, byte consumption, parsed material, source material, Evidence Lifecycle consumption, product/public/live wiring, or 2D-C.
