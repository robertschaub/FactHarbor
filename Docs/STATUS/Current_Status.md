...
**Phase**: **Alpha**
**Status**: V2 pipeline rebuild is the current strategic execution track. `Docs/STATUS/Backlog.md` is the authoritative active queue; this file is a high-level snapshot and historical context. V1 ClaimAssessmentBoundary work is maintenance-only until V2 cutover readiness. The V2 hidden direct-text runtime path has advanced through accepted Claim Understanding, Query Planning, bounded Source Acquisition, Source Material, corpus/readiness gates, W5 EvidenceItem extraction/admission/handoff, X7-W5-H retirement of the standalone W4-I route, W6-B contract-only sufficiency intake, and W6-C hidden/internal sufficiency assessment implementation at `8f7856b5`. Public V2 remains intentionally blocked/precutover/report_damaged, the live-job tranche ledger currently records `currentRemaining = 0`, and W6-C authorizes no live job, product route, public/API/UI/report/export behavior, verdict/warning/confidence behavior, parser/cache/SR/storage/provider expansion, ACS/direct URL support, prompt/model/config/schema/UCM edits beyond the committed W6-C gateway/model/cache approval metadata, V1 cleanup, or cutover. W7-A Boundary/Verdict Candidate Contract is prepared as review package only at `Docs/WIP/2026-05-20_V2_Slice_W7-A_Boundary_Verdict_Candidate_Contract_Review_Package.md`; implementation remains approval-gated.
