...
**Current V2 default/manual UI state:** Commit `79a5c31f` makes `claimboundary-v2` the default manual submission pipeline through UCM defaults, web/API job and draft creation defaults, and runner missing-variant fallback. Explicit legacy `claimboundary` requests still run V1. HJ21 default-manual-path canary job `15d19b57f0fb488ea820bac0e2fb6dac` passed on runtime `7b900247c8bf5a488923c23516f160cd51753396`: the job was submitted without an explicit `pipelineVariant`, stored `claimboundary-v2`, and produced authenticated-admin `reportMarkdown` length `7605` for the job page `V2 Report` path. Public/default V2 remains `4.0.0-cb-precutover` / `blocked_precutover` / `report_damaged`, and public/default job API returns no report markdown. The active lane projection is `Docs/STATUS/V2_Current_Lane.md`; the next HighJump step is report-quality review over the admin-visible V2 report and the smallest improvement to the next observed report-quality bar.
