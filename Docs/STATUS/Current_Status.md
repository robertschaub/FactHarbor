...
- **No analyzer behavior change is currently approved.** The structural_consistency bug fix (`289afa1c`) already shipped. All remaining quality items are monitor-only under EVD-1.
- **Optional diagnostics improvement** (not a quality-track reopening): persist `inputClassification` and claim-contract validation summary in result JSON for future forensics. Low priority.
