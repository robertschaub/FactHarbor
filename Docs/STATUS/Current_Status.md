...
- **Prepared legal-question direct-text smoke package:** X7-N-C is drafted at `Docs/WIP/2026-05-17_V2_Slice_X7-N-C_Legal_Question_Direct_Text_Live_Smoke_Package.md`. It is docs-only until accepted and committed. It authorizes at most one Bolsonaro/fair-trial Captain-approved direct-text job after clean worktree, runtime refresh from the committed package, explicit V2 shell and hidden Claim Understanding runtime gates, focused verifiers, and admin-route preflight. It exists only to test whether X7-M's externally assessable direct-question repair now produces accepted hidden Claim Understanding output while public output remains pre-cutover blocked; it does not approve truth/legal/fairness conclusions, Query Planning execution, source/provider/parser execution, EvidenceCorpus/report/verdict behavior, cache/SR/storage, ACS/direct URL, public cutover, or V1 cleanup.
