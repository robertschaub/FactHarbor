...
**Current W8-E package state:** `Docs/WIP/2026-05-20_V2_Slice_W8-E_W8B_Upstream_Stop_Attribution_Package.md` is prepared as the bounded next step after W8-D. It authorizes only enum-only upstream stop attribution inside the existing W8-B artifact/projection, using parent decisions already passed to W8-B. It authorizes no new route/sink, live job, prompt/model/config/schema/UCM/gateway edit, public/API/UI/report/export/compatibility behavior, parser/cache/SR/storage/provider expansion, ACS/direct URL, V1 work, V1 cleanup, or W8-B readiness relaxation.
