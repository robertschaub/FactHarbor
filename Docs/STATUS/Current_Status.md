...
**Current V2 default/manual UI state:** Commit `79a5c31f` makes `claimboundary-v2` the default manual submission pipeline through UCM defaults, web/API job and draft creation defaults, and runner missing-variant fallback. Commit `2e12b254` also makes the preparation page show the active draft pipeline (`Pipeline V2` / `Pipeline V1` / default) and improves V2 report-writer citation rendering. Explicit legacy `claimboundary` requests still run V1. HJ22 report-citation/readability canary job `4e81f840f6e04e2793e9ec162ee7bef2` passed and produced a cited hydrogen-family admin report. HJ23 then ran a stronger two-job mini-gauntlet on `asylum-235000-de` and `bolsonaro-en`: both stored `claimboundary-v2` and preserved public/precutover containment, but neither produced a completed admin internal report. Asylum reached W5 with `hidden_no_extractable_evidence`; Bolsonaro exposed W3-B fail-fast behavior on one structurally rejected page summary. Follow-up local repairs are `70644fcb` for W3-B continuation after invalid summaries and `02b35ebb` for direct-record query-planning intent. The active HighJump tranche has `0` live jobs remaining; continue local source-material generalization until budget is reset or extended.
