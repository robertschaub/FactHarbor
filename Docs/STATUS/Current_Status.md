...
- **Prepared post-route-repair live-smoke recovery package:** X7-N-B is drafted at `Docs/WIP/2026-05-17_V2_Slice_X7-N-B_Post_Route_Repair_Clean_Live_Smoke_Package.md`. It is docs-only until accepted and committed. It authorizes at most one German Captain-approved direct-text job after clean worktree, runtime refresh from the committed package, explicit V2 shell and hidden Claim Understanding runtime gates, focused verifiers, and admin-route preflight. It exists only to prove that the repaired route reaches V2 without V1 fallback while public output remains `4.0.0-cb-precutover` / `blocked_precutover`; it does not approve the Bolsonaro canary, live-job expansion, Query Planning execution, source/provider/parser execution, EvidenceCorpus/report/verdict behavior, cache/SR/storage, ACS/direct URL, public cutover, or V1 cleanup.
