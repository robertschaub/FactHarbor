...
- **X7-T runtime-refresh precondition failure and X7-T-R rerun addendum:** X7-T first submission `878828510c034cf7a72af502c38e48bd` failed closed before V2 execution with `v2-shell-disabled` because the local Windows runtime refresh did not set the V2 gate env vars on the actual Next.js runner process. It is classified as `PRECONDITION_FAIL_X7_T_RUNTIME_GATE_NOT_REFRESHED`, not as a Query Planning runtime sample. `Docs/WIP/2026-05-17_V2_Slice_X7-T-R_Runtime_Gate_Refresh_Rerun_Addendum.md` is the narrow docs-only rerun addendum: after commit, corrected runtime refresh, fresh process-gate proof, route preflight, and idle checkpoint, it may run only the same two exact X7-T Captain-defined inputs. X7-T-R does not authorize source/search/fetch/parser/SR/cache IO, EvidenceCorpus/evidence/report/verdict/warning/confidence behavior, prompt/config/model/schema edits, public cutover, ACS/direct URL, V1 reuse, or V1 cleanup.
