...
- **V2 implementation state:** 4C3b through X3-A are implementation-complete. The latest source slice is X3-A at `3db89dee`: it aligns Evidence Lifecycle task-policy, execution-readiness, query-planning input-envelope, source-acquisition-port metadata, and gate-register drift state so `evidence_query_planning` is the only hidden/internal executable Evidence Lifecycle task. It keeps source acquisition, real-byte parser consumption, later evidence tasks, product/orchestrator/runner/API/UI/report/export wiring, live jobs, cache IO, Source Reliability, ACS/direct URL execution, public result exposure, and V1 cleanup blocked. The public result remains the damaged pre-cutover envelope.
- **Latest V2 gate:** X3-A is implementation-complete under `Docs/AGENTS/Handoffs/2026-05-16_Lead_Architect_V2_X3A_Query_Planning_Metadata_Drift_Repair.md` at `3db89dee`. Reviewer consensus was PASS after one requested test addition for the changed batch-envelope execution scope. The X2 gate register now removes the repaired `static_task_policy_symbolic_not_executable` drift marker and keeps only `prompt_frontmatter_required_sections_lag`.
- **Next V2 gate:** X3-B prompt frontmatter/text alignment remains blocked until explicit Captain/LLM Expert prompt approval. If prompt approval is not provided in the current execution window, the next low-risk code gate is V2-X4 public cutover guard from `Docs/AGENTS/Handoffs/2026-05-16_External_Advisor_V2_Pipeline_Executable_Action_Plan.md`. Parser consumption of real fetched bytes, live jobs, product/public exposure, cache/SR integration, evidence/report generation, prompt/model/config/schema changes beyond approved gates, ACS/direct URL execution, and V1 cleanup remain blocked until later explicit gates. Captain-approved future direct-text canaries are `Mehr als 235 000 Personen aus dem Asylbereich sind zurzeit in der Schweiz` and `Did the legal proceedings against Jair Bolsonaro comply with Brazilian law, and did the proceedings and the verdicts meet international standards for a fair trial?`; do not run or substitute them until a later reviewed live-smoke gate makes live jobs meaningful. Separately review the `Plastic recycling is pointless` `blocked/no_valid_claim` outcome before treating broad benchmark claims as quality evidence.
