...
- **X7-W2-DIAG2 sanitized transport diagnostics package:** `Docs/WIP/2026-05-18_V2_Slice_X7-W2-DIAG2_Sanitized_Transport_Diagnostics_Source_Package.md` is Claude Opus-reviewed and approved for implementation. It may only add/propagate hidden-admin structural diagnostics for W2 network attempts: DNS address count, selected address family enum, final-address/response/content states, and transport-failure class enum. It does not authorize a live rerun, source material, content dereference, parser/cache/SR/storage, EvidenceCorpus/evidence/report/verdict/warning/confidence behavior, public output, ACS/direct URL, provider expansion, prompt/config/model/schema/provider-policy edits, V1 work, or V1 cleanup.
