...
- When authorized delegation adds useful independent evidence, use it in Step 3. Otherwise,
  perform the same investigation yourself with the provided checklist.
- Use only supported client/model settings and record actual reviewer participation.
...
## Step 5b — Integrator-owned index maintenance

Report any changed handoff sources to the designated integrator. WIP/status edits alone do not require a handoff-index rebuild. Only the integrator may run `node scripts/build-index.mjs --tier=2` when relevant sources changed and the effective Git hook has not already produced the current index. Assign the output/log state before execution and record any missing builder/index. Workers and read-only reviewers do not run index builders.
