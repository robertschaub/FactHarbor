...
Diagnose LLM prompting issues for: TASK_ARGUMENTS
...
**1a. If TASK_ARGUMENTS specifies a job ID, phase directory, validation batch, or failure description:**
...
```bash
sqlite3 -readonly apps/web/config.db "SELECT content FROM config_blobs WHERE content_hash = '<PROMPT-HASH>'"
```
...
```bash
sqlite3 -readonly apps/web/config.db \
...
## Phase 6 — Propose or record the Prompt Issue Register update

Only an assigned writer with current authority for this record may apply these steps. Read-only diagnosis returns proposed new/amended entries in chat for the integrator; it does not create a register, update timestamps or change statuses.
