...
Apply debt guardrails to: TASK_ARGUMENTS
...
- **§Failed-Attempt Recovery** — classify a failed attempt (`keep | amend |
  revert | quarantine | add`), log the `ATTEMPT <n>` line and name the last-known-good. An assigned writer persists via `node scripts/hooks/revert-classify.cjs`; read-only reviewers return the line in chat for the integrator.
...
3. **Attempt ≥ 2 is the danger zone.** A second edit in the same fix chain
   ...
   On repeated failed validation, obtain independent review under root policy, comparing amendment and revert-to-last-known-good before adding complexity. Use a compatible reviewer; do not require a Claude-only advisor.
...
5. **Ownership check before reverting** — never revert user-authored or
   ...
   `git reset`/`checkout` ; hooks are not proof of coverage in every client/session.
