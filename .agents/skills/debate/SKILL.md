...
Run a structured adversarial debate on: TASK_ARGUMENTS
...
`TASK_ARGUMENTS` is the proposition to debate, optionally prefixed with flags:
...
1. Extract proposition, tier, constraints, and context manifest from `TASK_ARGUMENTS`
...
**Model mapping:** use models/roles supported by the active client and permitted by the assignment. Choose suitable capacity without assuming Claude aliases or prices. When the client does not support delegation, provide a clearly labeled single-session analysis and request an independent review only if required; do not claim independent agents ran.
...
<<For an explicitly sequential rebuttal only, add the Advocate output. In independent parallel mode, omit it.>>
