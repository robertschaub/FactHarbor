...
Analyze local job reports for: TASK_ARGUMENTS
(Leave blank to scan all jobs executed at HEAD.)

`TASK_ARGUMENTS` may be: one or more `jobId`s or FactHarbor job URLs (comma-separated), a commit SHA (`commit=<sha>`), a benchmark input slug (`input=bolsonaro-en`), a directory under `test-output/`, or empty.
...
| Decide | Phase 4 (adaptive multi-agent debate) | Reconcile each finding on evidence and surviving challenges, without a panel-count quorum. Record missing review separately from evidentiary confidence. Lifecycle "Decide" ≠ phase name; Phase 4 is "Debate" functionally. |
...
| Act | Only actions covered by current task authority and rule 13; otherwise return proposals. | Deliberate non-destructive posture; the skill's default is "propose, don't execute". |
| Test | Phase 3i (historical regression bisection) + Phase 3j (stability across reruns) + allowed `/validate` | Execution may share the current session when the task covers it and rule 13 prerequisites hold; otherwise propose the bounded follow-up. |
...
```bash
sqlite3 -readonly apps/api/factharbor.db \
...
**Scope caps:** if scope has more than 20 reports, auto-switch to **sampled mode**: first 5 by `CreatedUtc` ascending + last 5 by `CreatedUtc` descending + 5 outliers by `confidence` or `verdict` distance from the family's expected band (deterministic — sort by `abs(observed_truthPct - midpoint(truthPercentageBand))` descending). Record the sampling choice in Phase 7a so Captain can override with `TASK_ARGUMENTS` (e.g. `jobId` list).
...
```bash
sqlite3 -readonly apps/web/config.db "SELECT content FROM config_blobs WHERE content_hash='<lastGoodJob.PromptContentHash>'"
sqlite3 -readonly apps/web/config.db "SELECT content FROM config_blobs WHERE content_hash IN (SELECT active_hash FROM config_active WHERE config_type='prompt' AND profile_key='claimboundary')"
```
...
Use independent review for material findings under root risk criteria or the user request. Select only relevant panels; do not require a full committee. If required independent review cannot run, return provisional findings and the limitation. Record actual participation, not simulated independent agents.
...
**Panel map — choose relevant independent perspectives. Run independent panels in parallel when supported; Devil's Advocate follows the outputs it must assess:**
...
**Model selection:** use a compatible reviewer available in the active client. The role labels below express responsibilities, not universal tool names. Respect task/session controls and supported model settings; no Claude alias or price is assumed.
...
- Rules 3, 4, 5, 6, 8, 9 — SUMMARY (cite rule number when rejecting):
  ...
  9 = exact requested-job inspection and comparator discipline; 12 = index-first history lookup

SESSION CONTRACT: <<paste all applicable rules 9–14 and the exact read sources/tools; no writes, tests, builds, cache refreshes, recovery logging, Git mutations or live/provider calls>>

CONTEXT BUNDLE (supplied evidence; state limits and inspect allowed source where needed):
- HEAD: <<sha>> | DIRTY: <<bool>>
...
4. Aim for at most 300 words; retain material evidence and uncertainty. Do not defend a finding against supported counter-evidence.
...
**Exit clause — empty panel set:** if Phase 3 produced findings but no dimension maps to a panel (e.g., only 3b input-integrity or 3g cross-report-tagging findings), Phase 4 is SKIPPED entirely. Carry findings forward with confidence justified by their evidence. Record unreviewed findings as such; absence of a panel is not a mechanical confidence downgrade. Record the skip in Phase 7a as `Phase 4: skipped — no panel-mapped findings`.

**Debate reconciliation (main thread):**
```
Finding | Panel-A | Panel-B | ... | Devil's Advocate | Verdict
```
Resolve each finding using inspected evidence, domain constraints and concrete counter-evidence. One relevant reviewer can confirm a single-dimension finding; no two-panel quorum applies. Reframe, reject or qualify a finding when a supported challenge warrants it, and record why. Unsupported disagreement does not erase evidence; unresolved material concerns remain visible.
...
1. **Pre-condition fixes first.** All sub-clauses below describe REQUIRED-before orderings:
   ...
   - Any new analysis input requires Captain's exact definition before submission. Record whether it replaces or extends the approved list. An authorized family-record change, when required by the runner, precedes the run; never synthesize substitute wording or treat a family entry as spending authority.
...
The apply-order block feeds directly into Captain's approval decision — Captain can approve the whole sequence, approve a prefix, or reject. Only actions already covered by the current mandate may execute after rule 13 prerequisites; all others remain proposals. There is no automatic reseed or validation allowance.
...
**Output mode** (determined from `TASK_ARGUMENTS` and scope size):
...
- `TASK_ARGUMENTS` contains `--full` OR scope has more than 5 reports → **full** — emit every subsection.
- `TASK_ARGUMENTS` contains `--dry-run` → emit 7a (scope summary) plus a "would analyze" list describing which dimensions and panels would activate, then STOP. No Phase 4 spawn, no fix proposals, no Phase 8 writes. (7g is moot under `--dry-run` since escalations fire during real execution, not planning.)
...
- **A. Debate deadlock with no compliant resolution.** Phase 4 reconciliation leaves a material finding with no surviving compliant proposal: inspected evidence supports the finding, Devil's Advocate rebuts every proposed fix with a concrete regression risk, and no alternative mechanism satisfies the non-negotiable constraint gate. Ordinary Devil's Advocate rebuttal that results in downgrade or drop is NOT this category — that is the normal reconciliation path under SKILL.md §Phase 4 "Debate reconciliation".
- **B. Pre-condition UNKNOWN.** The pre-condition gate (rule 13) returned UNKNOWN for drift state because scope contained no recoverable `meta.promptContentHash` values for Phase 3h to compare against the active hash. Keep live execution proposed until rollout state is verified within authorized access; ask Captain only for missing authority or unavailable evidence.
...
- **E. Additional execution outside current authority.** Resolving a material ambiguity requires a run beyond the authorized inputs, operations, batch size or state. Return the concrete proposed extension. Additional runs already covered by current authority may proceed after rule 13 prerequisites and cost/regression checks; there is no per-invocation allowance or automatic spending grant.
...
1. Use a compatible independent reviewer under the current session-control contract with this brief (verbatim; fill `<<...>>`):
...
**Phase 9 emits proposals in 7h. Persist only when current authority covers the named file and update; restricted reviewers never write. Rule 13 applies as in Phase 8.**
...
**Action-authority sweep (rule 13):** Before reporting, identify each operation actually performed and its current task authorization. Verify input/provenance/state prerequisites for any authorized reseed or live validation; report scope violations explicitly and exclude invalid runs from fix evidence. Unapproved file, register, expectation, prompt/config, Git or external actions remain proposals. Never invent a new analysis input or treat skill selection as spending authority. Existing authorization persists; the next agent receives the same scope boundaries.
