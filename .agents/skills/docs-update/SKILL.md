...
name: docs-update
description: >
  Reconcile the requested documentation scope (.md and .xWiki); expand to the whole Docs tree only when requested. For living documentation
...
Read root AGENTS.md, then only the references relevant to the selected files/formats:
...
5. The active client adapter when its execution behavior matters; no other client wrapper is mandatory
...
Preserve meaning, normative obligations, sources and useful context. Authorized tightening/merging is allowed; justify substantive deletions. Do not preserve a fixed word percentage or add filler to replace removed repetition.
...
Read the sections needed to understand the change, including relevant surrounding context. Understand what it describes, what claims it
...
6. **Semantic preservation** — preserve substantive meaning, obligations, qualifications, sources and useful history. Authorized tightening may remove repetition without a word-count floor; report substantive removals and their reasons.
...
adding one. Add it only when the current task covers that change; otherwise propose in the Step R report as:
...
## Step 4b — Integrator-owned index maintenance

Workers and read-only reviewers report required index updates in chat. Only the designated integrator runs a necessary rebuild in its assigned worktree/state. Inspect the effective Git hook and current index first; do not repeat a rebuild the hook already completed. When relevant handoff sources changed, use `node scripts/build-index.mjs --tier=2`; for changed analyzer sources, use `node scripts/build-index.mjs --tier=1`. Documentation-only edits do not by themselves require either rebuild. If the builder or index is unavailable, record the limitation and use direct source reads within scope.
