...
with open('test-output/quality-ts.csv') as f:
    for r in csv.DictReader(f):
        try:
            t = dt.datetime.fromisoformat(r['iso'][:19])
        except Exception:
            continue
...
ax.set_title(f'FactHarbor claimboundary report quality over time  (n={len(rows)} reports, 2026-03-01 .. 2026-06-03)')
...
ax2.set_xlabel('build timepoint (report run date, 2026)')
