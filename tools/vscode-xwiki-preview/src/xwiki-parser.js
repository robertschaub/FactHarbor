...
class XWikiParser {
  ...
  inl(text){
    if(!text)return'';
    text=this.esc(text);
    // Handle inline (% ... %) parameters as styled spans (quotes are &quot; after esc)
    text=text.replace(/\(%\s*(.*?)\s*%\)([\s\S]*?)(?=\(%\s*.*?\s*%\)|$)/g,(m,raw,content)=>{
      const attrs=raw.replace(/&quot;/g,'"').replace(/&amp;/g,'&');
      const ha=this.buildAttrs(this.parseParams(attrs));
      return ha?`<span${ha}>${content}</span>`:content;
    });
  ...
  renderTable(rawLines,tblAttrs){
    ...
    for(const row of logicalRows){
      ...
      for(const cell of cells){
        ...
        let content=isHdr?ct.slice(1):ct;
        // Extract inline parameters (% ... %) at start of cell
        let cellAttrs='';
        const pm=content.match(/^\s*\(%\s*(.*?)\s*%\)([\s\S]*)/);
        if(pm){cellAttrs=this.buildAttrs(this.parseParams(pm[1]));content=pm[2]}
        const rendered=this.renderCellContent(content.trim());
        h+=isHdr?`<th${cellAttrs}>${rendered}</th>`:`<td${cellAttrs}>${rendered}</td>`;
      }
      h+='</tr>';
    }
    return h+'</table>\n';
