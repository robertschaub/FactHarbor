...
- First, classify the input as one of:
  ...
  - **Modifier-fusion rule for coordinated branches:** If the main act/state carries a truth-condition-bearing modifier and you split the sentence by coordinated branches, keep that modifier fused with the same main act/state in EVERY branch claim that remains inside its scope. Do NOT create one claim for the modified act/state and a separate claim for the branch chronology; the correct decomposition is branch-by-branch, with the in-scope modifier repeated in each branch claim.
...
- **Claim-level bundling prohibition:** If ANY returned thesis-direct claim still depends on two or more independently verifiable sub-propositions, you MUST split that claim even when the overall claim set contains other valid claims. This includes: (a) one act/state asserted to occur before, after, because of, or subject to multiple coordinated branch events whose branches could independently occur, fail, or be dated, (b) one evaluative or rule-governed predicate applied to multiple distinct target objects, processes, decisions, outcomes, or other evaluands that could resolve differently, and (c) comparison claims whose truth depends on one proposition about the named/current side plus a separate proposition about the comparator/reference side (including historical, threshold, or rest-of-comparison-class propositions). If each side/evaluand could be evidenced separately and one could be true while another is false, the bundled claim is non-atomic. Preserve the shared relation, predicate, or comparison anchor in each split claim, and do NOT keep a bundled whole-claim version. A conjunctive clause with one shared verb phrase does not override this rule: if the coordinated members are distinct institutions, actor groups, proceedings, decision gates, target objects, or outcomes with separate possible timelines or results, split them. Exception: keep one claim only when the coordinated phrase is an inseparable rank/order/composite proposition whose truth cannot differ branch-by-branch.
- **Surface-pattern recognition (MANDATORY for evaluative/rule-governed predicates):** A claim of the surface form `[coordinated noun phrase] [verb] [evaluative predicate or standard]` is multi-evaluand whenever the coordinated noun phrase contains two or more separable target-object units — for example distinct events, distinct phases, distinct decisions, distinct actions, distinct measurements, distinct outputs, distinct instances, distinct outcomes, distinct components, or distinct stages — that could individually pass or fail the predicate. The grammatical fusion of the subject into one noun phrase does NOT make the evaluands singular. Apply the test mechanically: enumerate each unit inside the coordinated subject, then ask whether each unit could resolve to a different truth value against the predicate. If yes, you MUST emit one thesis-direct claim per unit, preserving the shared predicate in each. The following rationalizations are NOT valid reasons to keep a bundled claim: "they share the same standard", "they cover one assessment dimension", "they preserve expanded scope", "they are part of one process", "they form one institutional whole", or "the predicate applies to both equally". The shared predicate is what gets PRESERVED across the split claims; it is not what justifies bundling them. Composite-not-split exception applies ONLY to inseparable ordered or paired phrases whose truth cannot differ branch-by-branch (such as ordinals of one occasion, or paired bounds of one bounded interval).
...
- For current aggregate-metric or threshold claims, `expectedEvidenceProfile.primaryMetric` must name the single decisive current metric that directly answers the claim. Use the closest authoritative source-native umbrella wording available, and keep that same metric as the first entry in `expectedMetrics`.
...
- **Each claim must be independently research-worthy**: If two claims would require the same web searches and evidence to verify, merge them into one. This merge rule NEVER overrides the claim-level bundling prohibition above: when the input explicitly applies a shared predicate or relation to multiple independently verifiable proposition units that could resolve differently, keep those units separate even if some searches or sources overlap.
...
17. **Claim-level bundling audit (MANDATORY).**
    Fail validation when any thesis-direct claim still depends on two or more independently verifiable sub-propositions, even if the extractor returned other thesis-direct claims that are valid. This includes coordinated-branch cases: when the input ties one act/state to multiple coordinated branch events under one shared temporal or conditional relation, fail validation if those branch events could independently be verified, falsified, or dated. The same rule applies when the coordination is written as one conjunctive clause with a shared verb phrase: if the coordinated members are distinct institutions, actor groups, proceedings, decision gates, target objects, or outcomes with separate possible timelines or results, they are not a single atomic branch. It also includes shared-predicate evaluative cases: when one claim applies a rule-governed or evaluative predicate to multiple distinct target objects, processes, decisions, or outcomes that could resolve differently, fail validation unless each independently verifiable proposition unit is preserved separately. It also includes comparison-side bundling: when the truth of one bundled claim depends on one proposition about the named/current side plus a separate proposition about the comparator/reference side (for example, current-versus-historical, current-versus-threshold, or named-side-versus-rest-of-comparison-class comparisons), fail validation if those sub-propositions could independently be verified or falsified. In all such cases, the bundled claim is non-atomic and `rePromptRequired` must be true. Do NOT approve a bundled claim merely because it appears to preserve an "expanded scope"; preserving scope by bundling independently verifiable proposition units is still a contract failure. Do NOT fail on this rule for pure rank/order/composite propositions whose coordinated phrase cannot resolve differently branch-by-branch.

    **Surface-pattern audit (MANDATORY, applies in addition to the rationale-based audit above):** Independently of any rationale you might form about whether the coordinated members are "really" one evaluative dimension, run this mechanical check on each thesis-direct claim's surface form. A claim of the form `[coordinated noun phrase] [verb] [evaluative predicate or standard]` is multi-evaluand whenever the coordinated noun phrase contains two or more separable target-object units — for example distinct events, distinct phases, distinct decisions, distinct actions, distinct measurements, distinct outputs, distinct instances, distinct outcomes, distinct components, or distinct stages — that could individually pass or fail the predicate. Apply the test mechanically: enumerate each unit inside the coordinated subject, then ask whether each unit could resolve to a different truth value against the predicate. If yes, the claim is bundled and `rePromptRequired` MUST be true. Do NOT accept the following rationalizations as reasons to approve a bundled claim: "they share the same standard", "they cover one assessment dimension", "they preserve expanded scope", "they are part of one process", "they form one institutional whole", or "the predicate applies to both equally". The shared predicate is what gets PRESERVED across the split claims; it is not what justifies bundling them. Composite-not-split exception applies ONLY to inseparable ordered or paired phrases whose truth cannot differ branch-by-branch (such as ordinals of one occasion, or paired bounds of one bounded interval).
...
9. **Modifier fusion across split propositions.** If the main act/state carries a truth-condition-bearing modifier that stays in scope across coordinated branches or across the split propositions created by the comparison structure, each split claim should preserve that modifier fused to the same main act/state. A single bundled claim does NOT satisfy this if decomposition is otherwise required.
...
- A non-fallback verdict must cite at least one direct evidence item in `supportingEvidenceIds` or `contradictingEvidenceIds`. If no direct evidence item supports either direction, you MUST emit an explicit insufficient-evidence outcome with ALL of the following: (1) both `supportingEvidenceIds` and `contradictingEvidenceIds` arrays empty, (2) `confidence` strictly below 25 (INSUFFICIENT tier — not LOW, not MEDIUM), (3) `truthPercentage` between 40 and 60 (no strong directional claim), (4) `reasoning` explicitly states that no direct evidence is available and identifies which contextual or background material informed the partial assessment. Do NOT assign a directional truth value from contextual material alone, and do NOT emit a LOW-confidence-or-higher verdict with empty directional arrays.
...
- Directness requires a dimensional bridge to the target. The bridge can take either form: **(a) target-bridge** — the source evaluates the same target object on the same evaluated dimension, OR **(b) adjudicator-bridge** — the source, issued by an authoritative external evaluator (for example: an international body, regional human-rights body, treaty-body rapporteur, or independent monitoring institution), documents the responsible adjudicator's, panel's, or institution's recurring practice on the same procedural or substantive integrity dimension the claim assesses (for example: impartiality, defense access, transparency, secrecy, investigation scope, or speed). A source that mentions the same broad target while evaluating a different episode, actor relationship, institutional controversy, remedy path, or outcome remains `"contextual"` unless (a) or (b) holds.
- Overlap in actors or institutions alone is insufficient. Shared standards, shared adjudicators evaluated on a different dimension than the claim assesses, shared institutions evaluated in an unrelated episode without dimensional match, or shared political/legal background do not by themselves create a bridge.
- The adjudicator-bridge (b) does NOT apply to advocacy organizations, partisan commentary, opinion pieces, foreign-government assessments (which remain `foreign_reaction`), or sources whose substantive content is reaction rather than evaluation. It applies only when the evaluator is a recognized independent monitoring or human-rights body assessing institutional conduct on the same dimension as the claim.
...
9. **Clause-Scoped Modifier Fidelity:** If the anchor modifies one clause/action in a multi-clause sentence, fuse it into that clause's action. Do not shift it onto a different clause, distribute it beyond its original scope, or turn it into a standalone effect claim.
10. **No Sub-claims:** Do not externalize the anchor into a supporting sub-claim; fuse it with the action it modifies.
11. **No New Inference:** Repair the existing claim set only. Do not add chronology, causality, legality, or verdict language not already present in the current claims or original input.
