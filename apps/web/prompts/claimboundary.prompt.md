...
- For claims about whether a target process, proceeding, decision, verdict, or outcome satisfied legality, procedure, fairness, or another rule-governed standard, `expectedEvidenceProfile` must carry BOTH route classes needed for balanced verification:
  1. target-path record routes: the direct record of the evaluated process/decision itself, including procedural steps, decision artifacts, safeguards, remedies, review paths, evidence access, hearing/publicity records, reasoned judgment, or equivalent source-native records;
  2. standards/challenge routes: the governing standard, external monitor, appeal, dissent, objection, or independent assessment routes that could support or challenge compliance.
  Do NOT let the profile collapse entirely into external criticism, broader institutional controversy, standards manuals, or concern reports while omitting the target process's own record and safeguards. Also do NOT let it collapse entirely into internal process records while omitting the standards/challenge route. Keep these as evidence routes inside `expectedEvidenceProfile`, not as extra AtomicClaims.
...
0a. **Rule-governed standard profile gate.**
   Before approving any thesis-direct claim about whether a target process, proceeding, decision, verdict, or outcome satisfied legality, procedure, fairness, or another rule-governed standard, inspect that claim's `expectedEvidenceProfile`. The profile must preserve both: (1) a route to the target path's own record, procedural safeguards, remedies, review path, decision artifact, evidence-access/hearing/publicity record, or equivalent source-native process record; and (2) a route to the applicable standard, external monitor, appeal, dissent, objection, or independent assessment that can support or challenge compliance. If the profile only names standards bodies, concern reports, broader institutional controversy, or criticism routes without a target-path record/safeguard route, set `rePromptRequired: true`. If it only names internal records without a standards/challenge route, set `rePromptRequired: true`. Keep the repair inside `expectedEvidenceProfile`; do not add new claims for profile routes.
...
- For these rule-governed standard claims, keep the pro/con query set balanced at the route level: at least one supporting-evidence query should target target-path safeguards, procedural protections, remedies, review paths, evidence access, public-hearing/publicity records, reasoned decision artifacts, or equivalent process records; at least one refuting-evidence query should target target-path violations, successful challenges, operative criticisms, or standards-based objections. Do NOT spend all query slots on concerns, dissent, external criticism, or collateral controversy when no query seeks the target path's own safeguard/record route.
...
For `"operative_finding"` and `"direct_record"`, `directnessJustification` must name the evaluated target path from the claim, not just the same institution, judge, actor, inquiry family, or legal system. For rule-governed standard claims, a directional basis is valid only when the evidence identifies a formal step, safeguard, remedy, review path, decision artifact, evidence-access/hearing/publicity record, reasoned judgment, or operative standards outcome of the directly evaluated target itself, or explicitly bridges the criticized/supportive mechanism to that target. If the item instead reports concerns about the same institution, the same decision-maker, earlier inquiries, parallel investigations, broader institutional practices, or unresolved allegations without that target-path bridge, use `"collateral_context"` or `"concern_or_position"` and set `claimDirection` to `"neutral"`.
