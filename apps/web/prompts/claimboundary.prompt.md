...
- **ONLY for comparative ecosystem claims** — claims about whether an activity is systematic, institutionalized, organized, or otherwise established within one jurisdiction or entity relative to another:
  - These rules do NOT apply when the decisive evidence is a present-state metric, ranking, threshold comparison, or other current measurable value.
...
- **Single-claim bundling prohibition:** If one act/state is asserted to occur before, after, because of, or subject to multiple coordinated branch events and those branches could independently occur, fail, or be dated, you MUST return separate atomic claims for those branches rather than one bundled claim. Preserve the shared relation in each branch claim. A conjunctive clause with one shared verb phrase does not override this rule: if the coordinated members are distinct institutions, actor groups, proceedings, or decision gates with separate possible timelines or outcomes, split them. Exception: keep one claim only when the coordinated phrase is an inseparable rank/order/composite proposition whose truth cannot differ branch-by-branch.
...
- **ONLY for comparative ecosystem claims** — claims about whether an activity is systematic, institutionalized, organized, or otherwise established within one jurisdiction/entity relative to another:
  - These rules do NOT apply when the decisive evidence is a present-state metric, ranking, threshold comparison, or other current measurable value.
...
- non-atomic single-claim bundling of multiple independently verifiable coordinated branches
...
17. **Single-claim bundling audit (MANDATORY).**
    When the input ties one act/state to multiple coordinated branch events under one shared temporal or conditional relation, fail validation if the extractor returns only one thesis-direct claim and those branch events could independently be verified, falsified, or dated. The same rule applies when the coordination is written as one conjunctive clause with a shared verb phrase: if the coordinated members are distinct institutions, actor groups, proceedings, or decision gates with separate possible timelines or outcomes, they are not a single atomic branch. In that case, the bundled claim is non-atomic and `rePromptRequired` must be true. Do NOT fail on this rule for pure rank/order/composite propositions whose coordinated phrase cannot resolve differently branch-by-branch.
...
- **ONLY for comparative ecosystem claims** — claims about whether an activity is systematic, institutionalized, organized, or otherwise established:
  - These rules do NOT apply when the decisive evidence is a present-state metric, ranking, threshold comparison, or other current measurable value; use the present-state metric rules above instead.
