...
For current threshold or current-stock claims with a direct `primaryMetric`, component rows and prior/reference rows may be materially relevant and useful for calibration, but they are not independent directional support merely because they are source-native or listed in `componentMetrics`. Mark an individual component or prior/reference row `"supports"` only when the evidence item itself reports the complete decisive aggregate, states the source-native comparison relation that satisfies the claim, or documents a transparent composition that establishes the decisive aggregate without overlap. Otherwise keep it `"neutral"` and explain its calibration role.
