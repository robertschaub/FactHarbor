...
- First, classify the original input. **Before applying the four classification types below, check the overrides below in order:**
  ...
  - **Coordinated branch rule:** When a shared temporal or conditional relation links one act/state to multiple coordinated branches (for example, one action said to occur before or after two different decisions, approvals, ratifications, adjudications, or other branch events), treat the input as `multi_assertion_input` whenever those branches could independently be verified, falsified, or dated on their own. In that case, split into one claim per branch, preserve the shared anchor in each returned claim, and do NOT keep the unsplit whole sentence alongside branch claims. A conjunctive clause such as "A and B decided" does NOT count as a single atomic branch merely because it shares one verb phrase; if A and B are distinct institutions, actor groups, proceedings, or decision gates with separate possible timelines or outcomes, they are separate branches for decomposition. Keep it unsplit only when the coordinated phrase functions as one inseparable rank/order/composite proposition whose truth cannot differ branch-by-branch.
  - **Modifier-fusion rule for coordinated branches:** If the main act/state carries a truth-condition-bearing modifier and you split the sentence by coordinated branches, keep that modifier fused with the same main act/state in EVERY branch claim that remains inside its scope. Do NOT create one claim for the modified act/state and a separate claim for the branch chronology; the correct decomposition is branch-by-branch, with the in-scope modifier repeated in each branch claim.
  - **Coherent-axis rule for coordinated subjects and criteria:** When one predicate applies to multiple coordinated subjects, acts, decisions, or outcomes AND to multiple coordinated evaluation criteria, standards, frameworks, or dimensions, choose one coherent complete decomposition axis unless the input itself requires a finer split. A complete subject-axis split is valid when each returned subject/act/outcome claim preserves all in-scope criteria. A complete criteria-axis split is valid when each returned criterion/framework claim preserves all in-scope subjects, acts, decisions, or outcomes. Do NOT return a partial cross-product that combines some subject-criterion pairs while omitting another explicitly in-scope pair. If you choose cross-product granularity, include every explicitly in-scope combination or choose a complete broader axis instead.
...
22. **Complete-axis coverage for coordinated subjects and criteria (MANDATORY).**
    When the input applies one predicate to multiple coordinated subjects, acts, decisions, or outcomes and also to multiple coordinated evaluation criteria, standards, frameworks, or dimensions, evaluate the claim set by complete decomposition axis. A complete subject-axis split can preserve the contract if every returned subject/act/outcome claim carries all in-scope criteria. A complete criteria-axis split can preserve the contract if every returned criterion/framework claim carries all in-scope subjects, acts, decisions, or outcomes. Do NOT require a full cross-product merely because both axes are present, but do fail validation when the claim set mixes axes into a partial cross-product and omits an explicitly in-scope subject-criterion pairing. When failing this pattern, set `rePromptRequired: true` and state that the retry should choose one complete axis or include every in-scope pair.
