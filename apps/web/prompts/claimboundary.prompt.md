...
lastModified: "2026-04-19T15:05:00Z"
variables:
  - currentDate
  - analysisInput
  - originalClaim
...
- First, classify the input as one of:
  ...
  - **Exclusivity/uniqueness override:** If the input combines (a) a subject-specific proposition and (b) an assertion that no other member of the relevant comparison class shares that status, classify as `multi_assertion_input`. Extract at least those two claims, and extract additional claims only when the input itself contains further independently verifiable propositions. Do not force exactly two claims if the wording supports more. Do not create extra claims merely because verification requires multiple evidence routes. Each decomposed claim must be a genuine sub-assertion, not a verbatim copy of the full input.
  - **Ordering-rank guard:** Do NOT apply this override when the proposition is primarily about temporal, ordinal, or rank order. In those cases, preserve the ordering relation as part of the claim rather than decomposing it into a subject-level proposition plus a separate exclusivity proposition. After excluding this override, classify the input under the normal rules. Only treat it as `multi_assertion_input` if the input itself contains an additional independent assertion.
...
- For comparative claims about whether an activity is systematic, institutionalized, organized, or otherwise established within one jurisdiction or entity relative to another, keep roughClaims close to the original proposition and make `searchHint` point toward decisive institutional evidence routes (for example directory/registry presence, certified or affiliated members, dedicated units or desks, recurring organizational outputs, or governance/monitoring structures) rather than only the broad activity label.
...
- First, classify the original input. **Before applying the four classification types below, check the overrides below in order:**
  ...
  - **Exclusivity/uniqueness override:** If the input combines (a) a subject-specific proposition and (b) an assertion that no other member of the relevant comparison class shares that status, classify as `multi_assertion_input`. Extract at least those two claims, and extract additional claims only when the input itself contains further independently verifiable propositions. Do not force exactly two claims if the wording supports more. Do not create extra claims merely because verification requires multiple evidence routes. Each decomposed claim must be a genuine sub-assertion — not a verbatim copy of the full input.
  - **Ordering-rank guard:** Do NOT apply this override when the proposition is primarily about temporal, ordinal, or rank order. In those cases, preserve the ordering relation as part of the claim rather than decomposing it into a subject-level proposition plus a separate exclusivity proposition. After excluding this override, classify the input under the normal rules below. Only treat it as `multi_assertion_input` if the input itself contains an additional independent assertion.
...
- When a comparative claim asks whether an activity is systematic, institutionalized, organized, or otherwise established within one jurisdiction/entity relative to another, `expectedEvidenceProfile` must name the decisive institutional existence signals on BOTH sides (for example: directories/registries, certified or affiliated members, dedicated teams/desks, recurring official or organizational outputs, governance/monitoring frameworks, or formal cross-organization arrangements). Do NOT reduce such a claim to broad topical mentions that merely discuss the activity.
...
- **Scope-of-truth rule:** Assess truth ONLY against the proposition the AtomicClaim actually states. If a claim asserts a temporal sequence ("A happened before B"), truthPercentage answers only whether A preceded B. Any implication beyond that — such as "therefore improper," "therefore unconstitutional," "therefore not yet legally effective," or "therefore procedurally irregular" — belongs in `misleadingness` or `reasoning`, NOT in `truthPercentage`. Do NOT expand the proposition being judged beyond what the claim text says.
- Do NOT uphold an `UNVERIFIED` verdict solely because no single source printed an explicit umbrella figure when aligned source-native component figures already allow a well-supported synthesis of the decisive quantity or threshold comparison. In that situation, explain the compositional reasoning and reserve `UNVERIFIED` for genuinely missing, stale, inconsistent, or non-combinable evidence.
...
2. **Structural Integrity:** Do not change any existing claim `id`.
