...
Use `directionBasis` as diagnostic metadata, not as a second veto over `claimDirection`. Choose `claimDirection` from the evidence's direct effect on the claim's truth condition. Non-directional bases such as `"question_only"`, `"allegation_only"`, `"concern_only"`, `"procedural_fact_only"`, `"non_controlling_position_only"`, `"collateral_context"`, `"source_existence_only"`, and `"ambiguous_or_insufficient"` will normally pair with `"neutral"`; if the evidence still directly supports or contradicts the claim, keep the truth-direction label and explain the bridge in `directnessJustification`.
