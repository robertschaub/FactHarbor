...
- If input is **ambiguous_single_claim**:
  ...
  - For broad evaluative predicates, use the same discipline: "[Subject] [same evaluative predicate] in terms of [dimension]" or the input-language equivalent. Do not append an explanatory proxy clause after the neutral dimension label; mechanisms, thresholds, causal stories, or metric details belong in `searchHint` or later verification framing, not rough-claim text.
...
- If the input is an **ambiguous_single_claim**:
  ...
  - **No explanatory proxy tails:** A dimension claim is not faithful merely because it starts with the original evaluative predicate. The `statement` must not append a dash, colon, relative clause, causal clause, complement clause, or equivalent explanatory tail that redefines the dimension as a specific mechanism, threshold, causal chain, failure mode, viability test, contribution test, or metric result. Stop the `statement` after the neutral dimension qualifier whenever possible. Put candidate mechanisms, measurable routes, thresholds, and evidence strategy in `expectedEvidenceProfile`, search queries, or evidence scopes.
...
3. **Proxy drift is not allowed unless explicit in the input.**
   ...
   A claim can still materially drift when it repeats the original predicate at the start but then appends an explanatory tail that makes a narrower proxy proposition the thing being verified. Treat a dash, colon, relative clause, causal clause, complement clause, or equivalent explanatory tail as material proxy drift when it redefines the dimension into a mechanism, threshold, causal chain, failure mode, viability test, contribution test, or metric result that was not explicit in the input.
