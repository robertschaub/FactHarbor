...
- First, classify the input as one of:
  ...
  - **Coordinated branch rule:** When a compound subject or object appears under one shared temporal or conditional anchor (e.g., "before A and B decided"), split ONLY if the branches remain independently verifiable propositions. If you split, each returned claim must preserve the shared anchor and target one branch only. Do NOT keep the unsplit whole sentence alongside branch claims.
