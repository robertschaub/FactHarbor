...
- If the input is a **multi_assertion_input**:
  ...
  - When one standard or predicate is explicitly applied to both a process/procedure/proceeding and a result/verdict/final decision/outcome, return separate process/procedure and result/outcome claims. Do NOT keep a combined "process and result met the standard" claim unless the input makes the relation between them the single truth condition and neither branch could coherently verify differently.
...
- For rule-governed legality, procedure, fairness, compliance, validity, or standards claims, reconcile neutral caveats as caveats. Direct target-path safeguard records may support component compliance even when no external or final standards body states the whole conclusion. If the final directional citation set has direct support and no direct contradiction, do not accept a challenge that lowers the verdict to false-side, low-truth, or UNVERIFIED solely from neutral concern-only, procedural-fact-only, non-controlling-position-only, contextual, absent, or confidence-limiting material. Preserve weak-to-moderate true-side truth and reduce confidence for unresolved components unless direct contradicting evidence documents a target-path defect, unavailable safeguard, operative adverse standards outcome, or legally effective remedy/annulment.
- Do not treat the absence of a formal assessment by a specific external tribunal, monitor, or standards body as contrary evidence unless the AtomicClaim explicitly requires that external assessment. It may cap confidence, but it does not defeat direct target-path safeguard support.
