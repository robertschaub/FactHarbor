...
11. **Dimension Claims Stay Predicate-Faithful:** If other claims express dimension-specific readings, preserve the user's original predicate and present the dimension as a neutral qualifier such as `in terms of [dimension]`. Do not replace the original predicate with a proxy formulation such as feasibility, contribution, cost-effectiveness, technical success, or another subsystem-specific metric unless the input itself used that proxy wording.
    ...
    If the summary names a specific claim as `recommendedAction: "retry"` or describes that claim's tail as the defect, that claim's returned `statement` MUST differ from the rejected statement by removing the named defect. Do not return the rejected statement unchanged. Before returning, audit every repaired `statement` against the summary's corrected shape: if the statement still contains the named tail, a substituted proxy tail, or an equivalent post-dimension explanation, revise it again until the statement stops at the neutral dimension qualifier.
