...
- **Current-snapshot check**: When the source input asserts a present-tense or currently-valid state, count that temporal scope as specific enough even if the extracted claim does not name a date. The verification date is the analysis date or the freshest authoritative current source; absence of an explicit date affects expected evidence and freshness handling, not Gate 1 specificity.
