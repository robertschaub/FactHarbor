...
- If input is **ambiguous_single_claim**:
  ...
  - For broad evaluative predicates, use the same discipline: "[Subject] [same evaluative predicate] in terms of [dimension]" or the input-language equivalent. Do not append an explanatory proxy clause after the neutral dimension label; the rough claim should end at the dimension label unless the input itself contains the additional condition, mechanism, threshold, causal story, or metric. Those details belong in `searchHint` or later verification framing, not rough-claim text.
  - **Dimension-vs-reason distinction:** A dimension-specific rough claim evaluates the original predicate within a neutral dimension. It must not replace an absolute, categorical, or broad evaluative predicate with a narrower condition, mechanism, consequence, prerequisite, or metric that would merely be evidence for, or a possible reason supporting, that predicate. If the only faithful way to express a proposed dimension is to change the predicate into a narrower condition, do not emit that dimension as a separate rough claim; keep the condition as verification framing.
...
- **Language detection**: Detect the primary language of the input text and return the BCP-47 language code (for example, a two-letter or region-qualified language code). Base this on the input text itself, not on entity names within it.
- **Geography inference**: Return a country code ONLY when the claim describes events, conditions, or measurements **occurring within** a specific place. **Priority rule**: if the claim explicitly names a sub-national geographic entity (city, district, canton, province, region, county, municipality, or other administrative unit), derive the country from that entity — the input language is irrelevant to this determination. Return `null` when a country is merely the subject or actor but evidence would come from international sources. Do NOT infer geography from input language, institutions, or cultural associations. When in doubt, return `null`.
...
```json
{
  ...
  "detectedLanguage": "string — BCP-47 code of input language",
  "inferredGeography": "string | null — ISO 3166-1 alpha-2 country code inferred from claim content, or null if not geographically specific"
}
```
...
- First, classify the original input. **Before applying the four classification types below, check the overrides below in order:**
  ...
  - **Process/result assessment override:** If the input's **own wording separately names** both a target process/procedure/proceeding and its result, verdict, final decision, final determination, or outcome as covered by the same rule-governed, legal, procedural, fairness, validity, compliance, or similar evaluative standard, classify as `multi_assertion_input`. Split the process/procedure assessment and the result/outcome assessment into separate thesis-direct AtomicClaims when they could receive different directional verdicts, even if they share the same entity, institution, standard, or sentence. Do not infer a separate result/outcome claim merely because a broad process word could include later phases; the result/outcome branch must be explicitly named as a target of that predicate. Preserve any additional separately coordinated standard or predicate as its own claim rather than merging it into a process/result bundle.
...
- If the input is a **multi_assertion_input**:
  ...
  - When one standard or predicate is explicitly applied to both a process/procedure/proceeding and a separately named result/verdict/final decision/outcome, return separate process/procedure and result/outcome claims. Do NOT keep a combined "process and result met the standard" claim unless the input makes the relation between them the single truth condition and neither branch could coherently verify differently. Do not create an extra result/outcome claim for a predicate that the input applied only to a broad process/proceeding noun.
...
- If the input is an **ambiguous_single_claim**:
  ...
  - For comparative efficiency, optimization, or resource-use predicates, treat distinct measurement windows or system boundaries as separate dimensions when those windows could produce different factual answers (for example, full-pathway vs. use-phase-only vs. conversion-stage efficiency). Preserve the original comparative predicate in each dimension claim; vary only the dimension qualifier. A short neutral measurement-window or system-boundary label may be part of that qualifier when it merely names the boundary being assessed; it must not add a result, threshold, causal mechanism, loss narrative, or adjacent performance proxy.
  ...
  - **No explanatory proxy tails:** A dimension claim is not faithful merely because it starts with the original evaluative predicate. For a broad evaluative predicate, any dash, colon, relative clause, causal clause, complement clause, or equivalent explanatory tail after the neutral dimension qualifier is material proxy drift by default unless that added condition, mechanism, threshold, causal chain, failure mode, viability test, contribution test, or metric result is explicit in the input. Stop the `statement` after the neutral dimension qualifier. Put candidate mechanisms, measurable routes, thresholds, and evidence strategy in `expectedEvidenceProfile`, search queries, or evidence scopes. Do not keep a tail because it is consistent with the predicate or explains what the predicate looks like in that dimension.
  - **Dimension-vs-reason distinction:** A thesis-direct dimension claim must remain the original predicate evaluated within a neutral dimension. Do not replace an absolute, categorical, or broad evaluative predicate with a narrower condition, mechanism, consequence, prerequisite, or metric that would merely be evidence for, or a possible reason supporting, that predicate. If the only faithful way to express a proposed dimension is to change the predicate into a narrower condition, do not emit that dimension as a separate claim; keep the condition in `expectedEvidenceProfile`, search strategy, or later evidence context.
  - For every thesis-direct dimension claim, preserve the original predicate's polarity, intensity, and scope in the input language. A preserved broad primary claim does not license sibling dimension claims that rewrite the predicate into sufficient-condition claims.
...
- `expectedEvidenceProfile` may use preliminary evidence to learn source-native labels, metric classes, route families, publication families, and methodology names. It must not turn preliminary evidence's observed result values into accepted profile targets unless the original input itself contains those values. Evidence-discovered measured values, prior-period figures, percentage changes, and component counts belong in evidence items, not as literal `expectedMetrics` or `componentMetrics`.
- Input-authored numeric anchors may remain literal in the profile because they define the claim. Evidence-discovered values may be described only as abstract metric classes, freshness windows, comparator roles, or source routes. A date or edition label may appear when it identifies a source route to retrieve, but not as an already-accepted factual result.
...
- When preliminary evidence already reveals a recurring official statistics series, program name, archive path, or source-native umbrella phrase for the decisive quantity, copy that source-native label or route wording into `expectedEvidenceProfile` verbatim instead of paraphrasing it as a generic "all categories" or "overall total" label. Do not copy the source's observed measured result, prior-period comparator value, percentage change, or component count as an expected profile value unless that value was already in the input. Keep this enrichment inside `expectedEvidenceProfile`, not in the claim statement.
...
2. **Dimension qualification is allowed, but tail narrowing is not.**
   A claim may qualify a broad evaluative predicate or broad comparative efficiency/resource-use predicate by adding a neutral dimension qualifier such as "in terms of [dimension]" when that dimension is a natural reading of the input. Qualification stops at the dimension label. For comparative efficiency/resource-use claims, a short measurement-window or system-boundary label, including a parenthetical alias that merely names the same boundary, can be part of the neutral dimension qualifier. Do not treat an explanatory condition, mechanism, threshold, causal story, or metric added after that label as part of the allowed dimension qualifier unless that added content is explicit in the input.
   The neutral dimension label itself is not an explanatory tail. Do not fail a claim merely because the dimension label names the natural assessment dimension or measurement boundary; fail only when the statement replaces the original predicate, or adds a condition, mechanism, threshold, causal story, consequence, viability/contribution test, result, loss narrative, or metric after the dimension label.
...
3. **Proxy drift is not allowed unless explicit in the input.**
   ...
   A claim materially drifts when it repeats the original predicate at the start but then appends an explanatory tail that makes a narrower proxy proposition the thing being verified. For broad evaluative dimension claims, treat a dash, colon, relative clause, causal clause, complement clause, or equivalent explanatory tail after the neutral dimension qualifier as material proxy drift by default unless the added condition, mechanism, threshold, causal chain, failure mode, viability test, contribution test, or metric result is explicit in the input.
   If the statement ends immediately after the neutral dimension label, do not reinterpret that label as the explanatory tail. A label that distinguishes natural readings of the original predicate is allowed claim content; mechanisms, thresholds, metrics, and sufficiency tests belong outside the statement.
   For broad comparative efficiency/resource-use dimension claims, do not reinterpret a concise measurement-window or system-boundary label as an explanatory tail when it merely names the dimension. It becomes material drift only if the added wording asserts a result, threshold, causal mechanism, loss narrative, sufficiency condition, or adjacent performance proxy not explicit in the input.
   Do not approve such a tail on the rationale that it is consistent with the predicate, specifies what the predicate looks like, or identifies how the predicate would manifest in that dimension. Those are evidence strategies, not claim content.
   Also fail sufficient-condition drift: a dimension claim may not replace the original broad predicate with a narrower condition, mechanism, consequence, prerequisite, or metric that would merely serve as evidence for, or a possible reason supporting, that predicate. A preserved broad primary claim does not cure material proxy drift in sibling thesis-direct dimension claims.
...
9. **Be conservative about retries.**
   Request a retry only when the contract drift is material enough that downstream search and verdicting would likely analyze a different proposition. Broad-evaluative explanatory-tail drift meets this threshold by default, because downstream search and verdicting will usually analyze the added tail rather than the original broad predicate.
...
- **Broad-evaluative tail override:** For thesis-direct dimension claims derived from a broad evaluative predicate, an explanatory tail after the neutral dimension qualifier that adds a non-input condition, mechanism, threshold, causal story, consequence, viability/contribution test, or metric MUST be assessed as `proxyDriftSeverity: "material"` and `recommendedAction: "retry"`. The top-level `inputAssessment.preservesOriginalClaimContract` MUST be `false` and `inputAssessment.rePromptRequired` MUST be `true` if any such claim exists. This override applies even if the tail is consistent with the original predicate, specifies what the predicate looks like, or would otherwise be borderline under the conservative retry rule or the "substantially different proposition" field guidance.
- **Neutral-dimension acceptance guard:** If a thesis-direct dimension claim preserves the original broad evaluative predicate and stops immediately after a short neutral dimension qualifier, do NOT classify the qualifier itself as proxy drift or as the explanatory tail. In that case set `usesNeutralDimensionQualifier: true`, `proxyDriftSeverity: "none"` or `"mild"`, and `recommendedAction: "keep"` unless another material contract defect exists.
- **Comparative/system-boundary acceptance guard:** If a thesis-direct dimension claim preserves the original broad comparative efficiency/resource-use predicate and stops after a short neutral measurement-window or system-boundary qualifier, do NOT classify that qualifier or a parenthetical alias naming the same boundary as proxy drift or as the explanatory tail. In that case set `usesNeutralDimensionQualifier: true`, `proxyDriftSeverity: "none"` or `"mild"`, and `recommendedAction: "keep"` unless another material contract defect exists. This guard does NOT permit result claims, mechanism claims, thresholds, loss narratives, sufficiency tests, or adjacent performance proxies after the qualifier.
...
5. **Process-result bundles.** When the original input explicitly names both a target process/procedure and its result, final decision, final determination, or outcome as covered assessment targets, do not preserve the bundle merely because both use the same standard or entity. If the process could satisfy the standard while the result/outcome does not, or vice versa, treat them as input-authored separate truth conditions. Do not infer a result/outcome branch from a predicate applied only to a broad process/proceeding noun; the result/outcome must be separately named in the input under that same predicate.
6. **Seed, do not decide.** Proposed subclaims are candidate seeds for Pass 2 re-extraction. They are not final accepted claims and must not drop input-authored proposition units.
7. **Dimension-decomposition caution.** If the accepted set contains dimension-qualified variants of a single comparison, process, or standard, audit each claim internally. Do not split one dimension-qualified claim merely because that dimension can be measured through aggregate, component, sub-process, safeguard-specific, requirement-specific, or mechanism-specific evidence. Those are evidence routes, not input-authored proposition units, unless the original input explicitly asserts them as separate verdictable claims.
8. **Generated wording is not input authorship.** Do not treat component lists, submetrics, examples, safeguards, requirements, mechanisms, or outcome categories introduced by the extractor's accepted claim text as `explicit_subpropositions`. A sub-proposition is input-authored only when the original input itself names that target path or branch as a separate truth condition.
9. **Input-authored split basis.** Set `inputAuthoredSplitBasis: "explicit_subpropositions"` only when the original input itself asserts the proposed sub-propositions as separate truth conditions. Use `"derived_submetrics"` when the proposed split comes from measurement methods, component metrics, background mechanisms, broad standards, requirement classes, safeguards, or evidence routes inferred during analysis. High-confidence repair requires `"explicit_subpropositions"`.
10. **Confidence discipline.** Use `splitConfidence: "high"` only when the split is required by the directional-verdict test, has `inputAuthoredSplitBasis: "explicit_subpropositions"`, and the proposed seeds preserve the original input's thesis-central proposition units. Do not downgrade to `"medium"` solely because the sub-propositions share the same standard, entity, or broad process; shared criteria can still require high-confidence repair when the original input authored distinct target paths. Use `"medium"` for plausible but uncertain split pressure, and `"low"` for weak observations.
11. **Preserved relation tracking.** When you considered a possible split but preserved the claim because it is a relation, comparison, temporal, dimension-qualified, or whole-process claim, record it in `preservedRelationClaims`.
12. **No topic-specific assumptions.** Apply the same criteria in any language and any subject area. Preserve the input language in proposed subclaim seeds where possible.
...
- If `${freshnessRequirement}` is `current_snapshot`, at least one query must target a source-native or official route with `retrievalLane = primary_direct` or `navigational`.
- For `current_snapshot`, aim the current-route query at the newest source-native publication, update stream, archive month/period, dashboard, or data artifact available on or before `${currentDate}`. Treat the newest routine current-statistics route and the latest complete official or institutional publication/data artifact as complementary when either may carry the decisive value. Use `freshnessWindow: none` for latest complete source-native publications, tables, annexes, archives, or recurring data artifacts when provider date filtering could exclude the decisive artifact; use `freshnessWindow: w` or `m` for live update streams, current dashboards, newly published material, or source routes where recent-provider filtering is expected to improve retrieval. Do not exclude the latest complete source-native release solely because it is periodic or retrospective if the current route may expose only partial, component, or navigational evidence.
- For `current_snapshot`, when query budget allows more than one query, return separate queries for (a) the newest current/live source-native route and (b) the latest complete source-native artifact. Outside refinement, if returning only one query, prioritize the newest current route; do not return only a fixed past reporting period, archive entry, or edition unless the input, profile, distinct-events metadata, or existing evidence establishes that fixed route as the freshest available decisive route.
...
- When an overview, landing page, current dashboard, or component source may not expose the direct `primaryMetric` or umbrella total, reserve one query for the latest complete source-native official/institutional publication, data artifact, annex, table, or file likely to carry the aggregate value. Keep this route concise and source-native; do not invent an exact date, month, edition label, or page label unless the claim, profile, distinct-events metadata, or existing evidence already names it.
...
- When the claim is explicitly about a present-state, current, live, latest-status, or as-of-current-time condition in any language, prioritize the newest source-native route intended to reflect that current state before falling back to historical or retrospective summaries.
...
- For rule-governed standard claims, include at least one query for broad target-specific standards/compliance discovery using neutral safeguard, record, compliance, assessment, or standards wording from the claim/profile; do NOT make every query an alleged-defect, violation, criticism, dissent, official-docket, or artifact-only route.
- When query budget or downstream truncation may leave only one returned query for a rule-governed standards claim, prioritize the directly evaluated target's source-native safeguard/process record over external standards commentary, broad controversy, or alleged-defect discovery. Standard-definition and commentary routes are secondary unless they are also likely to surface target-path safeguard records.
...
- When `iterationType` is `refinement`:
  ...
  - For `current_snapshot` claims with `expectedEvidenceProfile.primaryMetric`, the first executable refinement query should target the latest complete source-native official/institutional publication, data artifact, table, annex, or recurring report likely to carry the complete `primaryMetric`, with `freshnessWindow: none`. Do not spend the first refinement query on a generic current landing page, live dashboard, update stream, or component-only route unless that route itself is likely to expose the complete decisive metric.
  - Use a non-`none` `freshnessWindow` when the claim appears to depend on live, newly published, or provider-date-sensitive primary material.
  - Use `freshnessWindow: none` when the best route is a latest complete source-native publication, archive entry, data artifact, table, annex, or recurring report that may sit outside a narrow provider date window.
  ...
  - If current official evidence is still incomplete, target the newest source-native page, complete publication, artifact, table, annex, or data file that can close the missing metric gap rather than repeating only the same current landing page, partial component route, or over-specific dated route.
...
- If a current/present aggregate claim is represented mainly by partial components, flow metrics, source inventories, landing pages, or report-existence evidence without the direct aggregate value, pivot the next queries toward the latest complete source-native publication, official/institutional data artifact, table, annex, or data file for that aggregate. Pair it with the source-native archive or recurring-series path when needed, and avoid adding precise month/day labels unless existing evidence already established them.
...
- `claimDirection`: How this relates to the claim (`"supports"`, `"contradicts"`, or `"contextual"`). Use `"contextual"` for non-directional evidence at extraction time; downstream persistence normalizes that value to neutral claim direction while retaining relevance and scope metadata.
...
- For current aggregate, standing-stock, or threshold claims, keep component-only or incomplete component arithmetic `contextual` unless the source content itself provides all components required by the source's stated methodology, the components share the same analytical window, and the source or item establishes that they are non-overlapping for the decisive aggregate. Do not classify partial components as `supports` or `contradicts` by arguing that each component is below the threshold, that missing addends are unlikely to close the gap, or that known magnitudes outside the cited source would be insufficient. Preserve those calculations and missing-addend caveats in `evidenceScope` instead.
...
- For threshold or operator comparisons, normalize grouped numerals before comparing: ignore digit-group separators such as spaces, thin spaces, apostrophes, and punctuation only when source-local notation or regular digit grouping makes them grouping separators between digit groups; never ignore punctuation that functions as a decimal marker. Preserve true decimal fractions. A value strictly above a greater-than threshold supports that threshold relation even when the positive margin is tiny; treat narrow margins as confidence, misleadingness, precision, or freshness caveats, not as opposite direction.
...
- `claimDirection`:
  ...
  - **Target-specific vs. comparator/precedent evidence for target-object claims — classify by what the evidence evaluates, not what topic it shares:**
    ...
    - "Same target" means the same named proceeding, process path, decision, verdict, final determination, or outcome being assessed by the claim. The same judge, panel, court, institution, jurisdiction, claimant, defendant group, investigation theme, event family, supporters, allies, or other parties are not enough by themselves. Broader institutional reports about related investigations, confidentiality practices, expression limits, precautionary measures, sanctions, or overlapping actors are collateral unless the item evaluates the exact target proceeding/path under the claim's standard or documents a concrete safeguard/defect in that target path.
    - For rule-governed standards claims, a source does not need to pronounce the final compliance answer before target-specific safeguard or defect records can be directional. When the claim or `expectedEvidenceProfile` identifies procedural safeguards, rights, remedies, reasoned decisions, public hearings, evidence access, witness participation, appeal paths, recusal/remedy review, or comparable standard components, evidence that directly records those target-path components may be `supports` or `contradicts` according to whether the component was provided, exercised, heard, considered, granted, unavailable, blocked, found deficient, annulled, remedied, or otherwise legally resolved for the directly evaluated target. Put scope limits and unresolved caveats in `evidenceScope`; do not demote the item to `contextual` merely because no international body or final standards authority states the whole conclusion.
    - Do NOT treat a denied, rejected, or upheld party request as a contradiction merely because the losing side alleged unfairness. It contradicts a standards claim only when the source states that the denial/rejection made a target safeguard unavailable, impaired a target right, violated the governing standard, produced a remedy/annulment/disqualification, or otherwise had a legally effective standards consequence. Otherwise it is `contextual` or a caveat.
    - Separate restrictive measures, custody conditions, sanctions, disciplinary acts, publicity limits, institutional-role facts, or adjacent litigation are `contextual` unless the source directly connects them to the target proceeding's defense rights, remedy availability, decision validity, or standards outcome.
    - Potential or apparent adjudicator bias, dual-role facts, victim/decision-maker overlap, court-composition controversy, party accusations, supporter accusations, or broad impartiality debate are not direct safeguard defects by themselves. Classify them as `contextual` unless the source reports an adopted recusal/disqualification decision, an operative finding that the adjudicator was legally barred, or a legally effective remedy/annulment tied to the target proceeding.
    - A dissent, minority vote, expert view, party motion, or non-binding statement that says a defense right was impaired is not controlling contradiction unless it was adopted by the tribunal or produced a legally effective target-path consequence. Otherwise it is `contextual` or a caveat.
    - Limited, narrow, short-deadline, clarification-only, low-probability, vote-count-limited, or unlikely-to-change-the-outcome appeal/remedy mechanics are not contradictions merely because they make reversal unlikely. They contradict only when the source itself connects the remedy path to no effective review under the applicable standard, states a violation of the governing fair-trial/procedural standard, or reports an operative target-path remedy/annulment for that defect. Do not infer a standards violation from an outcome-probability prediction.
...
- For threshold, lower-bound, upper-bound, rank, equality, or current-stock claims, distinguish directional support from certainty calibration. A current or time-appropriate value that barely satisfies the asserted relation still supports the claim, but a narrow margin is not near-certain proof by itself. If routine revision, snapshot lag, boundary choice, component composition, or source-family concentration could plausibly change a barely satisfied relation, keep the verdict true-side when direct evidence supports it, but do not use the `TRUE`/near-certain band unless the evidence also establishes a robust margin or stable repeated confirmation. In such narrow-margin cases, use a middle or weak true-side `truthPercentage` and lower `confidence`, and explain that the reduction calibrates robustness of the measured relation rather than reversing evidence direction.
- For `current_snapshot` threshold or current-stock claims, if the evidence pool's only complete decisive value comes from a prior reporting snapshot while current-route evidence is absent, neutral, component-only, or not fetchable, treat snapshot lag as a live calibration limit. Keep the verdict true-side when that prior value supports the relation, but avoid the `TRUE`/near-certain band unless the evidence also establishes a robust margin or repeated current confirmation.
...
- For rule-governed legality, procedure, fairness, compliance, validity, or standards claims, direct target-path safeguard records can verify component compliance even when no external or final standards body states the whole conclusion. If the claim-local direct evidence is one-sided support and the remaining material is neutral, contextual, concern-only, procedural-fact-only, non-controlling-position-only, absent, or confidence-limiting, do not move the verdict to a false-side, low-truth, MIXED, or UNVERIFIED result solely because unanswered components or concerns remain. Keep the truth at or above the weak true-side band and lower confidence for the unresolved caveats unless direct contradicting evidence documents a target-path defect, unavailable safeguard, operative adverse standards outcome, or legally effective remedy/annulment.
- Absence of a formal assessment by a specific external tribunal, monitor, or standards body is not itself contrary evidence for a standards claim. Treat that absence as a confidence limitation, not as a reason to make documented safeguard compliance unverifiable, unless the AtomicClaim explicitly requires that external assessment.
- When direct target-path support exists and direct contradiction is absent, absence of an external final assessment may lower confidence but should not by itself drive confidence into the system's unverifiable range. Reserve very low confidence for missing direct target-path evidence, material direct conflict, stale or invalid evidence, or citation-integrity failure.
...
- For rule-governed legality, procedure, fairness, compliance, validity, or standards claims, reconcile neutral caveats as caveats. Direct target-path safeguard records may support component compliance even when no external or final standards body states the whole conclusion. If the final directional citation set has direct support and no direct contradiction, do not accept a challenge that lowers the verdict to false-side, low-truth, MIXED, or UNVERIFIED solely from neutral concern-only, procedural-fact-only, non-controlling-position-only, contextual, absent, or confidence-limiting material. Preserve truth at or above the weak true-side band and reduce confidence for unresolved components unless direct contradicting evidence documents a target-path defect, unavailable safeguard, operative adverse standards outcome, or legally effective remedy/annulment.
- Do not treat the absence of a formal assessment by a specific external tribunal, monitor, or standards body as contrary evidence unless the AtomicClaim explicitly requires that external assessment. It may cap confidence, but it does not defeat direct target-path safeguard support and should not reduce truth into MIXED or UNVERIFIED when direct support is one-sided.
- When direct target-path support exists and direct contradiction is absent, absence of an external final assessment may lower confidence but should not by itself drive confidence into the system's unverifiable range. Reserve very low confidence for missing direct target-path evidence, material direct conflict, stale or invalid evidence, or citation-integrity failure.
- When reconciling threshold, lower-bound, upper-bound, rank, equality, or current-stock claims, treat a barely satisfied relation as a calibration issue rather than a binary certainty signal. If valid challenges identify a narrow margin, snapshot lag, boundary ambiguity, component-composition uncertainty, or lack of independent corroboration, reduce overconfident truth and confidence while preserving the supporting direction unless direct contradicting evidence defeats the relation. A small reduction from one near-certain score to another near-certain score is insufficient when the accepted challenges could plausibly flip the relation; move the result out of the `TRUE`/near-certain band unless the cited evidence establishes a robust margin or stable repeated confirmation.
- For `current_snapshot` claims, when accepted challenges show that current-route evidence is missing, neutral, component-only, or not fetchable and the support comes from a prior reporting snapshot with a narrow margin, treat that as a plausible flip risk by default. Do not answer with `TRUE`/near-certain merely because the prior snapshot technically satisfies the relation.
...
- When a low-truth, midpoint, or UNVERIFIED verdict discusses neutral concern-only, procedural-fact-only, non-controlling-position-only, contextual, absent, or confidence-limiting material as if it materially defeats one-sided direct support, flag the issue as a truth/calibration mismatch. Do not instruct the repair step to move such neutral material into `contradictingEvidenceIds`; the repair should either raise truth into a weak-support range or explain why a direct contradicting item exists.
...
- If the direction issue arose because neutral concern-only, procedural-fact-only, non-controlling-position-only, contextual, absent, or confidence-limiting material was treated as defeating one-sided direct support, do not reclassify that neutral material as contradiction. Keep it in caveats/limitations, keep direct supporting IDs in `supportingEvidenceIds`, and set truth at or above the weak true-side band unless the evidence pool contains direct contradicting items.
- Pipeline band guide: `LEANING-TRUE` starts at 58. Values from 43 through 57 are middle-band (`MIXED` or `UNVERIFIED`, depending on confidence). Therefore, for a one-sided direct-support repair with no direct contradiction, 50 or 55 is NOT a directional repair; use 58 or higher unless you clear directional arrays because the cited direct support is not materially probative.
...
- When a candidate includes `storedClaimDirection`, `originalBucket`, `directionBasis`, or `directnessJustification`, treat those fields as diagnostics only. They are not binding; decide the evidence direction from the AtomicClaim and the candidate evidence substance.
...
- For threshold or operator comparisons, normalize grouped numerals before comparing. Ignore digit-group separators such as spaces, thin spaces, apostrophes, and punctuation only when source-local notation or regular digit grouping makes them grouping separators between digit groups; never ignore punctuation that functions as a decimal marker. Preserve true decimal fractions. A value strictly above a greater-than threshold supports that threshold relation even when the positive margin is tiny; note narrow margins as caveats rather than reversing direction.
...
- `"direct_safeguard_record"` — the evidence directly records that a target-specific safeguard, remedy, review path, defense right, notice, hearing, appeal, access right, or equivalent protection was provided, exercised, heard, considered, granted, unavailable, blocked, found deficient, annulled, remedied, or otherwise legally resolved.
...
Use `directionBasis` as diagnostic metadata, not as a second veto over `claimDirection`. Choose `claimDirection` from the evidence's direct effect on the claim's truth condition, then make `directionBasis` explain that choice. Directional entries (`"supports"` or `"contradicts"`) must be justified by a directional basis: `"direct_substantive_finding"`, `"direct_metric_value"`, `"direct_source_native_comparison_side"`, `"direct_safeguard_record"`, or `"operative_standards_outcome"`. If the best basis is `"question_only"`, `"allegation_only"`, `"concern_only"`, `"procedural_fact_only"`, `"non_controlling_position_only"`, `"collateral_context"`, `"source_existence_only"`, or `"ambiguous_or_insufficient"`, set that claim's `claimDirection` to `"neutral"`. Do not pair a non-directional basis with `"supports"` or `"contradicts"` to express suspicion, unresolved allegation, appearance risk, source availability, or off-target context.

For rule-governed standard claims, a directional basis requires a concrete bridge to the same evaluated proceeding, process path, decision, verdict, final determination, safeguard, remedy, or standards outcome. The same judge, panel, court, institution, jurisdiction, claimant, defendant group, investigation theme, event family, supporters, allies, or other parties are not the same target by themselves. Procedural roles, public-access limits, alleged coordination, dual-role facts, criticism, concern, appearance-risk material, separate restrictive measures, custody conditions, adjacent litigation, or adjacent controversies are not directional unless the source states a target-specific safeguard denial/provision, compliance/noncompliance outcome, remedy, annulment, disqualification, right impairment, decision-validity effect, or legally effective standards result.

For standards, compliance, legality, procedure, or fairness claims, `direct_safeguard_record` is component-level directional evidence, not only final-outcome evidence. If the claim or `expectedEvidenceProfile` names safeguards, rights, remedies, decision artifacts, reasoned decisions, public hearings, evidence access, witness participation, appeal paths, recusal/remedy review, or comparable standard components, then an item that records that component for the directly evaluated target may support or contradict the claim even when no source states the whole standards conclusion. Use `"supports"` when the record shows the safeguard, review path, or target-path protection was provided, exercised, heard, considered, granted, or adjudicated without a source-stated standards defect. Use `"contradicts"` when the record shows the safeguard, access, participation, remedy, review, or protection was unavailable, blocked, denied/withheld in a way the source connects to right impairment or standards noncompliance, annulled/remedied because of a defect, or found deficient. Use `"neutral"` for bare chronology, role labels, docket membership, denied/rejected/upheld party requests without a source-stated standards defect, allegations, concerns, or appearance-risk material that does not establish a target-path safeguard/defect or operative standards outcome.

Do not demote a target-path safeguard record merely because it comes from the adjudicating institution, official docket, tribunal statement, case record, or process actor. Issuer perspective affects probative value, confidence, and source-portfolio weighting downstream; it does not erase that the record documents the target path. When a request, objection, recusal motion, appeal, access right, hearing, rebuttal opportunity, or comparable remedy is itself a component of the expected evidence profile, a record that the path was available, heard, considered, or decided may support the safeguard-availability component. Keep the underlying party allegation neutral unless it was adopted or produced an operative remedy.

Potential or apparent adjudicator bias, dual-role facts, victim/decision-maker overlap, court-composition controversy, party accusations, supporter accusations, or broad impartiality debate are not `direct_safeguard_record` by themselves. Use `"neutral"` with `"concern_only"` or `"procedural_fact_only"` unless the source reports an adopted recusal/disqualification ruling, an operative finding that the adjudicator was legally barred, or a legally effective remedy/annulment tied to the target proceeding.

Dissents, minority votes, expert views, party motions, and non-binding statements alleging right impairment are `"non_controlling_position_only"` unless they were adopted by the tribunal or produced a legally effective target-path consequence. Limited, narrow, short-deadline, clarification-only, low-probability, vote-count-limited, or unlikely-to-change-the-outcome appeal/remedy mechanics are `"procedural_fact_only"` or `"concern_only"` unless the source itself connects the remedy path to no effective review under the applicable standard, states a violation of the governing fair-trial/procedural standard, or reports an operative target-path remedy/annulment for that defect. Do not infer a standards violation from an outcome-probability prediction. Do not use `expectedEvidenceProfile`, `sourceNativeRoutes`, component names, or model background knowledge as the missing standards bridge; those fields describe what to look for, but the evidence item must itself supply the target-path standards outcome, right impairment, or legally effective defect before the claim-local direction becomes `"contradicts"`.
Broader institutional reports about related investigations, confidentiality practices, expression limits, precautionary measures, sanctions, or overlapping actors are `"collateral_context"` unless the item evaluates the exact target proceeding/path under the claim's standard or documents a concrete safeguard/defect in that target path.
...
### Evidence-Pool Direction Consistency

Before returning, run a consistency pass across the provided evidence items for each claim. If two evidence items report the same substantive statement, the same measured value, or the same source-native finding for the same AtomicClaim, their `claimDirectionByClaimId` entries for that claim must not conflict unless the items have materially different scope, time window, metric route, or target object that changes their effect on the claim. URL variants, mirrored publications, translated titles, or repeated extracts of the same source finding do not justify opposite directions by themselves.

If your reasoning discovers that a proposed direction should be corrected, the JSON `claimDirection` field must contain the corrected direction. Do not leave `claimDirection` as `"supports"` while the `directnessJustification` says the evidence refutes the claim, and do not leave it as `"contradicts"` while the `directnessJustification` says the evidence supports the claim. If the item remains genuinely ambiguous after this consistency pass, use `"neutral"` and explain the ambiguity.
...
Keep metric routes separate. If an item reports a period/window total, cumulative total, sub-count, category component, report existence, archive existence, or methodology/source-portfolio fact while the claim/profile route requires an endpoint stock, standing population, threshold, or other substantive value, use `"neutral"` for that claim unless the claim/profile explicitly accepts the item's route for the asserted relation. Route acceptance must come from the input-authored truth condition, decisive metric route, comparison relation, or an explicitly accepted metric class; a source family, archive/report route, category inventory, evidence-discovered observed value in `expectedEvidenceProfile`, or `componentMetrics` list does not by itself accept an alternate route. Treat totals for units admitted, hosted, served, processed, ever counted, or present for varying durations across a whole period as period/window or cumulative routes, not endpoint/standing-stock support. Do not turn source-existence or route-availability evidence into `"supports"` for a substantive numeric comparison merely because the named source is authoritative or likely contains relevant data.

For current threshold or current-stock claims with a direct `primaryMetric`, component rows and prior/reference rows may be materially relevant and useful for calibration, but they are not independent directional support merely because they are source-native or listed in `componentMetrics`. Mark an individual component or prior/reference row `"supports"` or `"contradicts"` only when the evidence item itself reports the complete decisive aggregate, states the source-native comparison relation that satisfies or refutes the claim, or documents a complete source-authorized composition from all required non-overlapping components in the same analytical window. Otherwise keep it `"neutral"` and explain its calibration role. Incomplete component arithmetic, missing addends, component-only totals, or claims that each component is below the aggregate threshold are not directional for an umbrella aggregate threshold.

Before returning each `claimDirectionByClaimId` entry, run this self-check:
1. If `claimDirection` is `"supports"` or `"contradicts"`, `directnessJustification` must name the target-specific substantive finding, measured value, source-native comparison side, safeguard record, operative standards outcome, or legally effective target-path result. If it cannot, return `"neutral"` and use the best non-directional `directionBasis`.
2. For rule-governed standard claims, role facts, objections, criticism, concern, appearance-risk material, alleged coordination, adjacent controversies, separate restrictive measures, custody conditions, and broader institutional context are not directional unless the item itself records the same target's safeguard/remedy action, right impairment, operative outcome, or standards conclusion.
3. For current-snapshot, endpoint, standing-stock, or threshold claims, older endpoint values, prior-period totals, alternate-window values, and alternate-category values are neutral context or calibration caveats unless they are the current/recent decisive route or the claim/profile explicitly accepts that route. A current/recent authoritative value that satisfies the threshold or comparison side is directional support even when label, scope, or boundary caveats remain; put those caveats in `directnessJustification` or `evidenceScope`, not in the opposite direction.
4. For current aggregate or threshold claims, if the proposed direction depends on partial components, omitted addends, inferred residual magnitudes, or comparing individual components to an umbrella threshold, return `"neutral"` and document the missing-composition caveat.
5. If another item in the batch reports the same substantive evidence for the same claim, verify that the two directions are consistent or that the scope/route/time/target difference that changes direction is explicit in `directnessJustification`.
...
- **contextual**: Evidence that provides relevant background but does not directly evaluate the target object. This includes:
  - Evidence from neutral external observers (international academic studies, NGO reports, comparative legal analyses) when it supplies background, general standards, comparative context, or commentary rather than concrete target-path documentation. Neutral external observers exclude foreign governments, foreign legislative bodies, executive offices, state media, and official government publications.
...
- For claims about whether a target process or decision satisfied legality, procedure, fairness, or similar rule-governed standards, evidence from an earlier or parallel episode, collateral inquiry, sanction episode, separate restrictive measure, adjacent litigation, or broader institutional controversy involving overlapping actors or institutions is `"contextual"` unless it directly documents the target path or explicitly states that the criticized/supportive procedure governed that same target.
...
- Foreign media or neutral external reporting on the directly evaluated target's events is usually "contextual" when it only summarizes events, commentary, allegations, criticism, or general background. It may be "direct" for a rule-governed standards claim when the item itself documents a concrete target-specific safeguard, defect, remedy, decision artifact, evidentiary act, appeal/review path, or standards outcome needed by the claim or `expectedEvidenceProfile`. Do not treat publisher nationality alone as a directness blocker; decide by the evidence substance and keep source quality/reliability for weighting.
- State media, government press offices, and official government publications are not neutral external observers — classify by the issuing authority.
...
- Neutral external reporting or analysis about the directly evaluated target remains "contextual" unless it supplies concrete sourced target-path evidence that qualifies as a direct safeguard/defect record, direct substantive finding, or operative standards outcome under the rules above. Foreign government's own actions or official assessments remain `foreign_reaction` under the issuing-authority rules.
...
- `directionBasis` is diagnostic metadata. It should explain the selected truth-direction label, but it must not override the claim-local direction when the evidence directly affects the claim's truth condition.
...
11. **Dimension Claims Stay Predicate-Faithful:** If other claims express dimension-specific readings, preserve the user's original predicate and present the dimension as a neutral qualifier such as `in terms of [dimension]`. Do not replace the original predicate with a proxy formulation such as feasibility, contribution, cost-effectiveness, technical success, or another subsystem-specific metric unless the input itself used that proxy wording.
    This applies to every thesis-direct dimension claim, not only one primary carrier. Do not preserve the predicate and then narrow it with an explanatory proxy tail or sufficient-condition rewrite. Remove explanatory tails unless their added content is explicit in the input. Dimension-specific statements should end after the neutral dimension qualifier; put mechanisms, thresholds, causal chains, and metric routes in `expectedEvidenceProfile` rather than the claim `statement`.
    For broad comparative efficiency/resource-use claims, a short neutral measurement-window or system-boundary label may be the dimension qualifier itself, including a parenthetical alias that merely names the same boundary. Do not delete that label merely because it is method-like or parenthetical; delete only added result claims, thresholds, causal mechanisms, loss narratives, sufficiency tests, or adjacent performance proxies not explicit in the input.
    If the Contract Validation Summary identifies broad-evaluative explanatory-tail drift, repair is primarily deletion, not reinterpretation: rewrite each affected `statement` by keeping the original predicate plus its neutral dimension qualifier and deleting the explanatory tail. Do not leave a dash, colon, "i.e.", parenthetical explanation, causal clause, complement clause, or equivalent tail that turns the dimension into a narrower proxy proposition. Do not substitute a different proxy predicate. Move any useful evidence route, measurable mechanism, threshold, causal chain, or metric target into `expectedEvidenceProfile` instead of the claim `statement`.
