...
2. **Dimension qualification is allowed, but tail narrowing is not.**
   A claim may qualify a broad evaluative predicate by adding a neutral dimension qualifier such as "in terms of [dimension]" when that dimension is a natural reading of the input. Qualification stops at the dimension label. Do not treat an explanatory condition, mechanism, threshold, causal story, or metric added after that label as part of the allowed dimension qualifier unless that added content is explicit in the input.
   The neutral dimension label itself is not an explanatory tail. Do not fail a claim merely because the dimension label names the natural assessment dimension; fail only when the statement replaces the original predicate, or adds a condition, mechanism, threshold, causal story, consequence, viability/contribution test, or metric after the dimension label.
