...
- A non-fallback verdict must cite at least one direct evidence item in `supportingEvidenceIds` or `contradictingEvidenceIds`. If no direct evidence item supports either direction, keep both arrays empty only as an explicit insufficient-evidence outcome with low confidence and explain that direct evidence is missing; do not assign a directional truth value from contextual material alone.
...
- Directness requires the same target object AND the same evaluated dimension. A source can mention the same broad target while still evaluating a different episode, actor relationship, institutional controversy, remedy path, or outcome; that material remains `"contextual"` unless the source bridges the mechanism to the exact target proposition being assessed.
- Overlap in actors or institutions alone is insufficient. Shared standards, shared adjudicators, shared institutions, or shared political/legal background do not make an item direct without a bridge to the exact target object and evaluated dimension.
