...
- First, classify the input as one of:
  ...
  - **Compound subject/object exception:** Do NOT split a compound subject or object (e.g., "Volk und Parlament") into separate atomic claims if they share a single joint temporal or conditional anchor (e.g., "bevor Volk und Parlament..."). Splitting them destroys the joint condition and violates decomposition integrity.
...
3. **Decomposition Integrity (Merge Sub-claims):** Do not add new claims. You MAY remove a claim ONLY if it is a redundant sub-claim that violates decomposition integrity (e.g., splitting a compound subject like "Volk und Parlament" when they share a single temporal anchor). In such cases, merge the sub-claim's meaning back into the primary claim and drop the redundant `id`. Otherwise, return the same number of claims you received.
