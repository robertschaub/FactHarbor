...
- If the input is a **multi_assertion_input**:
  ...
  - For current-versus-historical, current-versus-reference, named-side-versus-threshold, or other two-sided quantitative comparisons, do NOT return a whole-comparison claim alongside a separate claim for the current/named side. Choose either one unsplit claim when the comparison is truly atomic, or a decomposed set where the current/named side is one claim and the companion is written from the comparator/reference/relation side. The companion may contain only a compact relational reference to the already-isolated side; it must not independently reassert the isolated side as a second current-state fact.
  - In that decomposed comparison shape, the companion claim's `expectedEvidenceProfile` must still name the referenced side's metric/source-native route, the comparator/reference metric, and the comparison relation. Its `freshnessRequirement` must follow the freshest decisive side, even when that side is only a compact reference in the companion `statement`.
...
- For any thesis-direct quantitative comparison claim, this both-side profile rule applies whether the claim is unsplit or is a decomposed companion. Another AtomicClaim may separately isolate one side for reporting clarity, but it does not replace the comparison claim's own need to carry the side route, metric class, and freshest-side `freshnessRequirement`.
- When a decomposed comparison companion claim uses an anaphoric or compact reference back to an already-isolated side, its `expectedEvidenceProfile` must carry the referenced side's input-authored anchor, metric class, comparison relation, and likely source-native measurement route/source family when the input or preliminary evidence indicates one. Put route strings in `expectedEvidenceProfile.sourceNativeRoutes` when available. Keep this context in `expectedEvidenceProfile`, not as a new standalone proposition in the `statement`. Downstream relevance classification and evidence extraction need this profile context to retrieve both sides and classify one-sided source values directionally without making the companion claim non-atomic.
...
- comparison evidence-profile loss where a thesis-direct comparison claim semantically depends on multiple sides but its `expectedEvidenceProfile` or `freshnessRequirement` preserves only one side
...
1. Comparison decomposition integrity overrides verbatim-anchor preservation. A whole-comparison claim cannot be approved merely because it preserves original wording when another thesis-direct claim separately isolates one side of the same comparison.
2. Comparison evidence-profile and freshness integrity override anchor-carrier approval. A claim may preserve the input wording but still fail if its profile or freshness contract would cause downstream research to miss one decisive comparison side.
3. Sibling claims do not substitute for the target claim's own evidence contract. If a target thesis-direct comparison claim needs evidence from both sides, that target claim's `expectedEvidenceProfile` and `freshnessRequirement` must carry those obligations.
...
11. **Truth-condition-bearing modifier audit (MANDATORY).**
    ...
    This verbatim-presence guard does NOT override decomposition integrity for comparison claim sets: a near-verbatim whole-comparison claim is not an acceptable anchor carrier when another thesis-direct claim separately isolates one side of that same comparison. In that case, apply the decomposition integrity rule and fail validation.
...
20. **Decomposition integrity (MANDATORY).**
    ...
    This applies to decomposed comparison sentences as well and overrides generic verbatim-anchor approval: if one thesis-direct claim isolates a named/current-side metric or event, another thesis-direct claim may NOT restate that same side plus the full comparison/reference proposition. The companion claim must isolate the remaining comparator/reference or parity proposition instead of semantically subsuming the isolated-side claim. For approximate quantitative comparisons, fail validation if the companion comparator/reference claim copies the named/current-side numeric anchor onto the comparator/reference side as an exact or approximate standalone value that the input did not assign to that side. The approximation must remain a relation, not an invented comparator-side metric. Do NOT fail merely because the companion claim contains a short reference-link back to the already-isolated quantity, including its input-authored number, when that reference-link is only the comparator for the remaining relation and is not asserted as a separate current-state fact inside the companion claim. If the companion claim uses an anaphoric or compact reference, verify that its `expectedEvidenceProfile` carries the referenced side's input-authored anchor, metric class, and comparison relation; if both the statement and profile omit context needed for downstream evidence direction, treat that as material anchor loss and set `rePromptRequired: true`. If the profile preserves a present/current referenced side but `freshnessRequirement` drops to `"none"` even though that side must be measured as a current stock, latest status, present-state total, or current administrative count, treat that as material freshness loss and set `rePromptRequired: true`.
    Also fail the double-loss case: a thesis-direct comparison claim still semantically depends on both a current/named side and a comparator/reference side, but its `expectedEvidenceProfile` names only the comparator/reference side or only the current/named side. Treat this as material profile loss even if another AtomicClaim separately isolates the missing side. If any missing or preserved side requires current or present-state evidence, `freshnessRequirement: "none"` is material freshness loss.
...
10. **Clause-Scoped Modifier Fidelity:** If the anchor modifies one clause/action in a multi-clause sentence, fuse it into that clause's action. Do not shift it onto a different clause, distribute it beyond its original scope, or turn it into a standalone effect claim.
11. **No Sub-claims:** Do not externalize the anchor into a supporting sub-claim; fuse it with the action it modifies.
12. **No New Inference:** Repair the existing claim set only. Do not add chronology, causality, legality, or verdict language not already present in the current claims or original input.
