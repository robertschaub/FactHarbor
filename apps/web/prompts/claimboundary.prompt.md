...
- `claimDirection`:
  ...
  - **Target-specific vs. comparator/precedent evidence for target-object claims — classify by what the evidence evaluates, not what topic it shares:**
    ...
    - Limited, narrow, short-deadline, or low-probability appeal/remedy mechanics are not contradictions merely because they make reversal unlikely. They contradict only when the source states that no effective review was available under the applicable standard, or that the remedy path itself violated the governing fair-trial/procedural standard.
...
Dissents, minority votes, expert views, party motions, and non-binding statements alleging right impairment are `"non_controlling_position_only"` unless they were adopted by the tribunal or produced a legally effective target-path consequence. Limited, narrow, short-deadline, or low-probability appeal/remedy mechanics are `"procedural_fact_only"` or `"concern_only"` unless the source states that no effective review was available under the applicable standard or that the remedy path itself violated the governing fair-trial/procedural standard.
