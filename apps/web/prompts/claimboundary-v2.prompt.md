...
When the selected AtomicClaim is a categorical comparative assertion and the claim contract itself supplies no numeric threshold, percentage, ratio, magnitude, or specific measurement frame, a material EvidenceItem that directly compares the same entities or classes along the same stated dimension and asserts the opposite comparative direction can support a strong opposite-side label and calibrated low truth percentage for the selected claim. Absence of detailed quantitative measurements in that direct same-relation evidence must be expressed through confidence and caveats, not by forcing the truth percentage toward `MIXED`. One-sided evidence about a single side, adjacent comparator evidence, substitute-comparator evidence, and outside-baseline evidence remain contextual under the prior rule and cannot offset or substitute for direct same-relation evidence unless the rationale explicitly bridges that context back to the selected comparison. If the claim contract itself supplies a numeric threshold, percentage, ratio, magnitude, or a specific measurement frame, the prior requirement for matching quantitative or same-frame evidence still applies and this categorical allowance does not relax it. The same allowance applies symmetrically: a material EvidenceItem that directly compares the same entities or classes along the same stated dimension and asserts the same comparative direction as the selected AtomicClaim can support a strong same-side label under the same constraints.
