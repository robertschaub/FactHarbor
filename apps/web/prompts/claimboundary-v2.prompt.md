...
For direct comparative input, preserve the comparison endpoints as submitted. If the input states that one method, material, product, policy, treatment, route, source, or class for a purpose performs better or worse than another endpoint, the selected AtomicClaim must keep both endpoints, the stated purpose when supplied, and the compared property or outcome. Do not substitute a broader, narrower, default, conventional, adjacent, inverse, or more common comparator for either endpoint. If an endpoint is shorthand, colloquial, or ambiguous, preserve its submitted wording in the AtomicClaim and leave interpretation, caveats, and measurement-frame refinement to later evidence stages.
...
### Target-Frame and Direction Checkpoint

Before selecting EvidenceItems, check the selected AtomicClaim's target relation: subject or actor, asserted property or outcome, comparator or baseline, measurement frame, time/status posture, category or threshold, and polarity. Do not output this checklist; use it to decide whether a supplied source point is direct, partial, contextual, or off-frame.

Evidence about an adjacent or substitute comparator, one side of a comparison, a broader topic, a generic benefit or limitation, or the inverse of the selected claim may be useful context, but it must not be labeled as direct support or direct opposition unless the supplied content itself bridges the gap to the selected claim's same target relation. When such evidence is included, express the mismatch through `claimDirection`, `probativeValue`, `evidenceStrength`, `extractionConfidence`, `evidenceScope.limitations`, and `provenance.rationale`.

When the supplied packets contain materially aligned points on both sides of the selected claim, or a categorical claim has both limitation evidence and countervailing value, benefit, compliance, effectiveness, or support evidence, include a balanced bounded set where the 2-to-5 item budget permits. Prefer materially distinct claim-direct contrasts over repeated same-side limitations, broad background, or adjacent-domain context. If only partial or indirect target-frame evidence is available, an accepted output may still contain limited, contextual, mixed, or unclear items, but the rationale and item metadata must make the direct-evidence gap visible for downstream sufficiency and verdict calibration.

### Temporal and Procedural Relation Claims

For claims whose selected AtomicClaim turns on the order between an event and another decision, approval, ratification, vote, publication, implementation, deadline, status change, or other procedural milestone, extract distinct EvidenceItems for the relation components when supplied content supports them. Useful components can include: the source-attributed date or status of the first event; the source-attributed date or status of the compared milestone; evidence that the compared milestone occurred after the first event; evidence that the compared milestone was still pending, not yet held, not yet submitted, not yet finalized, or otherwise not completed at the relevant time; and source-attributed distinctions between preliminary, institutional, formal, final, or procedural stages when those distinctions affect the relation. Treat a source-attributed pending or not-yet-completed status as evidence about the temporal relation, not as absence of evidence, while preserving uncertainty in `claimDirection`, `probativeValue`, `evidenceStrength`, `extractionConfidence`, `evidenceScope.limitations`, and `provenance.rationale`.
...
For an accepted branch, the only valid top-level `status` literal is `accepted`; the only valid accepted `extractionStatus` literals are `evidence_extracted` and `no_extractable_evidence`. Do not emit alternate success, completion, partial, or extraction status strings. If the accepted branch cannot be represented exactly with these literals and field nullability rules, return the appropriate blocked or damaged branch under the schema instead of inventing a new branch shape.
...
For endpoint-comparison claims, evidence comparing one selected endpoint to an outside baseline, default alternative, conventional reference, adjacent class, or substitute comparator may be extracted only as contextual, unclear, mixed, or limited unless the supplied content explicitly bridges that outside comparison back to the selected endpoints and compared property. Do not label such outside-baseline evidence as `supports` or `opposes`, do not give it high probative value, and do not let it crowd out lower-volume source material that directly addresses the selected endpoints. If the supplied packets contain only outside-baseline comparison material, make that comparator gap visible in the EvidenceItem statement, `evidenceScope.limitations`, and `provenance.rationale`.
...
For selected endpoint comparisons, direct EvidenceItems that explicitly preserve both specific compared endpoints from the selected AtomicClaim and assert the opposite comparative direction may support a stronger opposite-side label and lower truth percentage for that selected AtomicClaim. "Both specific compared endpoints" means the endpoints as written in the selected claim, including submitted shorthand when present; it does not mean a broader category, adjacent alternative, default baseline, inverse frame, or substitute comparator. Evidence that names only one endpoint, replaces either endpoint, or compares against an outside baseline remains contextual under the adjacent-comparator rule and must not drive selected-claim truth calibration unless the evidence itself explicitly bridges the mismatch.

### Pre-Verdict Target-Frame Checkpoint

Before assigning any true-side or false-side label, check whether the cited EvidenceItems directly establish the selected AtomicClaim's target relation, not a related or easier proposition. If the best available evidence depends mainly on an adjacent comparator, one-sided context, generic benefit or limitation, broad institutional background, inverse wording, or a partial component with unresolved bridge to the selected claim, use a caveated lower-confidence candidate, `MIXED`, or `UNVERIFIED` rather than a confident true-side or false-side candidate.

Treat supplied `claimDirection` labels as evidence metadata, not as automatic verdict instructions. If an EvidenceItem statement, scope, limitation, or rationale shows that its comparator, category, time posture, standard, or polarity is narrower, broader, adjacent, inverse, or only partially aligned, calibrate from that visible limitation even when the item is labeled `supports` or `opposes`.

### Selected Claim Coherence and Candidate Economy

Each accepted verdict candidate must answer a materially distinct assessment of the selected AtomicClaim. A context-only boundary, residual procedural milestone, expected downstream step, theoretical caveat, or adjacent-benefit statement that does not independently change the selected claim's truth conditions should normally be represented as caveat, material uncertainty, context boundary, or limitation on the top-line candidate, not as an equal second verdict candidate.

Frame each verdict candidate around the truth of the selected AtomicClaim as written. Do not silently replace the selected claim with its inverse, negation, narrower opposite, or a more convenient counter-proposition. Evidence that establishes an opposite proposition must be translated into lower truth for the selected AtomicClaim, not into a true-side verdict for the opposite proposition. A verdict title or rationale may mention opposing evidence, benefits, harms, compliance, non-compliance, purpose, lack of purpose, existence, non-existence, effectiveness, or ineffectiveness, but `internalVerdictLabelCandidate` and `internalTruthPercentageCandidate` must still answer the selected AtomicClaim itself.

When the selected AtomicClaim uses categorical or absolute wording about value, usefulness, effectiveness, existence, legality, fairness, or causal relation, calibrate the verdict against the whole categorical assertion. Evidence of limited performance, inefficiency, incompleteness, caveats, process barriers, or a pending procedural posture can support a caveated same-side candidate, but it should not by itself justify a strong same-side label for the whole categorical assertion when the cited evidence or uncertainty signals leave material countervailing value, benefit, compliance, support, or unresolved scope, including unresolved source-attributed usefulness. If your rationale or uncertainty signals say the evidence establishes only a weaker adjacent proposition than the selected AtomicClaim, the label and truth percentage must reflect that gap; do not leave the gap only in caveat prose while still assigning a strong same-side verdict. Conversely, evidence of some countervailing value or caveat should not flip the whole claim unless it materially addresses the same claim relation. Use calibrated labels, truth percentages, confidence, caveats, and material uncertainty signals to preserve nuance.

For temporal or procedural sequence claims, if the supplied EvidenceItems support the asserted order between the focal event and a later decision or milestone, do not create a separate `UNVERIFIED` verdict candidate merely because the later decision, review, vote, implementation, or legal effect remains pending or caveated. Keep that pending posture as a caveat or material uncertainty on the claim-aligned top-line candidate unless the selected AtomicClaim separately asserts that the later milestone has already happened or that legal effect is already final.
...
`internalTruthPercentageCandidate` is the estimated truth of the selected AtomicClaim itself, not confidence in the chosen verdict label and not confidence that the opposite of the claim is true. Choose `internalVerdictLabelCandidate` and `internalTruthPercentageCandidate` as one coupled judgment from the same seven-point truth scale: `TRUE` 86-100, `MOSTLY-TRUE` 72-85, `LEANING-TRUE` 58-71, `MIXED` or `UNVERIFIED` 43-57, `LEANING-FALSE` 29-42, `MOSTLY-FALSE` 15-28, `FALSE` 0-14. True-side labels require high truth scores for the selected claim; never pair `TRUE`, `MOSTLY-TRUE`, or `LEANING-TRUE` with a low or false-side truth percentage. False-side labels require low truth scores for the selected claim; never pair `FALSE`, `MOSTLY-FALSE`, or `LEANING-FALSE` with a high or true-side truth percentage. If cited evidence mainly contradicts the selected AtomicClaim, use a false-side label with a low truth score for that claim. If cited evidence mainly supports the selected AtomicClaim, use a true-side label with a high truth score for that claim. If direct support and contradiction are both material or unresolved, use `MIXED` or `UNVERIFIED` in the 43-57 band. Use `internalConfidenceCandidate` to express confidence in the evidence and calibration.
