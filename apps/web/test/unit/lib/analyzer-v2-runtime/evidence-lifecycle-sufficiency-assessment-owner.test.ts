...
describe("sufficiency assessment runtime owner", () => {
  ...
  it("projects dimension/materiality enums into missingEvidenceDimensionProjections for accepted W6-C result (V2-RL-020)", async () => {
    const resultWithDimensions: EvidenceSufficiencyResult = {
      schemaVersion: EVIDENCE_SUFFICIENCY_ASSESSMENT_SCHEMA_VERSION,
      taskKey: "evidence_sufficiency",
      status: "accepted",
      sufficiencyAssessment: {
        ...
        missingEvidenceDimensions: [
          { dimension: "source_diversity", materiality: "material", rationale: "Provider rationale must not appear in projection" },
          { dimension: "direct_evidence", materiality: "material", rationale: "Another provider rationale" },
        ],
        recommendedNextAction: "refine_retrieval",
        materialScarcityCandidate: "possible",
        rationale: "Overall assessment rationale must not appear in projection.",
      },
      integrityEvents: [],
      blockedReason: null,
      damagedReason: null,
    };
    vi.mocked(generateText).mockResolvedValueOnce(generateTextResult(resultWithDimensions));
    const w5Decision = markBoundedEvidenceExtractionRuntimeOwnedDecision(w5());
    const decision = await runSufficiencyAssessmentDecision({
      context: context(),
      sufficiencyIntake: intakeFrom(w5Decision),
    ...
    // Amendment 5 negative-scope: no free-text, rationale, sufficiencyStatus, materialScarcityCandidate
    const serializedProjection = JSON.stringify(decision.missingEvidenceDimensionProjections);
    expect(serializedProjection).not.toContain("Provider rationale must not appear");
    expect(serializedProjection).not.toContain("Another provider rationale");
    expect(serializedProjection).not.toContain("sufficiencyStatus");
    expect(serializedProjection).not.toContain("materialScarcityCandidate");
    expect(serializedProjection).not.toContain("needs_refinement");
    expect(serializedProjection).not.toContain("rationale");

    // Amendment 5 negative-scope on full decision: excluded fields must not leak
    const serializedDecision = JSON.stringify(decision);
    expect(serializedDecision).not.toContain("Provider rationale must not appear");
    expect(serializedDecision).not.toContain("Overall assessment rationale must not appear");
  });

  it("rejects runtime ownership when missingEvidenceDimensionProjections contains invalid values (V2-RL-020 provenance)", async () => {
    vi.mocked(generateText).mockResolvedValueOnce(generateTextResult(acceptedSufficiencyResult()));
    const w5Decision = markBoundedEvidenceExtractionRuntimeOwnedDecision(w5());
    const decision = await runSufficiencyAssessmentDecision({
      context: context(),
    ...
    // Start with a valid owned decision
    expect(inspectSufficiencyAssessmentRuntimeOwnership(decision)).toBe("owned");

    // Tamper with the projection to include an invalid dimension value
    const tamperedInvalidDimension = {
      ...decision,
      missingEvidenceDimensionProjections: [
        { dimension: "invalid_dimension", materiality: "material" },
      ],
    };
    expect(inspectSufficiencyAssessmentRuntimeOwnership(tamperedInvalidDimension)).not.toBe("owned");

    // Tamper with the projection to include an invalid materiality value
    const tamperedInvalidMateriality = {
      ...decision,
      missingEvidenceDimensionProjections: [
        { dimension: "source_diversity", materiality: "critical" },
      ],
    };
    expect(inspectSufficiencyAssessmentRuntimeOwnership(tamperedInvalidMateriality)).not.toBe("owned");

    // Tamper with the projection to include extra keys (rationale leaking)
    const tamperedExtraKeys = {
      ...decision,
      missingEvidenceDimensionProjections: [
        { dimension: "source_diversity", materiality: "material", rationale: "leaked text" },
      ],
    };
    expect(inspectSufficiencyAssessmentRuntimeOwnership(tamperedExtraKeys)).not.toBe("owned");

    // Tamper with the projection to be a non-array
    const tamperedNonArray = {
      ...decision,
      missingEvidenceDimensionProjections: "not_an_array",
    };
    expect(inspectSufficiencyAssessmentRuntimeOwnership(tamperedNonArray)).not.toBe("owned");
  });
});
