...
describe("Stage-1 prompt contract", () => {
  ...
  describe("decomposition integrity guard", () => {
    it("forbids whole-input carry-through when a comparison sentence is decomposed", () => {
      ...
      expect(pass2).toContain("Claim-level bundling prohibition");
      expect(pass2).toContain("If ANY returned thesis-direct claim still depends on two or more independently verifiable sub-propositions");
      expect(pass2).toContain("even when the overall claim set contains other valid claims");
      expect(pass2).toContain("one evaluative or rule-governed predicate applied to multiple distinct target objects, processes, decisions, outcomes, or other evaluands");
      expect(pass2).toContain("you MUST split that claim");
      expect(pass2).toContain("named/current side plus a separate proposition about the comparator/reference side");
      expect(pass2).toContain("Preserve the shared relation, predicate, or comparison anchor in each split claim");
      expect(pass2).toContain("A conjunctive clause with one shared verb phrase does not override this rule");
      expect(pass2).toContain("This merge rule NEVER overrides the claim-level bundling prohibition above");
      ...
      expect(contract).toContain("Claim-level bundling audit (MANDATORY)");
      expect(contract).toContain("any thesis-direct claim still depends on two or more independently verifiable sub-propositions");
      expect(contract).toContain("even if the extractor returned other thesis-direct claims that are valid");
      ...
      expect(contract).toContain("shared-predicate evaluative cases");
      expect(contract).toContain("Do NOT approve a bundled claim merely because it appears to preserve an \"expanded scope\"");
      ...
      expect(contract).toContain("distinct institutions, actor groups, proceedings, decision gates, target objects, or outcomes");
