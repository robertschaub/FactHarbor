...
describe("Stage-4 prompt contract", () => {
  ...
  describe("verdict prose contract", () => {
    ...
    it("verdict prompts keep neutral standards caveats from defeating one-sided direct support", () => {
      ...
      expect(advocate).toContain("do not move the verdict to a false-side, low-truth, MIXED, or UNVERIFIED result solely because unanswered components or concerns remain");
      expect(advocate).toContain("Keep the truth at or above the weak true-side band");
      ...
      expect(reconciliation).toContain("do not accept a challenge that lowers the verdict to false-side, low-truth, MIXED, or UNVERIFIED solely from neutral concern-only");
      expect(reconciliation).toContain("It may cap confidence, but it does not defeat direct target-path safeguard support and should not reduce truth into MIXED or UNVERIFIED");
      ...
      expect(repair).toContain("set truth at or above the weak true-side band unless the evidence pool contains direct contradicting items");
    });
...
describe("Stage-2 prompt contract", () => {
  ...
  describe("APPLICABILITY_ASSESSMENT", () => {
    ...
    it("can map side-specific evidence to decomposed comparison claims through expectedEvidenceProfile", () => {
      ...
      expect(section).toContain("Route acceptance must come from the input-authored truth condition, decisive metric route");
      expect(section).toContain("evidence-discovered observed value in `expectedEvidenceProfile`");
