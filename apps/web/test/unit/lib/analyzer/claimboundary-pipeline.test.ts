  ...
  describe("buildClaimBoundaryResultJson", () => {
    it("persists articleAdjudication and adjudicationPath into resultJson", () => {
      ...
      const assessment: OverallAssessment = {
        ...
        articleAdjudication: {
          articleTruthPercentage: 42,
          articleConfidence: 71,
          dominanceAssessment: {
            mode: "single",
          ...
          claimWeightRationale: [{ claimId: "AC_01", effectiveInfluence: "primary", reasoning: "Decisive." }],
          adjudicationReasoning: "Single decisive claim.",
        },
        adjudicationPath: {
          baselineAggregate: { truthPercentage: 60, confidence: 71 },
          directionConflict: true,
          llmAdjudication: {
            rawTruthPercentage: 42,
            rawConfidence: 71,
            dominanceAssessment: { mode: "single", dominantClaimId: "AC_01", strength: "decisive", rationale: "AC_01 governs." },
            claimWeightRationale: [{ claimId: "AC_01", effectiveInfluence: "primary", reasoning: "Decisive." }],
            adjudicationReasoning: "Single decisive claim.",
          },
          guardsApplied: { deviationCapped: false, confidenceCeiled: false, integrityDowngraded: false, boundsClamped: false },
...
describe("filterByCentrality", () => {
  ...
  it("should preserve required repair carrier claims through the centrality cap", () => {
    const claims = makeClaims(["high", "high", "high", "low"]);
...
describe("Stage 1: runPass2", () => {
  ...
  it("should keep audit mode on the base Pass 2 prompt without loading the binding appendix", async () => {
    const pass2Fixture = {
      impliedClaim: "Entity A achieved metric X",
      backgroundDetails: "Background info",
      articleThesis: "Overall thesis",
    ...
    expect(renderedSections).toContain("CLAIM_EXTRACTION_PASS2");
    expect(renderedSections).not.toContain("CLAIM_EXTRACTION_PASS2_BINDING_APPENDIX");
    ...
    expect(firstCall.messages[0].content).not.toContain("CLAIM_EXTRACTION_PASS2_BINDING_APPENDIX");
  });

  it("should append the binding appendix when precommitted salience anchors are active", async () => {
    const pass2Fixture = {
      impliedClaim: "Entity A achieved metric X",
      backgroundDetails: "Background info",
      articleThesis: "Overall thesis",
...
describe("Stage 5: aggregateAssessment (integration)", () => {
  ...
  it("caps article-level adjusted confidence at the deterministic baseline", async () => {
    ...
    // Narrative adjustedConfidence (95) exceeds deterministic baseline (68) → capped at 68
    expect(result.confidence).toBe(68);
    // Narrative is explanatory-only — truth is always the baseline for same-direction jobs
    expect(result.truthPercentage).toBe(75);
  });

  it("narrative cannot override article truth (explanatory-only)", async () => {
    await mockStage5Configs();

    // Even if the narrative somehow returned adjustedTruthPercentage (stale schema),
    // the code ignores it — truth comes from baseline or LLM adjudication only.
    const narrativeOutput = {
      headline: "Adjusted article verdict",
      evidenceBaseSummary: "1 item",
  ...
  it("narrative adjustedConfidence caps confidence downward", async () => {
    await mockStage5Configs();

    const narrativeWithCeiling = {
      headline: "Confidence capped",
      evidenceBaseSummary: "1 item",
      keyFinding: "The narrative caps confidence lower than the baseline.",
      limitations: "One claim remains unverified.",
      adjustedConfidence: 40,
    };
    mockLoadSection.mockResolvedValue({ content: "prompt", variables: {} } as any);
    mockGenerateText.mockResolvedValue({ text: JSON.stringify(narrativeWithCeiling) } as any);
    mockExtractOutput.mockReturnValue(narrativeWithCeiling as any);
...
// ============================================================================
// AGGREGATION: DOMINANCE + COMPLETE-ASSESSMENT GUARD
// ============================================================================

describe("aggregateAssessment article adjudication and direction conflict (Option G)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  async function setupConfigMocks(adjudicationEnabled: boolean) {
    const { loadPipelineConfig, loadCalcConfig } = await import("@/lib/config-loader");
    vi.mocked(loadPipelineConfig).mockResolvedValue({ config: {} as any } as any);
    vi.mocked(loadCalcConfig).mockResolvedValue({
      config: {
        aggregation: {
          centralityWeights: { high: 3.0, medium: 2.0, low: 1.0 },
          derivativeMultiplier: 0.5,
          articleAdjudication: { enabled: adjudicationEnabled, maxDeviationFromBaseline: 30, borderlineMargin: 10 },
        },
        mixedConfidenceThreshold: 40,
      } as any,
    } as any);
  ...
  it("returns adjudicationPath baseline_same_direction when adjudication is disabled", async () => {
    ...
    expect(result.adjudicationPath!.llmAdjudication).toBeUndefined();
    expect(result.articleAdjudication).toBeUndefined();
  });

  it("same-direction claims: uses baseline even when adjudication is enabled", async () => {
    await setupConfigMocks(true);
    mockLoadSection.mockResolvedValue({ content: "prompt", variables: {} } as any);
    mockGenerateText.mockResolvedValue({ text: JSON.stringify(narrativeOutput) } as any);
    mockExtractOutput.mockReturnValue(narrativeOutput);

    // Both claims are true-leaning (both supports_thesis, both > 60%)
    const claims = [
      createAtomicClaim({ id: "AC_01" }),
      createAtomicClaim({ id: "AC_02", statement: "Claim 2" }),
    ];
    ...
    // Truth equals baseline — narrative cannot override
  ...
  it("direction conflict with adjudication enabled: fires LLM adjudication", async () => {
    ...
    const adjudicationPayload = {
      ...
      articleConfidence: 70,
      dominanceAssessment: {
        mode: "single", dominantClaimId: "AC_02", strength: "decisive",
        rationale: "AC_02 is the defining proposition.",
      },
      claimWeightRationale: [
        { claimId: "AC_01", effectiveInfluence: "minor", reasoning: "Supporting context." },
        { claimId: "AC_02", effectiveInfluence: "primary", reasoning: "Decisive claim." },
      ],
      adjudicationReasoning: "The decisive false claim overrides the true supporting claim.",
    };
    mockLoadSection.mockResolvedValue({ content: "prompt", variables: {} } as any);
    mockGenerateText.mockResolvedValue({ text: "" } as any);
    mockExtractOutput
    ...
    // Direction conflict: AC_01=90 true-leaning, AC_02=20 false-leaning
    ...
    expect(result.articleAdjudication!.dominanceAssessment.mode).toBe("single");
    // LLM said 35% — within 30pp deviation cap of baseline, so it should be applied
    const baseline = result.adjudicationPath!.baselineAggregate.truthPercentage;
    expect(result.truthPercentage).toBeLessThan(baseline);
  });

  it("direction conflict but adjudication disabled: uses baseline", async () => {
    await setupConfigMocks(false);
    mockLoadSection.mockResolvedValue({ content: "prompt", variables: {} } as any);
    mockGenerateText.mockResolvedValue({ text: JSON.stringify(narrativeOutput) } as any);
    mockExtractOutput.mockReturnValue(narrativeOutput);

    // Direction conflict but adjudication disabled
    const claims = [createAtomicClaim({ id: "AC_01" }), createAtomicClaim({ id: "AC_02", statement: "Claim 2" })];
    const verdicts = [
      createCBClaimVerdict({ claimId: "AC_01", truthPercentage: 90, confidence: 88, confidenceTier: "HIGH" }),
    ...
    // Direction conflict exists but adjudication is disabled → baseline_fallback (not baseline_same_direction)
    expect(result.adjudicationPath!.path).toBe("baseline_fallback");
    expect(result.articleAdjudication).toBeUndefined();
  });

  it("single-claim inputs skip adjudication entirely", async () => {
    await setupConfigMocks(true);
    mockLoadSection.mockResolvedValue({ content: "prompt", variables: {} } as any);
    mockGenerateText.mockResolvedValue({ text: JSON.stringify(narrativeOutput) } as any);
    mockExtractOutput.mockReturnValue(narrativeOutput);
  ...
  it("inverted or out-of-window LLM articleTruthRange is normalized after clamping", async () => {
    await setupConfigMocks(true);
    // LLM returns an inverted range (min > max) outside the deviation window
    const adjudicationPayload = {
      articleTruthPercentage: 35,
      articleConfidence: 70,
      articleTruthRange: { min: 80, max: 20 }, // inverted and outside cap window
      dominanceAssessment: {
        mode: "none",
        rationale: "No single claim dominates.",
      },
      claimWeightRationale: [
        { claimId: "AC_01", effectiveInfluence: "significant", reasoning: "Important." },
        { claimId: "AC_02", effectiveInfluence: "significant", reasoning: "Important." },
      ],
      adjudicationReasoning: "Both claims matter.",
    };
    mockLoadSection.mockResolvedValue({ content: "prompt", variables: {} } as any);
    mockGenerateText.mockResolvedValue({ text: "" } as any);
    mockExtractOutput
    ...
    // Direction conflict: AC_01=90 true-leaning, AC_02=15 false-leaning
    const claims = [createAtomicClaim({ id: "AC_01" }), createAtomicClaim({ id: "AC_02", statement: "Claim 2" })];
    const verdicts = [
      createCBClaimVerdict({ claimId: "AC_01", truthPercentage: 90, confidence: 88, confidenceTier: "HIGH" }),
      createCBClaimVerdict({ claimId: "AC_02", truthPercentage: 15, confidence: 75, confidenceTier: "MEDIUM" }),
    ];
    const boundaries = [createClaimAssessmentBoundary({ id: "CB_01" })];
    const coverageMatrix = buildCoverageMatrix(claims, boundaries, []);
    ...
    expect(result.adjudicationPath!.path).toBe("llm_adjudicated");
    // Range must be normalized: min <= max, and within deviation cap
    const range = result.truthPercentageRange;
...
describe("Stage 1: extractClaims reprompt loop", () => {
  ...
  function makeCoordinatedPass2(claims: AtomicClaim[]) {
    return {
      ...makePass2(claims.length, { inputClassification: "multi_assertion_input" }),
      atomicClaims: claims,
    };
  }

  function makeContractResult(preserves: boolean, claimIds: string[], summary: string) {
    return {
      inputAssessment: {
        preservesOriginalClaimContract: preserves,
        rePromptRequired: !preserves,
        summary,
      },
      claims: claimIds.map((claimId) => ({
        claimId,
        preservesEvaluativeMeaning: preserves,
        usesNeutralDimensionQualifier: false,
        proxyDriftSeverity: preserves ? "none" : "material",
        recommendedAction: preserves ? "keep" : "retry",
        reasoning: preserves ? "complete coherent decomposition axis" : "partial cross-product omits an in-scope pairing",
      })),
      antiInferenceCheck: {
  ...
  async function setupContractExtractionTest() {
  ...
  function makeExtractionState(originalInput: string): any {
    return {
      originalInput,
      inputType: "claim",
      understanding: null,
  ...
  it("accepts a complete subject-axis split for coordinated criteria", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    await setupContractExtractionTest();

    const subjectSplit = [
      createAtomicClaim({ id: "AC_01", statement: "Subject A satisfied criteria X and Y.", centrality: "high" }),
      createAtomicClaim({ id: "AC_02", statement: "Subject B satisfied criteria X and Y.", centrality: "high" }),
    ];
    ...
    mockExtractOutput.mockImplementation(() => {
      llmCallIndex++;
      switch (llmCallIndex) {
        case 1: return pass1Fixture;
        case 2: return makeCoordinatedPass2(subjectSplit);
        case 3: return makeContractResult(true, ["AC_01", "AC_02"], "complete subject-axis split preserves all criteria");
        case 4: return makeGate1Pass(2);
        default: throw new Error(`Unexpected LLM call #${llmCallIndex}`);
      }
    });
    mockGenerateText.mockResolvedValue({ text: "" } as any);

    const result = await extractClaims(makeExtractionState("Subjects A and B satisfied criteria X and Y."));
  ...
  it("accepts a complete criteria-axis split for coordinated subjects", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    await setupContractExtractionTest();

    const criteriaSplit = [
      createAtomicClaim({ id: "AC_01", statement: "Subjects A and B satisfied criterion X.", centrality: "high" }),
      createAtomicClaim({ id: "AC_02", statement: "Subjects A and B satisfied criterion Y.", centrality: "high" }),
    ];
    ...
    mockExtractOutput.mockImplementation(() => {
      ...
      switch (llmCallIndex) {
        case 1: return pass1Fixture;
        case 2: return makeCoordinatedPass2(criteriaSplit);
        case 3: return makeContractResult(true, ["AC_01", "AC_02"], "complete criteria-axis split preserves all subjects");
        case 4: return makeGate1Pass(2);
        default: throw new Error(`Unexpected LLM call #${llmCallIndex}`);
      }
    });
  ...
  it("recovers when an incomplete cross-product retry resolves to one complete axis", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    await setupContractExtractionTest();

    const partialCrossProduct = [
      createAtomicClaim({ id: "AC_01", statement: "Subject A satisfied criterion X.", centrality: "high" }),
      createAtomicClaim({ id: "AC_02", statement: "Subject A satisfied criterion Y.", centrality: "high" }),
      createAtomicClaim({ id: "AC_03", statement: "Subject B satisfied criterion X.", centrality: "high" }),
    ];
    const completeCriteriaSplit = [
      createAtomicClaim({ id: "AC_01", statement: "Subjects A and B satisfied criterion X.", centrality: "high" }),
      createAtomicClaim({ id: "AC_02", statement: "Subjects A and B satisfied criterion Y.", centrality: "high" }),
    ];
    ...
    mockExtractOutput.mockImplementation(() => {
      ...
      switch (llmCallIndex) {
        case 1: return pass1Fixture;
        case 2: return makeCoordinatedPass2(partialCrossProduct);
        case 3: return makeContractResult(false, ["AC_01", "AC_02", "AC_03"], "partial cross-product omits Subject B with criterion Y");
        case 4: return makeCoordinatedPass2(completeCriteriaSplit);
        case 5: return makeContractResult(true, ["AC_01", "AC_02"], "retry chose a complete criteria-axis split");
        case 6: return makeGate1Pass(2);
        default: throw new Error(`Unexpected LLM call #${llmCallIndex}`);
      }
    });
    mockGenerateText.mockResolvedValue({ text: "" } as any);
    ...
    expect(result.atomicClaims.map((claim) => claim.statement)).toEqual([
      "Subjects A and B satisfied criterion X.",
      "Subjects A and B satisfied criterion Y.",
    ]);
    expect(result.contractValidationSummary).toMatchObject({
      preservesContract: true,
      rePromptRequired: false,
  ...
  it("keeps the damaged contract state when partial cross-product retry still fails", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    await setupContractExtractionTest();
    ...
    mockExtractOutput.mockImplementation(() => {
      ...
      switch (llmCallIndex) {
        ...
        case 2: return makeCoordinatedPass2(partialCrossProduct);
        case 3: return makeContractResult(false, ["AC_01", "AC_02", "AC_03"], "partial cross-product omits Subject B with criterion Y");
        case 4: return makeCoordinatedPass2(partialCrossProduct);
        case 5: return makeContractResult(false, ["AC_01", "AC_02", "AC_03"], "retry still omits an in-scope pair");
        case 6: return makeGate1Pass(3);
        default: throw new Error(`Unexpected LLM call #${llmCallIndex}`);
      }
    });
    mockGenerateText.mockResolvedValue({ text: "" } as any);
    ...
    expect(result.contractValidationSummary).toMatchObject({
      ...
      summary: "partial cross-product omits Subject B with criterion Y",
    });
  });
  ...
  it("should refresh contractValidationSummary when contract retry output is accepted", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    const { loadPipelineConfig, loadSearchConfig, loadCalcConfig } = await import("@/lib/config-loader");
        ...
        case 5:
          return {
            inputAssessment: {
              ...
              rePromptRequired: false,
              summary: "contract preserved after retry",
            },
            claims: [
              {
                claimId: "AC_01",
    ...
    expect(result.atomicClaims).toHaveLength(1);
    expect(result.atomicClaims[0].statement.toLowerCase()).toContain("final");
    expect(result.contractValidationSummary).toMatchObject({
      ran: true,
      preservesContract: true,
      rePromptRequired: false,
        ...
        case 7:
          return {
            inputAssessment: {
              preservesOriginalClaimContract: true,
              rePromptRequired: false,
              summary: "final accepted claims still preserve the repaired anchor",
            },
            claims: [
              { claimId: "AC_01", preservesEvaluativeMeaning: true, usesNeutralDimensionQualifier: true, proxyDriftSeverity: "mild", recommendedAction: "keep", reasoning: "timing preserved" },
              { claimId: "AC_02", preservesEvaluativeMeaning: true, usesNeutralDimensionQualifier: true, proxyDriftSeverity: "mild", recommendedAction: "keep", reasoning: "timing preserved" },
            ...
            anchors: [
              {
                ...
                inputSpan: "final",
                type: "modal_illocutionary",
                rationale: "Finality changes the claim's meaning.",
                truthConditionShiftIfRemoved: "The claim loses the finality qualifier.",
              },
            ],
          };
        case 3:
  ...
  it("should persist disabled salience status distinctly from success or failure", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    const { loadPipelineConfig, loadSearchConfig, loadCalcConfig } = await import("@/lib/config-loader");
    ...
    vi.mocked(loadCalcConfig).mockResolvedValue({
      config: {
        claimDecomposition: { minCoreClaimsPerContext: 1, supplementalRepromptMaxAttempts: 0 },
        claimContractValidation: { enabled: false, maxRetries: 1 },
        salienceCommitment: { enabled: false, mode: "audit" },
        mixedConfidenceThreshold: 40,
      } as any,
      contentHash: "__TEST__", fromDefault: false, fromCache: false, overrides: [],
    } as any);
  ...
  it("MT-5(C): should NOT trigger when a 1-claim set is already contract-approved", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    const { loadPipelineConfig, loadSearchConfig, loadCalcConfig } = await import("@/lib/config-loader");
    ...
    mockExtractOutput.mockImplementation(() => {
      ...
      switch (llmCallIndex) {
        ...
        case 3:
          return {
            inputAssessment: {
              preservesOriginalClaimContract: true,
              rePromptRequired: false,
              summary: "initial single-claim set preserves the contract",
            },
            claims: [
              {
                claimId: "AC_01",
            ...
            truthConditionAnchor: {
              presentInInput: true,
              anchorText: "rechtskräftig bevor Volk und Parlament darüber entschieden haben",
              preservedInClaimIds: ["AC_01"],
              preservedByQuotes: ["rechtskräftig bevor Volk und Parlament darüber entschieden haben"],
            },
            antiInferenceCheck: {
              normativeClaimInjected: false,
              injectedClaimIds: [],
        ...
        case 4:
          return makeGate1Pass(1);
        default:
          throw new Error(`Unexpected LLM call #${llmCallIndex} — MT-5(C) should not reprompt a contract-approved single-claim set`);
      }
    });
    mockGenerateText.mockResolvedValue({ text: "" } as any);
