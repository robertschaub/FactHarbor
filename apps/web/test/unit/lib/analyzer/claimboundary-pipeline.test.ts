...
describe("Stage 1: extractClaims reprompt loop", () => {
  ...
  it("should run a contract safety retry when contract validation returns undefined for a multi-claim output", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    ...
    mockExtractOutput.mockImplementation(() => {
      ...
      switch (llmCallIndex) {
        case 1:
          return pass1Fixture;
        case 2:
          return { ...makePass2(3), inputClassification: "ambiguous_single_claim" };
        case 3:
          return undefined; // Initial contract validation fail-open
        case 4:
          return { ...makePass2(2), inputClassification: "ambiguous_single_claim" }; // Safety retry
        case 5:
          return {
            inputAssessment: {
              preservesOriginalClaimContract: true,
              rePromptRequired: false,
              summary: "Merged overlapping dimensions",
            },
            claims: [
              {
                claimId: "AC_01",
    ...
    const state: any = {
      originalInput: "Broad evaluative input that should not stay over-fragmented",
      inputType: "claim",
      understanding: null,
      evidenceItems: [],
      sources: [],
    ...
    expect(result.contractValidationSummary?.summary).toContain("fail-open");
  });

  it("should keep the original pass2 when contract safety retry still fails validation even if it has fewer claims", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    const { loadPipelineConfig, loadSearchConfig, loadCalcConfig } = await import("@/lib/config-loader");
    ...
    mockExtractOutput.mockImplementation(() => {
      ...
      switch (llmCallIndex) {
        ...
        case 3:
          return undefined;
        case 4:
          return { ...makePass2(2), inputClassification: "ambiguous_single_claim" };
        case 5:
          return {
            inputAssessment: {
              preservesOriginalClaimContract: false,
              rePromptRequired: true,
              summary: "Still overlapping",
            },
            claims: [
              {
                claimId: "AC_01",
                ...
                usesNeutralDimensionQualifier: true,
                proxyDriftSeverity: "none",
                evidenceSeparable: false,
                recommendedAction: "retry",
                reasoning: "same evidence body",
              },
              {
                claimId: "AC_02",
                preservesEvaluativeMeaning: true,
    ...
    const state: any = {
      originalInput: "Broad evaluative input that should not accept a still-bad smaller split",
      inputType: "claim",
      understanding: null,
      evidenceItems: [],
      sources: [],
    ...
    expect(result.atomicClaims).toHaveLength(3);
  });

  it("should tell retry extraction to return exactly one claim for single_atomic_claim contract corrections", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    const { loadPipelineConfig, loadSearchConfig, loadCalcConfig } = await import("@/lib/config-loader");
    ...
    mockExtractOutput.mockImplementation(() => {
      ...
      switch (llmCallIndex) {
        case 1:
          return pass1Fixture;
        case 2:
          return { ...makePass2(3), inputClassification: "single_atomic_claim" };
        case 3:
          return {
            inputAssessment: {
              preservesOriginalClaimContract: false,
              rePromptRequired: true,
              summary: "Input should remain unified",
            },
            claims: [
              {
                claimId: "AC_01",
        ...
        case 4:
          return { ...makePass2(1), inputClassification: "single_atomic_claim" };
        case 5:
          return {
            inputAssessment: {
              preservesOriginalClaimContract: true,
              rePromptRequired: false,
              summary: "Unified claim restored",
            },
            claims: [
              {
                claimId: "AC_01",
                ...
                evidenceSeparable: true,
                recommendedAction: "keep",
                reasoning: "broad claim preserved",
              },
            ],
          };
        case 6:
    ...
    const state: any = {
      originalInput: "Single broad evaluative input",
      inputType: "claim",
      understanding: null,
      evidenceItems: [],
      sources: [],
    ...
    expect(result.atomicClaims).toHaveLength(1);
    const retryPass2Call = mockGenerateText.mock.calls[3]?.[0] as any;
    expect(retryPass2Call.messages[1].content).toContain("return exactly 1 atomic claim");
  });

  it("should keep the original pass2 when contract retry still fails even if retry has fewer claims", async () => {
    const { extractClaims } = await import("@/lib/analyzer/claimboundary-pipeline");
    const { loadPipelineConfig, loadSearchConfig, loadCalcConfig } = await import("@/lib/config-loader");
    ...
    mockExtractOutput.mockImplementation(() => {
      ...
      switch (llmCallIndex) {
        ...
        case 4:
          return { ...makePass2(2), inputClassification: "single_atomic_claim" };
        case 5:
          return {
            inputAssessment: {
              preservesOriginalClaimContract: false,
              rePromptRequired: true,
              summary: "Still overlapping after merge",
            },
            claims: [
              {
                claimId: "AC_01",
    ...
    const state: any = {
      originalInput: "Single broad evaluative input that should keep the original split if retry still fails",
      inputType: "claim",
      understanding: null,
      evidenceItems: [],
      sources: [],
