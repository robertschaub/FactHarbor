...
describe("sufficiency assessment runtime", () => {
  ...
  it("projects missingEvidenceDimension enum pairs from accepted sufficiency result (V2-RL-020 diagnostic projection)", async () => {
    const multiDimensionResult: EvidenceSufficiencyResult = {
      schemaVersion: EVIDENCE_SUFFICIENCY_ASSESSMENT_SCHEMA_VERSION,
      taskKey: "evidence_sufficiency",
      status: "accepted",
      sufficiencyAssessment: {
        ...
        missingEvidenceDimensions: [
          { dimension: "source_diversity", materiality: "material", rationale: "Only one encyclopedic source" },
          { dimension: "counter_evidence", materiality: "material", rationale: "No opposing evidence found" },
          { dimension: "temporal_coverage", materiality: "minor", rationale: "Recent data only" },
        ],
        recommendedNextAction: "refine_retrieval",
        materialScarcityCandidate: "possible",
        rationale: "Retrieval diversity insufficient for robust assessment.",
      },
      integrityEvents: [],
      blockedReason: null,
      damagedReason: null,
    ...
    const decision = await runSufficiencyAssessmentRuntime({
      ...
      promptContentHash: "8".repeat(64),
      configSnapshotHash: "config-hash-w6c",
      providerCall: providerCall(multiDimensionResult),
    });
    ...
    expect(decision.missingEvidenceDimensionProjections).toEqual([
      ...
      { dimension: "temporal_coverage", materiality: "minor" },
    ]);

    // Amendment 5 — negative-scope: each entry has ONLY dimension and materiality keys
    for (const projection of decision.missingEvidenceDimensionProjections) {
      expect(Object.keys(projection).sort()).toEqual(["dimension", "materiality"]);
    }

    // Amendment 5 — negative-scope: no rationale, sufficiencyStatus, materialScarcityCandidate, free-text, source text, quotes, or provider identifiers
    const serialized = JSON.stringify(decision.missingEvidenceDimensionProjections);
    expect(serialized).not.toContain("Only one encyclopedic source");
    expect(serialized).not.toContain("No opposing evidence found");
    expect(serialized).not.toContain("Recent data only");
    expect(serialized).not.toContain("rationale");
    expect(serialized).not.toContain("sufficiencyStatus");
    expect(serialized).not.toContain("materialScarcityCandidate");
    expect(serialized).not.toContain("needs_refinement");
    expect(serialized).not.toContain("possible");
    expect(serialized).not.toContain("anthropic");
    expect(serialized).not.toContain("wikimedia");

    // Amendment 5 — positive-scope: dimension values are within allowed set
    const allowedDimensions = new Set([
      "source_diversity", "direct_evidence", "counter_evidence",
      "temporal_coverage", "method_quality", "source_access", "other",
    ]);
    const allowedMateriality = new Set(["none", "minor", "material"]);
    for (const projection of decision.missingEvidenceDimensionProjections) {
      expect(allowedDimensions.has(projection.dimension)).toBe(true);
      expect(allowedMateriality.has(projection.materiality)).toBe(true);
    }
  });

  it("returns empty missingEvidenceDimensionProjections for accepted result with no missing dimensions", async () => {
    const sufficientResult: EvidenceSufficiencyResult = {
      schemaVersion: EVIDENCE_SUFFICIENCY_ASSESSMENT_SCHEMA_VERSION,
      taskKey: "evidence_sufficiency",
      status: "accepted",
      sufficiencyAssessment: {
        ...
        materialScarcityCandidate: "none",
        rationale: "Evidence corpus is sufficient.",
      },
      integrityEvents: [],
      blockedReason: null,
      damagedReason: null,
    ...
    const decision = await runSufficiencyAssessmentRuntime({
      ...
      promptContentHash: "8".repeat(64),
      configSnapshotHash: "config-hash-w6c",
      providerCall: providerCall(sufficientResult),
    });
  ...
  it("returns empty missingEvidenceDimensionProjections for blocked and damaged decisions", async () => {
    const w5Decision = w5();
    const intake = intakeFrom(w5Decision);

    // Blocked decision (no intake)
    const blockedDecision = await runSufficiencyAssessmentRuntime({
      context: context(),
  ...
  it("keeps redaction.sufficiencyResultPayloadReturned false even with diagnostic projection populated (V2-RL-020)", async () => {
    const w5Decision = w5();
    const result: EvidenceSufficiencyResult = {
      schemaVersion: EVIDENCE_SUFFICIENCY_ASSESSMENT_SCHEMA_VERSION,
      taskKey: "evidence_sufficiency",
      status: "accepted",
      sufficiencyAssessment: {
        sufficiencyStatus: "needs_refinement",
        missingEvidenceDimensions: [
          { dimension: "direct_evidence", materiality: "material", rationale: "No direct source evidence" },
        ],
        recommendedNextAction: "refine_retrieval",
        materialScarcityCandidate: "none",
        rationale: "Needs direct evidence from primary sources.",
      },
      integrityEvents: [],
      blockedReason: null,
      damagedReason: null,
    ...
    const decision = await runSufficiencyAssessmentRuntime({
      ...
      promptContentHash: "8".repeat(64),
      configSnapshotHash: "config-hash-w6c",
      providerCall: providerCall(result),
    });

    expect(decision.missingEvidenceDimensionProjections).toHaveLength(1);
    expect(decision.redaction.sufficiencyResultPayloadReturned).toBe(false);

    // Full decision serialization must not contain rationale text or excluded fields
    const serializedDecision = JSON.stringify(decision);
    expect(serializedDecision).not.toContain("No direct source evidence");
    expect(serializedDecision).not.toContain("Needs direct evidence from primary sources");
    expect(serializedDecision).not.toContain("needs_refinement");
  });
