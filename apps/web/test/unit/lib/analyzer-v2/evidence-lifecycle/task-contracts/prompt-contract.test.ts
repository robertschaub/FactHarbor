...
describe("analyzer-v2 Evidence Lifecycle prompt task contracts", () => {
  ...
  it("renders the evidence-extraction strict output branch contract", () => {
    ...
    expect(section).toContain("prefer 4 to 5 EvidenceItems");
    ...
    expect(section).toContain("hard schema maximum, not a target");
    expect(section).toContain("returning 6 or more EvidenceItems is invalid");
    expect(section).toContain("never emit a sixth item");
    expect(section).toContain("oversized array");
    ...
    expect(section).toContain("strict `provenance` object with exactly `locator` and `rationale`");
    expect(section).toContain("`provenance.rationale` must be one concise string");
    expect(section).toContain("Do not add any extra `provenance` keys of any name");
    expect(section).toContain("the only valid top-level `status` literal is `accepted`");
    expect(section).toContain("the only valid accepted `extractionStatus` literals are `evidence_extracted` and `no_extractable_evidence`");
    expect(section).toContain("Do not emit alternate success, completion, partial, or extraction status strings");
    expect(section).toContain("### Source Packet Reference Integrity");
    expect(section).toContain("copy its `sourceRecordId` and `contentPacketId` exactly from one allowed pair");
    expect(section).toContain("structural foreign keys");
    expect(section).toContain("Do not use `locatorRef`, `candidatePreviewId`, `sourceMaterialEndpointId`");
    expect(section).toContain("If a useful evidence point cannot be tied to one exact supplied pair");
    expect(section).toContain("use only selected AtomicClaim IDs from `claimContractJson`");
    expect(section).toContain("Do not invent, translate, shorten, renumber, or normalize claim IDs");
