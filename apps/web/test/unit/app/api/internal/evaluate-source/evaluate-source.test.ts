...
describe("Evidence ID parsing", () => {
  it("extracts evidence IDs across common formats", () => {
    const extracted = extractEvidenceIdsFromText(
  ...
  it("counts unique evidence IDs with evidenceId and basis", () => {
    ...
    expect(countUniqueEvidenceIds(result as any, pack as any)).toBe(3);
  });
});
...
describe("Refinement confidence boost tiers", () => {
  ...
  it("does not boost when refinement adds nothing", () => {
    ...
    const { boost } = computeRefinementConfidenceBoost(
      base as any,
      base as any,
      pack as any,
      false
...
describe("Language detection caveat", () => {
  it("returns caveats for timeout/failure", () => {
    languageDetectionStatus.set("timeout.example", "timeout");
    languageDetectionStatus.set("failed.example", "failed");
    languageDetectionStatus.set("ok.example", "ok");

    expect(getLanguageDetectionCaveat("timeout.example")).toMatch(/timed out/);
    expect(getLanguageDetectionCaveat("failed.example")).toMatch(/failed/);
    expect(getLanguageDetectionCaveat("ok.example")).toBeNull();
  });
});
