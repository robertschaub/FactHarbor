...
function readQuery(req: Request): { ledgerId: string; inspectDraftText: boolean } | null {
  const params = new URL(req.url).searchParams;
  const allowed = new Set(["ledgerId", "inspectDraftText"]);
  ...
  const inspectDraftTextParam = params.get("inspectDraftText");
  if (inspectDraftTextParam !== null && inspectDraftTextParam !== "true") {
    return null;
  }

  return { ledgerId, inspectDraftText: inspectDraftTextParam === "true" };
}
...
export async function GET(req: Request) {
  ...
  const artifacts = query.inspectDraftText
    ? readInternalAlphaReportDraftRuntimeArtifactInspectionProjections(query.ledgerId)
    : readInternalAlphaReportDraftRuntimeArtifactDefaultProjections(query.ledgerId);
  ...
  return json({
    ...
    visibility: "internal_admin_only",
    publicPointerExposure: "forbidden",
    defaultProjection: query.inspectDraftText
      ? "explicit_authenticated_admin_draft_markdown"
