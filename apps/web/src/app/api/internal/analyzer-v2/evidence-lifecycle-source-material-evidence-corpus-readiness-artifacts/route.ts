...
function readLedgerId(req: Request): string | null {
  ...
  if (
    ...
    || ledgerId.length > EVIDENCE_LIFECYCLE_SOURCE_MATERIAL_EVIDENCE_CORPUS_READINESS_ARTIFACT_MAX_LEDGER_ID_LENGTH
  ) {
    return null;
  }
...
export async function GET(req: Request) {
  ...
  const artifacts = readEvidenceLifecycleSourceMaterialEvidenceCorpusReadinessRuntimeArtifacts(ledgerId);
