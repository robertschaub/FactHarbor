...
5. **UNVERIFIED ambiguity discipline:** Treat every `UNVERIFIED` job as an unresolved suspicious outcome for review purposes. Agents MUST NOT accept `UNVERIFIED` as genuine real-world evidence scarcity from the inspected job alone. First inspect the exact job and local acquisition health, then apply `/report-review`'s UNVERIFIED Ambiguity Escalation when doing report review, quality/regression review, suspicious-job debugging, or any classification that would conclude genuine evidence scarcity. Escalation compares prior exact same-input runs when available, including prepared and selected `AtomicClaims`, and performs a bounded external evidence check before classifying the result. A bounded external evidence check uses the supplied input, source URL/title when available, source-language and multilingual neutral search variants, and likely authoritative or primary-source paths; stop when material accessible evidence is found, enough missed evidence exists to classify a FactHarbor miss, or multiple reasonable authoritative/primary-source searches fail to surface material evidence. `UNVERIFIED` may be accepted as analytical reality only after accessible material evidence appears insufficient and source acquisition appears healthy; otherwise classify it as `factharbor-miss`, `mixed`, or `inconclusive`. Lightweight monitoring may record `UNVERIFIED` as suspicious and preserve the need for escalation. Do not submit live LLM reruns, validation batches, or expensive tests unless the Live Job Submission Discipline permits it.
6. **Live inspection:** Keep the in-app browser on the relevant app or job page when it materially helps live inspection.
7. **Documentation:** Record findings, causes, fixes, decisions, validation state, and residual risks through the Exchange Protocol, `/handoff`, or `/context-extension` as appropriate.
